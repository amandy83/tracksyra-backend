export class FraudStore {
    ...
    async getTrackOwner(trackId) {
        const rows = await this.db.query(`SELECT user_id FROM tracks WHERE id = :trackId LIMIT 1`, { trackId });
        return rows[0]?.user_id ?? null;
    }
    async getPreviousHourStreams(trackId, timestamp) {
        const rows = await this.db.query(`SELECT COALESCE(SUM(stream_count_increment), 0)::int AS streams_count
       FROM streaming_events
       WHERE track_id = :trackId
         AND occurred_at >= CAST(:timestamp AS timestamptz) - INTERVAL '1 hour'
         AND occurred_at < CAST(:timestamp AS timestamptz)`, { trackId, timestamp });
        return rows[0]?.streams_count ?? 0;
    }
    async getDistinctCountriesLast5m(trackId, timestamp) {
        const rows = await this.db.query(`SELECT COUNT(DISTINCT listener_country)::int AS country_count
       FROM streaming_events
       WHERE track_id = :trackId
    ...
    async getSameFingerprintEventsLast10m(input) {
        if (!input.ipFingerprint && !input.deviceFingerprint)
            return 0;
    ...
    async getShortDurationEventsLast10m(trackId, timestamp) {
        const rows = await this.db.query(`SELECT COUNT(*)::int AS count
       FROM streaming_events
       WHERE track_id = :trackId
         AND occurred_at >= CAST(:timestamp AS timestamptz) - INTERVAL '10 minutes'
         AND occurred_at <= CAST(:timestamp AS timestamptz)
         AND COALESCE((raw_payload->>'listen_duration_seconds')::numeric, 999999) < 15`, { trackId, timestamp });
        return rows[0]?.count ?? 0;
    }
    async getRevenueAndStreamsLastDay(trackId, timestamp) {
        const rows = await this.db.query(`SELECT
         ...
         COALESCE((SELECT SUM(stream_count_increment) FROM streaming_events
           WHERE track_id = :trackId
             ...
             AND occurred_at <= CAST(:timestamp AS timestamptz)), 0)::int AS streams`, { trackId, timestamp });
        return { revenue: Number(rows[0]?.revenue ?? 0), streams: rows[0]?.streams ?? 0 };
    }
    async getDistributionFailuresLastDay(trackId, timestamp) {
        const rows = await this.db.query(`SELECT COUNT(*)::int AS count
       FROM distribution_state_history
       WHERE track_id = :trackId
         AND next_status IN ('FAILED', 'REJECTED', 'DEAD_LETTER')
         AND created_at >= CAST(:timestamp AS timestamptz) - INTERVAL '1 day'
         AND created_at <= CAST(:timestamp AS timestamptz)`, { trackId, timestamp });
        return rows[0]?.count ?? 0;
    }
    async appendFraudEvent(input) {
        const rows = await this.db.query(`INSERT INTO fraud_events (
       ...
       RETURNING id, event_id, track_id, user_id, platform, decision, fraud_score,
        ...
        if (rows[0])
            return rows[0];
        const existing = await this.db.query(`SELECT id, event_id, track_id, user_id, platform, decision, fraud_score,
         reasons, raw_event, created_at
       FROM fraud_events
       WHERE event_id = :eventId
       LIMIT 1`, { eventId: input.event.event_id });
        return existing[0];
    }
    async createReview(fraudEventId) {
        await this.db.query(`INSERT INTO fraud_reviews (fraud_event_id, decision, reviewer_id, notes)
       VALUES (:fraudEventId, 'PENDING', NULL, NULL)
       ON CONFLICT (fraud_event_id) WHERE decision = 'PENDING' DO NOTHING`, { fraudEventId });
    }
    async getFraudEventByReview(reviewId) {
        const rows = await this.db.query(`SELECT fe.id AS fraud_event_id, fe.user_id, fe.raw_event
    ...
    async appendReviewDecision(input) {
        await this.db.query(`INSERT INTO fraud_reviews (fraud_event_id, decision, reviewer_id, notes)
       VALUES (:fraudEventId, :decision, :reviewerId, :notes)`, input);
    }
    async upsertUserRiskScore(input) {
        await this.db.query(`INSERT INTO fraud_user_risk_scores (user_id, risk_score, reason)
