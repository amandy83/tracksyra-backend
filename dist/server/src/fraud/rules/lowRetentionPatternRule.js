export class LowRetentionPatternRule {
    code = "LOW_RETENTION_PATTERN";
    evaluate(features) {
        const currentShortListen = typeof features.listen_duration_seconds === "number" && features.listen_duration_seconds < 15;
        if (!currentShortListen && features.short_duration_events_last_10m < 20)
            return null;
        if (features.short_duration_events_last_10m < 10 && features.stream_count_increment < 10)
            return null;
        return {
            rule: this.code,
