...
export function createTooLostPkcePair(state = randomBytes(24).toString("hex")) {
    const codeVerifier = randomBytes(32).toString("base64url");
    const codeChallenge = createHash("sha256").update(codeVerifier).digest("base64url");
    return { state, codeVerifier, codeChallenge };
}
export function createTooLostAuthorizationUrl(input) {
    if (!input.clientId)
        throw new Error("Too Lost client ID is required.");
    if (!input.redirectUri)
        throw new Error("Too Lost redirect URI is required.");
...
export async function exchangeTooLostAuthorizationCode(input) {
    if (!input.code)
        throw new Error("Too Lost authorization code is required.");
    if (!input.codeVerifier)
        throw new Error("Too Lost code verifier is required.");
    if (!input.clientId)
        throw new Error("Too Lost client ID is required.");
    if (!input.clientSecret)
        throw new Error("Too Lost client secret is required.");
    if (!input.redirectUri)
        throw new Error("Too Lost redirect URI is required.");
...
export async function refreshTooLostAccessToken(input) {
    if (!input.refreshToken)
        throw new Error("Too Lost refresh token is required.");
    if (!input.clientId)
        throw new Error("Too Lost client ID is required.");
    if (!input.clientSecret)
        throw new Error("Too Lost client secret is required.");
...
export function resolveTooLostOAuthEnvironment() {
...
function parseTooLostTokenResponse(response, failureMessage, fallbackRefreshToken) {
