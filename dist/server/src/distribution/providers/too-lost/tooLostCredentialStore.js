...
import { readTooLostConfig } from "./tooLostOAuth.js";
import { rethrowTooLostStageError } from "./tooLostError.js";
import { getTooLostStatusCache } from "./tooLostStatusCache.js";
export class TooLostCredentialStore {
    client;
    constructor(client) {
        this.client = client;
    }
    async syncProviderConfiguration() {
        ...
        const { data: existing, error: readError } = await this.client
            ...
            .maybeSingle();
        if (readError)
            throw new Error(`Failed to read current Too Lost provider configuration: ${readError.message}`);
        ...
        if (error)
            throw new Error(`Failed to sync Too Lost provider configuration: ${error.message}`);
        ...
        if (credentialError)
            throw new Error(`Failed to sync Too Lost credential configuration: ${credentialError.message}`);
    }
    async initializePendingCredentialRecord() {
        const now = new Date().toISOString();
        ...
        if (error)
            throw new Error(`Failed to initialize Too Lost credential record: ${error.message}`);
    }
    async storeOAuthState(input) {
        ...
        if (error)
            throw new Error(`Failed to store Too Lost OAuth state: ${error.message}`);
    }
    async loadOAuthState(state) {
        const { data, error } = await this.client
            ...
            .maybeSingle();
        if (error)
            throw new Error(`Failed to read Too Lost OAuth state: ${error.message}`);
    ...
    async markOAuthStateCompleted(state) {
        const { error } = await this.client
            ...
            .eq("state", state);
        if (error)
            throw new Error(`Failed to complete Too Lost OAuth state: ${error.message}`);
    }
    async storeTokenSet(token, input = {}) {
        ...
        const { error } = await this.client
            ...
            .eq("provider", "too_lost");
        if (error)
            throw new Error(`Failed to store Too Lost token references: ${error.message}`);
    ...
    async loadTokenSet() {
        try {
            const { data, error } = await this.client
                ...
                .maybeSingle();
            if (error)
                throw new Error(`Failed to read Too Lost token references: ${error.message}`);
            if (!data)
                return null;
            return {
    ...
    async clearConnection(reason = "Disconnected by operator") {
        const now = new Date().toISOString();
        const { error } = await this.client
            .from("distribution_provider_credentials")
            .update({
            ...
            .eq("provider", "too_lost");
        if (error)
            throw new Error(`Failed to clear Too Lost token references: ${error.message}`);
        const { error: providerError } = await this.client
            ...
            .eq("provider", "too_lost");
        if (providerError)
            throw new Error(`Failed to clear Too Lost provider status: ${providerError.message}`);
        this.writeStatusCache({
            connected: false,
    ...
    async updateProviderSyncStatus(input) {
        const { error } = await this.client
            ...
            .eq("provider", "too_lost");
        if (error)
            throw new Error(`Failed to update Too Lost provider sync status: ${error.message}`);
        if (input.lastError) {
            const { error: credentialError } = await this.client
                ...
                .eq("provider", "too_lost");
            if (credentialError)
                throw new Error(`Failed to persist Too Lost sync error: ${credentialError.message}`);
        }
    ...
    async recordHealth(health) {
        ...
        if (error)
            throw new Error(`Failed to record Too Lost health checks: ${error.message}`);
    }
    async recordSyncLog(input) {
        ...
        if (error)
            throw new Error(`Failed to record Too Lost sync log: ${error.message}`);
    }
    async recordSandboxRun(input) {
        ...
        if (error)
            throw new Error(`Failed to record Too Lost sandbox run: ${error.message}`);
    }
    ...
    async refreshConnectionStatus(reason = "background_refresh") {
        const cache = getTooLostStatusCache();
    ...
    async loadConnectionStatusSnapshot() {
        let credentials;
        try {
        ...
        try {
            const { data, error: providerError } = await this.client
                ...
                .maybeSingle();
            if (providerError)
                throw new Error(`Failed to read Too Lost provider status: ${providerError.message}`);
            provider = data;
        }
        catch (error) {
        ...
        try {
            const { data, error: oauthError } = await this.client
                ...
                .maybeSingle();
            if (oauthError)
                throw new Error(`Failed to read Too Lost OAuth state: ${oauthError.message}`);
            oauthState = data;
        }
        catch (error) {
        ...
        const connectionStatus = !credentials?.accessToken
            ? "needs_authorization"
    ...
    writeStatusCache(status, source) {
        getTooLostStatusCache().write(status, source);
    }
}
export function makeCodeVerifierRef(codeVerifier) {
    return makeSecretRef("too_lost_oauth_code_verifier", codeVerifier);
}
function makeSecretRef(prefix, secret) {
...
function encryptSecret(secret) {
    ...
    if (!key)
        throw new Error("TOO_LOST_TOKEN_ENCRYPTION_KEY is required for encrypted token storage.");
