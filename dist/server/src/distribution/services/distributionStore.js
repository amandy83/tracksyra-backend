...
export class SqlDistributionStore {
    ...
    async getReleaseWithTracks(releaseId) {
       ...
       WHERE release_id = :releaseId
        ...
        const mappedTracks = await Promise.all(musicRelease.audioFiles.map(async (audio) => {
            ...
            const writers = mergeWriterParticipants(sourceTrack?.composer ? [{ name: sourceTrack.composer, role: ["composer"] }] : [], contributorsByTrack.get(audio.trackId) ?? []);
    ...
    async getTrackWithRelease(trackId) {
        const rows = await this.db.query(`SELECT r.id AS release_id, COALESCE(r.artist_id, r.user_id) AS release_user_id,
    ...
    async getJobPayload(job) {
       ...
       WHERE release_id = :releaseId
         ...
         AND role IN ('composer', 'lyricist', 'songwriter')`, { releaseId: row.release_id, trackId: row.track_id });
        ...
        const writers = mergeWriterParticipants(row.composer ? [{ name: row.composer, role: ["composer"] }] : [], buildWriterParticipantMap(contributorRows).get(row.track_id) ?? []);
    ...
    async ensurePlatformDelivery(input) {
        await this.db.query(`INSERT INTO platform_deliveries (release_id, track_id, user_id, platform, status)
       VALUES (:releaseId, :trackId, :userId, :platform, 'PENDING')
       ON CONFLICT (track_id, platform) WHERE track_id IS NOT NULL DO NOTHING`, input);
    }
    async createDistributionJob(input) {
        const rows = await this.db.query(`INSERT INTO distribution_jobs (release_id, track_id, platform, status)
       VALUES (:releaseId, :trackId, :platform, 'PENDING')
       ON CONFLICT (track_id, platform) DO NOTHING
       RETURNING id, release_id, track_id, platform, status, created_at, attempts, next_retry_at`, input);
        const row = rows[0];
        if (!row)
            return null;
        return {
    ...
    async getPendingJobs(limit) {
        const rows = await this.db.query(`SELECT id, release_id, track_id, platform, status, created_at, attempts, next_retry_at
    ...
    async isWebhookConfirmed(input) {
       ...
       WHERE release_id = :releaseId
         ...
         AND source = 'WEBHOOK'
         AND next_status IN ('SUBMITTED', 'IN_REVIEW', 'APPROVED', 'DELIVERED', 'REJECTED')`, input);
    ...
    async recordDeliveryResult(input) {
       ...
       WHERE release_id = :releaseId AND track_id = :trackId AND platform = :platform`, {
        ...
        await this.db.query(`INSERT INTO distribution_sync_logs (
...
function asRecordWithUnknown(value) {
...
function normalizeTrackMetadata(value) {
    return asRecordWithUnknown(value);
}
...
function buildWriterParticipantMap(rows) {
