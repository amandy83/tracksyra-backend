...
function buildBoomplayMetadata(job) {
    const release = job.release;
    const metadata = release ? releaseMetadata(release) : {};
    ...
    const regionalLanguage = normalizeRegionalLanguage(metadata.BoomplayLanguage
    ...
    async normalizeMetadata(job) {
        ...
        return new ConnectorMetadata({
            ...
            payload: freeze({
                ...
                regionalLanguage: normalizeRegionalLanguage(metadata.BoomplayLanguage
                    ?? metadata.regionalLanguage
                    ?? metadata.language
                    ?? null),
                textDirection: isRtlLanguage(normalizeRegionalLanguage(metadata.BoomplayLanguage
                    ?? metadata.regionalLanguage
                    ?? metadata.language
                    ?? null)) ? "rtl" : "ltr",
                rtlRenderingCompatible: Boolean(isRtlLanguage(normalizeRegionalLanguage(metadata.BoomplayLanguage
                    ?? metadata.regionalLanguage
                    ?? metadata.language
                    ?? null))),
                territories: normalizedList(job.target.territories).map((territory) => territory.toUpperCase()),
...
export class BoomplayErrorTranslator {
    ...
    translate(error, job) {
        ...
        const code = /auth|unauthoriz|forbidden|token/i.test(message) ? "BOOMPLAY_MUSIC_AUTH_FAILED"
