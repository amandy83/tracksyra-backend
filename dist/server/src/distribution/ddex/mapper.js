...
function extractMetadataRecord(source) {
    return Object.freeze({ ...(source ?? {}) });
}
function getMetadataString(record, key) {
    return stableText(record[key]);
}
function getMetadataBoolean(record, key) {
...
function normalizeTerritories(values) {
    ...
    for (const value of values ?? []) {
        ...
        if (normalized)
            territories.add(normalized);
    }
    if (territories.size === 0)
        territories.add("WORLD");
    return Object.freeze([...territories]);
}
function mapContributor(contributor, index) {
...
function mapTechnicalDetails(source, metadata, previewAvailable) {
    return Object.freeze({
        multiDisc: Boolean(source.multiDisc ?? getMetadataBoolean(metadata, "multiDisc") ?? false),
        versionedRelease: isNonEmptyText(source.versionTitle ?? null) || isNonEmptyText(source.version ?? null),
        bundle: Boolean(getMetadataBoolean(metadata, "bundle") ?? false),
        previewAvailable,
        explicit: Boolean(source.explicit ?? source.explicit ?? false),
...
function mapSoundRecording(release, track, index) {
...
function mapImageResource(release) {
...
function mapTextResource(release, track) {
    if (!track.lyrics)
        return null;
    return Object.freeze({
...
function mapVideoResource(release) {
    const releaseMetadata = extractMetadataRecord(release.metadata);
...
function mapTrackContributors(release, track) {
    ...
    for (const [index, contributor] of trackContributors.entries()) {
        ...
        if (!contributors.has(key))
            contributors.set(key, mapped);
    }
    return [...contributors.values()];
}
function mapReleaseContributors(release, tracks) {
    const contributors = new Map();
...
function mapPartyList(contributors) {
    return contributors.map((contributor) => mapParty(contributor));
}
function mapDeal(release) {
    const releaseMetadata = extractMetadataRecord(release.metadata);
    ...
    const priceInformation = release.pricing
        ? release.pricing.territories.map((territory) => Object.freeze({
...
function mapRelease(release, resources, deals) {
    const releaseMetadata = extractMetadataRecord(release.metadata);
    const identifiers = [
        Object.freeze({ type: "upc", value: release.identifiers.find((identifier) => identifier.type === "upc")?.value ?? release.id, scope: "release", issuer: null }),
        ...release.identifiers.map((identifier) => Object.freeze({
    ...
    return Object.freeze({
        ...
        territories: normalizeTerritories(release.territories.map((territory) => territory.code)),
        technicalDetails: mapTechnicalDetails(release, releaseMetadata, resources.some((resource) => resource.resourceType === "SoundRecording" && Boolean(resource.previewUrl))),
...
function createHeader(messageType, options, schemaVersion) {
...
export class DdexErnMapper {
    ...
    buildResourceList(release) {
        const resources = release.tracks.map((track, index) => mapSoundRecording(release, track, index));
...
export class DdexRinMapper {
    map(release, options = {}) {
        ...
        return Object.freeze({
            ...
            identifiers: Object.freeze(release.identifiers.map((identifier) => Object.freeze({
                type: identifier.type,
                value: identifier.value,
