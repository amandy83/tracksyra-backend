...
const DEFAULT_TARGETS = Object.freeze(["spotify", "apple_music", "youtube_music"]);
const ALLOWED_STATES = new Map([
...
function localMidnight(dateValue, timeZone) {
...
export class ReleaseAutomationEngine {
    ...
    async scheduleRelease(input) {
        ...
        const state = input.freeze ? "locked" : "scheduled";
    ...
    async approveRelease(input) {
        ...
        const release = source.release;
        const track = source.track;
        const nextState = input.approved ? "approved" : "cancelled";
    ...
    async orchestrateDelivery(input) {
        const source = await this.resolveSource(input.releaseId, input.release, input.track, input.trackId ?? null);
        const release = source.release;
        const track = source.track;
    ...
    async rollbackRelease(input) {
        ...
        await this.deps.sql.query(`INSERT INTO public.release_rollback (rollback_id, release_id, track_id, package_id, reason, status, metadata, created_at, updated_at)
    ...
    async processWebhook(input) {
        ...
        await this.deps.sql.query(`INSERT INTO public.delivery_confirmations (confirmation_id, release_id, track_id, source, payload, metadata, created_at)
    ...
    async healthCheck() {
        const rows = await this.deps.sql.query(`SELECT
         ...
         (SELECT COUNT(*)::int FROM public.delivery_sla) AS sla`).catch(() => [{ workflows: 0, health: 0, sla: 0 }]);
    ...
    async generateReleaseCalendarReport(limit = 50) {
        const items = await this.deps.sql.query(`SELECT calendar_id, release_id, track_id, scheduled_for::text AS scheduled_for, timezone, embargo_until::text AS embargo_until, state, created_at::text AS created_at, metadata
       FROM public.release_calendar ORDER BY scheduled_for ASC LIMIT :limit`, { limit: Math.max(1, Math.trunc(limit)) }).catch(() => []);
        return freeze({ generatedAt: nowIso(this.deps.now), name: "release-calendar-report", items, summary: { count: items.length } });
    }
    async generateDeliveryReport(limit = 50) {
        const items = await this.deps.sql.query(`SELECT * FROM public.delivery_batches ORDER BY created_at DESC LIMIT :limit`, { limit: Math.max(1, Math.trunc(limit)) }).catch(() => []);
        return freeze({ generatedAt: nowIso(this.deps.now), name: "delivery-report", items, summary: { count: items.length } });
    }
    async generateDeliveryFailureReport(limit = 50) {
        const items = await this.deps.sql.query(`SELECT * FROM public.delivery_attempts WHERE status IN ('FAILED','REJECTED') ORDER BY created_at DESC LIMIT :limit`, { limit: Math.max(1, Math.trunc(limit)) }).catch(() => []);
        return freeze({ generatedAt: nowIso(this.deps.now), name: "delivery-failure-report", items, summary: { count: items.length } });
    }
    async generateRetryReport(limit = 50) {
        const items = await this.deps.sql.query(`SELECT * FROM public.delivery_attempts WHERE attempt_number > 1 OR next_retry_at IS NOT NULL ORDER BY created_at DESC LIMIT :limit`, { limit: Math.max(1, Math.trunc(limit)) }).catch(() => []);
        return freeze({ generatedAt: nowIso(this.deps.now), name: "retry-report", items, summary: { count: items.length } });
    }
    async generateSlaReport(limit = 50) {
        const items = await this.deps.sql.query(`SELECT * FROM public.delivery_sla ORDER BY created_at DESC LIMIT :limit`, { limit: Math.max(1, Math.trunc(limit)) }).catch(() => []);
        return freeze({ generatedAt: nowIso(this.deps.now), name: "sla-report", items, summary: { count: items.length } });
    }
    async generateReleaseAutomationReport(limit = 50) {
        const items = await this.deps.sql.query(`SELECT * FROM public.release_workflows ORDER BY updated_at DESC LIMIT :limit`, { limit: Math.max(1, Math.trunc(limit)) }).catch(() => []);
        return freeze({ generatedAt: nowIso(this.deps.now), name: "release-automation-report", items, summary: { count: items.length } });
    }
    async generateWorkflowReport(limit = 50) {
        const items = await this.deps.sql.query(`SELECT * FROM public.release_states ORDER BY created_at DESC LIMIT :limit`, { limit: Math.max(1, Math.trunc(limit)) }).catch(() => []);
        return freeze({ generatedAt: nowIso(this.deps.now), name: "workflow-report", items, summary: { count: items.length } });
    }
    async generateDeliveryHealthReport(limit = 50) {
        const items = await this.deps.sql.query(`SELECT * FROM public.delivery_health ORDER BY created_at DESC LIMIT :limit`, { limit: Math.max(1, Math.trunc(limit)) }).catch(() => []);
        return freeze({ generatedAt: nowIso(this.deps.now), name: "delivery-health-report", items, summary: { count: items.length } });
    }
    async generateDashboard(kind = "workflow", limit = 50) {
        switch (kind) {
            case "calendar":
    ...
    async resolveSource(releaseId, release, track, trackId) {
        if (release) {
    ...
    async releaseReadiness(release, track) {
    ...
    async persistWorkflow(input) {
        await this.deps.sql.query(`INSERT INTO public.release_workflows (workflow_id, release_id, track_id, state, priority, scheduled_for, embargo_until, batch_id, metadata, created_at, updated_at)
       VALUES (:workflowId, :releaseId::uuid, CASE WHEN :trackId IS NULL OR :trackId = '' THEN NULL ELSE :trackId::uuid END, :state, :priority, :scheduledFor, :embargoUntil, :batchId, CAST(:metadata AS jsonb), now(), now())`, { workflowId: randomUUID(), ...input, metadata: JSON.stringify(input.metadata) }).catch(() => undefined);
        await this.persistState(input.releaseId, input.trackId, input.state, input.metadata);
        await this.deps.sql.query(`INSERT INTO public.delivery_schedule (schedule_id, release_id, track_id, state, scheduled_for, timezone, embargo_until, priority, metadata, created_at, updated_at)
       VALUES (:scheduleId, :releaseId::uuid, CASE WHEN :trackId IS NULL OR :trackId = '' THEN NULL ELSE :trackId::uuid END, :state, :scheduledFor, NULL, :embargoUntil, :priority, CAST(:metadata AS jsonb), now(), now())`, { scheduleId: randomUUID(), ...input, metadata: JSON.stringify(input.metadata) }).catch(() => undefined);
        await this.deps.sql.query(`INSERT INTO public.release_calendar (calendar_id, release_id, track_id, scheduled_for, timezone, embargo_until, state, metadata, created_at, updated_at)
       VALUES (:calendarId, :releaseId::uuid, CASE WHEN :trackId IS NULL OR :trackId = '' THEN NULL ELSE :trackId::uuid END, :scheduledFor, NULL, :embargoUntil, :state, CAST(:metadata AS jsonb), now(), now())`, { calendarId: randomUUID(), ...input, metadata: JSON.stringify(input.metadata) }).catch(() => undefined);
    }
    async persistVersion(releaseId, trackId, state, metadata) {
        await this.deps.sql.query(`INSERT INTO public.release_versions (version_id, release_id, track_id, state, metadata, created_at, updated_at)
       VALUES (:versionId, :releaseId::uuid, CASE WHEN :trackId IS NULL OR :trackId = '' THEN NULL ELSE :trackId::uuid END, :state, CAST(:metadata AS jsonb), now(), now())`, { versionId: randomUUID(), releaseId, trackId, state, metadata: JSON.stringify(metadata) }).catch(() => undefined);
    }
    async persistDependencies(batchId, releaseId, trackId, dependencyReleaseIds) {
        if (!dependencyReleaseIds.length)
            return;
        for (const dependencyReleaseId of dependencyReleaseIds) {
            await this.deps.sql.query(`INSERT INTO public.delivery_dependencies (dependency_id, batch_id, release_id, track_id, dependency_release_id, dependency_state, metadata, created_at, updated_at)
         VALUES (:dependencyId, :batchId, :releaseId::uuid, CASE WHEN :trackId IS NULL OR :trackId = '' THEN NULL ELSE :trackId::uuid END, :dependencyReleaseId::uuid, 'pending', '{}'::jsonb, now(), now())`, { dependencyId: randomUUID(), batchId, releaseId, trackId, dependencyReleaseId }).catch(() => undefined);
        }
    }
    async persistState(releaseId, trackId, state, metadata) {
        await this.deps.sql.query(`INSERT INTO public.release_states (state_id, release_id, track_id, state, metadata, created_at, updated_at)
       VALUES (:stateId, :releaseId::uuid, CASE WHEN :trackId IS NULL OR :trackId = '' THEN NULL ELSE :trackId::uuid END, :state, CAST(:metadata AS jsonb), now(), now())`, { stateId: randomUUID(), releaseId, trackId, state, metadata: JSON.stringify(metadata) }).catch(() => undefined);
    }
    async persistLock(releaseId, trackId, lockKind, metadata) {
        await this.deps.sql.query(`INSERT INTO public.release_locks (lock_id, release_id, track_id, lock_kind, status, metadata, created_at, updated_at)
       VALUES (:lockId, :releaseId::uuid, CASE WHEN :trackId IS NULL OR :trackId = '' THEN NULL ELSE :trackId::uuid END, :lockKind, 'locked', CAST(:metadata AS jsonb), now(), now())`, { lockId: randomUUID(), releaseId, trackId, lockKind, metadata: JSON.stringify(metadata) }).catch(() => undefined);
    }
    async persistBatch(batchId, releaseId, trackId, targets, readiness, metadata) {
        await this.deps.sql.query(`INSERT INTO public.delivery_batches (batch_id, release_id, track_id, target_count, priority, readiness_score, status, metadata, created_at, updated_at)
    ...
    async persistEvent(releaseId, trackId, eventType, status, metadata) {
        await this.deps.sql.query(`INSERT INTO public.delivery_events (event_id, release_id, track_id, event_type, status, metadata, created_at)
       VALUES (:eventId, :releaseId::uuid, CASE WHEN :trackId IS NULL OR :trackId = '' THEN NULL ELSE :trackId::uuid END, :eventType, :status, CAST(:metadata AS jsonb), now())`, { eventId: randomUUID(), releaseId, trackId, eventType, status, metadata: JSON.stringify(metadata) }).catch(() => undefined);
    }
    async persistHealth(releaseId, trackId, state, status, metadata) {
        await this.deps.sql.query(`INSERT INTO public.delivery_health (health_id, release_id, track_id, state, status, score, metadata, created_at, updated_at)
       VALUES (:healthId, :releaseId::uuid, CASE WHEN :trackId IS NULL OR :trackId = '' THEN NULL ELSE :trackId::uuid END, :state, :status, 100, CAST(:metadata AS jsonb), now(), now())`, { healthId: randomUUID(), releaseId, trackId, state, status, metadata: JSON.stringify(metadata) }).catch(() => undefined);
    }
    async persistSla(releaseId, trackId, slaKind, score, metadata) {
        await this.deps.sql.query(`INSERT INTO public.delivery_sla (sla_id, release_id, track_id, sla_kind, score, status, metadata, created_at, updated_at)
       VALUES (:slaId, :releaseId::uuid, CASE WHEN :trackId IS NULL OR :trackId = '' THEN NULL ELSE :trackId::uuid END, :slaKind, :score, 'met', CAST(:metadata AS jsonb), now(), now())`, { slaId: randomUUID(), releaseId, trackId, slaKind, score, metadata: JSON.stringify(metadata) }).catch(() => undefined);
    }
    async persistAttempt(batchId, releaseId, trackId, target, attempt, status, error, metadata) {
        await this.deps.sql.query(`INSERT INTO public.delivery_attempts (attempt_id, batch_id, release_id, track_id, target, attempt_number, status, error, metadata, created_at)
    ...
    async persistRetry(releaseId, trackId, attempt, nextRetryAt, error, metadata) {
        await this.deps.sql.query(`INSERT INTO public.delivery_attempts (attempt_id, batch_id, release_id, track_id, target, attempt_number, status, next_retry_at, last_error, metadata, created_at)
    ...
    async recordAudit(releaseId, action, status, metadata, trackId) {
        await this.deps.sql.query(`INSERT INTO public.delivery_audit (audit_id, release_id, track_id, action, status, actor, metadata, created_at)
