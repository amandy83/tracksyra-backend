...
export class EnterpriseOperationsService {
    ...
    constructor(sql, distributionStore, enterpriseDistributionService) {
        this.sql = sql;
        this.distributionStore = distributionStore;
        this.enterpriseDistributionService = enterpriseDistributionService;
    }
    async recordCatalogSnapshot(releaseId, audit) {
        ...
        await this.sql.query(`INSERT INTO public.catalog_health (
         release_id,
         overall_score,
         metadata_score,
         artwork_score,
    ...
    async recordRightsSnapshot(releaseId, audit) {
        const bundle = await this.distributionStore.getReleaseWithTracks(releaseId);
        if (!bundle)
            return;
        ...
        for (const ownership of [releaseOwnership, ...trackOwnership]) {
            await this.sql.query(`INSERT INTO public.rights_ownership (
           release_id,
           track_id,
           owner_type,
           owner_name,
         ...
         ON CONFLICT DO NOTHING`, ownership);
            await this.sql.query(`INSERT INTO public.ownership_history (
           ownership_id,
           release_id,
           track_id,
           action,
    ...
    async recordIdentifierHistory(releaseId, audit) {
        const report = await this.enterpriseDistributionService.getIdentifierReportByRelease(releaseId);
        if (!report)
            return;
        await this.sql.query(`INSERT INTO public.release_identifier_history (
         release_id,
         identifier_type,
         identifier_value,
         generated,
    ...
    async queueDelivery(releaseId, audit) {
        const bundle = await this.distributionStore.getReleaseWithTracks(releaseId);
        if (!bundle)
            return;
        await this.sql.query(`INSERT INTO public.delivery_queue (
         release_id,
         track_id,
         provider,
         platform,
    ...
    async queueSpotifyDelivery(releaseId, audit) {
        const bundle = await this.distributionStore.getReleaseWithTracks(releaseId);
        if (!bundle)
            return;
        await this.sql.query(`INSERT INTO public.delivery_queue (
    ...
    async queueFingerprintReview(input) {
        await this.sql.query(`INSERT INTO public.review_queue (
         release_id,
         artist_id,
         queue_status,
         priority,
    ...
    async queueFingerprintFraud(input) {
        await this.sql.query(`SELECT public.append_fraud_signal(
         :eventId,
         'AUDIO_FINGERPRINT',
         'AUDIO_DUPLICATION',
         :score,
    ...
    async processDeliveryQueue(limit = 25) {
        const rows = await this.sql.query(`SELECT id, release_id, track_id, platform, status, attempts, max_attempts, next_retry_at
       ...
       LIMIT :limit`, { limit: Math.max(1, Math.trunc(limit)) });
        for (const row of rows) {
            ...
            if (!report || !report.ownershipVerified || report.stage === "Rejected") {
                ...
                await this.sql.query(`INSERT INTO public.delivery_failures (
             delivery_queue_id,
             release_id,
             track_id,
             platform,
            ...
            await this.sql.query(`INSERT INTO public.delivery_attempts (
           delivery_queue_id,
           attempt_number,
           status,
           request,
    ...
    async scoreFraud(releaseId, audit) {
        const report = await this.enterpriseDistributionService.getCatalogReport(releaseId);
        if (!report)
            return;
        ...
        await this.sql.query(`INSERT INTO public.fraud_scores (
         release_id,
         fraud_type,
         score,
         reasons,
    ...
    async recordAuditEvent(input) {
        await this.sql.query(`INSERT INTO public.audit_events (
         aggregate_type,
         aggregate_id,
         action,
         actor,
    ...
    buildReleaseOwnership(release, audit) {
        return {
    ...
    buildTrackOwnership(release, track, audit) {
        return {
            releaseId: release.id,
            trackId: track.id,
            ownerType: "artist",
    ...
    async markDelivery(deliveryQueueId, status, reason) {
        await this.sql.query(`UPDATE public.delivery_queue
       SET status = :status,
           updated_at = now(),
       ...
       WHERE id = :deliveryQueueId`, { deliveryQueueId, status, reason });
    }
}
