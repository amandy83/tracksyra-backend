...
import { DistributionStatus, mapProviderStatus, } from "../intelligence/index.js";
import { captureException } from "../../observability/errorTracker.js";
import { loadRuntimeEnv } from "../../config/envLoader.js";
import { checkRateLimit, getClientIp, rateLimitRules, suspiciousActivityLog } from "../../security/rateLimiter.js";
export class TooLostWebhookController {
    ...
    normalize(payload) {
        const type = String(payload.type ?? payload.eventType ?? payload.event ?? "").toUpperCase();
        ...
        if (!providerReleaseId)
            throw new Error("Webhook payload missing releaseId");
        return {
    ...
    async resolveEventTargets(event) {
    ...
    async findLocalReleaseTarget(providerReleaseId, providerTrackId) {
        const matches = await this.deps.db.query(`SELECT dj.release_id::text AS release_id,
       ...
       LIMIT 1`, { providerReleaseId, providerTrackId });
    ...
    verifySignature(body, headers) {
        ...
        if (!signature)
            throw new Error("Missing Too Lost webhook signature");
    ...
    mapEventTypeToStatus(type, payload) {
        if (typeof payload.status === "string" && payload.status.trim())
            return mapProviderStatus(payload.status);
        switch (type) {
    ...
    async persistRawWebhook(event) {
        await this.deps.db.query(`INSERT INTO distribution_events (
    ...
    async updateMatchingJobs(event) {
        await this.deps.db.query(`UPDATE distribution_jobs
       ...
       WHERE release_id = :releaseId
         ...
         AND (:trackId IS NULL OR track_id = :trackId)`, { status: event.status, releaseId: event.releaseId, trackId: event.trackId ?? null });
    }
    async updateReleaseLifecycle(event) {
    ...
    toReleaseStatus(status) {
        ...
        if (status === DistributionStatus.APPROVED)
            return "approved";
        if (status === DistributionStatus.IN_REVIEW)
            return "in_review";
        if (status === DistributionStatus.SUBMITTED || status === DistributionStatus.PROCESSING)
            return "processing";
        if (status === DistributionStatus.REJECTED || status === DistributionStatus.FAILED || status === DistributionStatus.DEAD_LETTER)
            return "rejected";
        return null;
    }
    async persistReleaseLifecycleStatus(releaseId, desiredStatus) {
        ...
        for (const status of candidates) {
            try {
                await this.deps.db.query(`UPDATE releases
