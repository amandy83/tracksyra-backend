import { DomainInvariantError, DistributionJobId, DistributionStateMachine, ProviderReceipt, Release, ReleaseAggregate, RoyaltyBatchId, SnapshotId, ProviderSubmissionAggregate, RoyaltyBatchAggregate, DistributionJobAggregate, PaymentRecord, } from "../domain/index.js";
import { nowIso } from "../domain/index.js";
import { createSubmissionLock, mapProviderStatusToState, publishMany, requireReleaseState } from "./helpers.js";
async function resolve(value) {
    return await Promise.resolve(value);
}
async function loadRelease(deps, releaseId) {
...
async function loadProviderSubmission(deps, releaseId) {
...
async function loadRoyaltyBatch(deps, releaseId) {
...
async function saveRelease(deps, aggregate) {
    await resolve(deps.releases.save(aggregate));
}
async function saveProviderSubmission(deps, aggregate) {
    await resolve(deps.providerSubmissions.save(aggregate));
}
async function saveRoyaltyBatch(deps, aggregate) {
    await resolve(deps.royaltyBatches.save(aggregate));
}
function releaseEvent(aggregate, type, payload) {
    return {
        type,
...
function jobEvent(aggregate, type, payload) {
    return {
        type,
...
function providerEvent(aggregate, type, payload) {
    return {
        type,
...
function royaltyEvent(aggregate, type, payload) {
    return {
        type,
        aggregateId: aggregate.id.value,
...
function cloneRelease(release, patch = {}) {
    return new Release({
        id: release.id,
        title: release.title,
...
export class ProcessProviderStatus {
    ...
    async execute(command) {
        ...
        if (events.length > 0)
            await publishMany(this.deps.events, events);
        return status;
    }
