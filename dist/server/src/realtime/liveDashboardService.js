export class LiveDashboardService {
    ...
    async getSnapshot(artistId) {
        const [streams, revenue, fraud, distribution, payouts, rolling] = await Promise.all([
            this.deps.db.query(`SELECT ss.track_id::text, SUM(ss.streams_count)::int AS streams_count
         FROM streaming_stats ss
         JOIN tracks t ON t.id = ss.track_id
         WHERE t.user_id = :artistId
         GROUP BY ss.track_id`, { artistId }),
            this.deps.db.query(`SELECT track_id::text, SUM(total_revenue)::text AS total_revenue
         FROM royalty_records
         WHERE artist_id = :artistId
         GROUP BY track_id`, { artistId }),
            this.deps.db.query(`SELECT event_id, track_id, platform, decision, fraud_score, reasons, created_at
         FROM fraud_events
         WHERE user_id = :artistId AND decision IN ('SUSPICIOUS', 'BLOCKED')
         ORDER BY created_at DESC
         LIMIT 25`, { artistId }),
            this.deps.db.query(`SELECT pd.release_id::text, pd.track_id::text, pd.platform::text, pd.status::text
         FROM platform_deliveries pd
         JOIN releases r ON r.id = pd.release_id
         WHERE r.user_id = :artistId`, { artistId }),
            this.deps.db.query(`SELECT id, amount, currency, state, created_at, updated_at
         FROM payout_requests
         WHERE user_id = :artistId
         ORDER BY created_at DESC
         LIMIT 25`, { artistId }),
            this.deps.db.query(`SELECT rolling_metrics
        ...
        return {
            ...
            rolling_metrics: rolling[0]?.rolling_metrics ?? {},
            updated_at: new Date().toISOString(),
        };
    }
    async publishSnapshot(artistId) {
