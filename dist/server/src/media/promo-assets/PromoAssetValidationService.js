...
import { PROMO_ASSET_RULES } from "./promoAssetRules.js";
...
export class PromoAssetValidationService {
    ...
    async validate(input) {
        ...
        if (!ALLOWED_EXTENSIONS.has(ext))
            errors.push("Promo video must be MP4 or MOV.");
        if (!ALLOWED_MIME_TYPES.has(input.mimeType))
            errors.push("Promo video MIME type must be video/mp4 or video/quicktime.");
        if (input.sizeBytes > rule.maxBytes)
            errors.push("Promo video exceeds the 100 MB limit.");
        if (input.sizeBytes <= 0)
            errors.push("Promo video is empty.");
        if (errors.length)
            return { ok: false, status: "failed", errors, warnings };
        try {
            const probe = await this.ffmpeg.probe(input.path);
            const video = probe.streams?.find((stream) => stream.codec_type === "video");
            const audio = probe.streams?.find((stream) => stream.codec_type === "audio");
            if (!video)
                return { ok: false, status: "failed", errors: ["Video stream is missing or corrupted."], warnings };
            ...
            if (!ALLOWED_VIDEO_CODECS.has(videoCodec))
                errors.push(`Unsupported video codec "${videoCodec || "unknown"}". Use H.264 or HEVC.`);
            ...
            if (width <= 0 || height <= 0)
                errors.push("Video resolution could not be read.");
            ...
            if (fps !== null && (fps < 23 || fps > 60))
                warnings.push(`Frame rate ${fps.toFixed(2)} fps is outside the typical 23-60 fps range.`);
            ...
            if (blackScreen)
                errors.push("Black screen detected across a significant portion of the promo video.");
            ...
            if (frozenFrame)
                errors.push("Frozen frame sequence detected in promo video.");
            const metadata = {
    ...
    async detectBlackScreen(path, durationSeconds) {
...
function matchesAspectRatio(width, height, expected) {
