...
export class MediaStorageValidationHooks {
    storage;
    mediaValidation;
    log = logger.child({ component: "media-storage-validation" });
    constructor(storage, mediaValidation) {
        this.storage = storage;
        this.mediaValidation = mediaValidation;
    }
    async validateUploadedAudio(input) {
    ...
    async validateUploadedArtwork(input) {
...
function validateArtworkBuffer(bytes, mimeType, sizeBytes) {
    const errors = [];
    if (mimeType !== "image/jpeg" && mimeType !== "image/png")
        errors.push("Artwork must be JPEG or PNG.");
    if (sizeBytes > 10 * 1024 * 1024)
        errors.push("Artwork file size must be 10 MB or less.");
    const header = mimeType === "image/png" ? readPngHeader(bytes) : readJpegHeader(bytes);
    if (!header.ok)
        return { ok: false, errors: [...errors, ...header.errors] };
    if (header.width < 3000 || header.height < 3000)
        errors.push("Artwork must be at least 3000x3000.");
    if (header.width > 10000 || header.height > 10000)
        errors.push("Artwork must be at most 10000x10000.");
    if (header.width !== header.height)
        errors.push("Artwork must be square.");
    if (header.hasTransparency)
        errors.push("Artwork must not contain transparency.");
...
function readPngHeader(bytes) {
    ...
    if (bytes.length < 33 || !bytes.subarray(0, 8).equals(signature))
        return { ok: false, errors: ["PNG signature is invalid."] };
    const width = bytes.readUInt32BE(16);
    const height = bytes.readUInt32BE(20);
    const colorType = bytes[25];
    if (![2, 6].includes(colorType))
        return { ok: false, errors: ["Artwork must use RGB color."] };
    let offset = 8;
...
function readJpegHeader(bytes) {
    if (bytes.length < 4 || bytes[0] !== 0xff || bytes[1] !== 0xd8)
        return { ok: false, errors: ["JPEG signature is invalid."] };
    let offset = 2;
    while (offset + 9 < bytes.length) {
        ...
        if (length < 2)
            return { ok: false, errors: ["JPEG header is corrupted."] };
        if ((marker >= 0xc0 && marker <= 0xc3) || (marker >= 0xc5 && marker <= 0xc7) || (marker >= 0xc9 && marker <= 0xcb) || (marker >= 0xcd && marker <= 0xcf)) {
            const height = bytes.readUInt16BE(offset + 5);
            const width = bytes.readUInt16BE(offset + 7);
            const components = bytes[offset + 9];
            if (components !== 3)
                return { ok: false, errors: ["Artwork must use RGB color profile."] };
