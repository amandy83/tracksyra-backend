...
export class MediaProcessingEngine {
    ...
    artwork;
    fingerprint;
    log = logger.child({ component: "media-processing" });
    constructor(deps) {
        this.deps = deps;
    ...
    async processAudio(input) {
        ...
        try {
            ...
            if (fingerprint.duplicateAssetId) {
                ...
                return { status: "duplicate", validation, fingerprint };
            }
            ...
            return { status: "ready", validation, fingerprint };
        }
        catch (error) {
    ...
    async processArtwork(input) {
        ...
        try {
            ...
            if (moderation.nsfw) {
                ...
                return { status: "rejected", moderation };
            }
            ...
            return { status: "ready", validation: result.validation };
        }
        catch (error) {
            await this.markAsset(input.assetId, "failed").catch(() => undefined);
    ...
    async analyzeFingerprint(input) {
    ...
    async persistVariant(assetId, variantType, object, metadata = {}) {
        if (!this.deps.db)
            return;
    ...
    async persistWaveform(input, waveform) {
        if (!this.deps.db)
            return;
    ...
    async persistFingerprint(input, fingerprint) {
        if (!this.deps.db)
            return;
