...
export class MediaValidationService {
    ffmpeg;
    log = logger.child({ component: "media-validation" });
    constructor(ffmpeg) {
        this.ffmpeg = ffmpeg;
    }
    validateUploadEnvelope(input) {
        ...
        if (REJECTED_AUDIO_EXTENSIONS.has(ext))
            issues.push(error("rejected_format", "Audio masters must be WAV or FLAC. MP3, AAC, M4A, OGG, WMA, and AIFF are rejected."));
        else if (!ALLOWED_AUDIO_EXTENSIONS.has(ext))
            issues.push(error("unsupported_extension", "Audio masters must be WAV or FLAC."));
        if (!ALLOWED_AUDIO_MIME_TYPES.has(input.mimeType.toLowerCase()))
            issues.push(error("unsupported_mime", "Audio MIME type is not allowed."));
        if (input.sizeBytes > MAX_AUDIO_BYTES)
            issues.push(error("oversized_file", "Audio file exceeds the 500 MB limit."));
        if (input.sizeBytes <= MIN_AUDIO_BYTES)
            issues.push(error("file_too_small", "Audio file must be larger than 1 MB."));
        ...
        if (/\.(exe|dll|bat|cmd|sh|msi|scr|ps1)$/i.test(input.filename))
            issues.push(error("executable_upload", "Executable uploads are blocked."));
    ...
    async validateAudioFile(path, envelope) {
        ...
        try {
            ...
            if (durationSec < MIN_DURATION_SEC)
                issues.push(error("duration_too_short", `Audio duration must be at least ${MIN_DURATION_SEC} seconds.`));
            if (durationSec > MAX_DURATION_SEC)
                issues.push(error("duration_too_long", "Audio duration exceeds the 3 hour limit."));
            if (sampleRateHz < MIN_SAMPLE_RATE_HZ)
                issues.push(error("sample_rate_too_low", "Audio sample rate must be at least 44.1 kHz."));
            if (channels !== 2)
                issues.push(error("not_stereo", "Audio must be stereo."));
            if (bitDepth !== null && bitDepth < MIN_BIT_DEPTH)
                issues.push(error("bit_depth_too_low", "Audio bit depth must be at least 16-bit."));
            if (bitDepth === null && codec === "wav")
                issues.push(error("bit_depth_unknown", "WAV bit depth could not be verified."));
            if (hasClipping)
                issues.push(error("clipping_detected", "Audio contains clipping and must be fixed before distribution."));
            if (silenceRatio > MAX_SILENCE_RATIO)
                issues.push(error("excessive_silence", "Audio contains too much silence for distribution."));
            if (bitrateKbps !== null && codec === "mp3" && bitrateKbps < 120)
                issues.push(error("bitrate_too_low", "MP3 bitrate is too low for distribution."));
            const metadata = {
                bitrateKbps,
    ...
    async validateFromContainer(path, envelope) {
        ...
        const issues = [];
        if (metadata.durationSec < MIN_DURATION_SEC)
            issues.push(error("duration_too_short", `Audio duration must be at least ${MIN_DURATION_SEC} seconds.`));
        if (metadata.durationSec > MAX_DURATION_SEC)
            issues.push(error("duration_too_long", "Audio duration exceeds the 3 hour limit."));
        if (metadata.sampleRateHz < MIN_SAMPLE_RATE_HZ)
            issues.push(error("sample_rate_too_low", "Audio sample rate must be at least 44.1 kHz."));
        if (metadata.channels !== 2)
            issues.push(error("not_stereo", "Audio must be stereo."));
        if ((metadata.bitDepth ?? 0) < MIN_BIT_DEPTH)
            issues.push(error("bit_depth_too_low", "Audio bit depth must be at least 16-bit."));
        const ok = !issues.some((issue) => issue.severity === "error");
    ...
    async estimateSilenceRatio(path) {
...
function normalizeCodec(value) {
    ...
    if (codec.includes("flac"))
        return "flac";
    if (codec.includes("aiff") || codec.includes("aif"))
        return "aiff";
...
function readWavHeader(bytes) {
    if (bytes.length < 44)
        return { ok: false, errors: ["WAV header is truncated or corrupted."] };
    ...
    if (dataOffset < 0 || dataSize <= 0)
        return { ok: false, errors: ["WAV data chunk is missing."] };
...
function isMissingProbeBinary(error) {
