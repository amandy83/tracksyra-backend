import { splitFeaturedArtists } from "./musicMetadataMappings.js";
import { validateMusicRelease } from "./musicReleaseValidation.js";
export function mapLegacySongToMusicRelease(song) {
...
export function mapReleaseAndTracksToMusicRelease(input) {
...
export function mapMusicReleaseToDistribution(input) {
    ...
    if (!audio)
        throw new Error(`MusicRelease ${input.release.id} has no audio file for distribution`);
...
export function mapMusicReleaseToRoyaltyInput(release, trackId) {
    return {
        track_id: trackId,
...
function mapTrackToAudioFile(track) {
    return {
        trackId: track.id,
        title: track.title,
...
function assertValidMusicRelease(release) {
    const validation = validateMusicRelease(release);
    if (validation.ok === false)
        throw new Error(`Invalid MusicRelease: ${validation.errors.join("; ")}`);
    return validation.release;
}
function normalizeType(type) {
    if (type === "ep" || type === "album" || type === "compilation")
        return type;
    return "single";
}
