...
export function registerMetaDeliveryWorker(worker, options = {}) {
    return createWorker(queueNames.metaDelivery, async (job) => {
        await worker.process(job.data);
    }, { concurrency: options.concurrency });
}
export function registerMetaPollingWorker(worker, options = {}) {
    return createWorker(queueNames.metaPolling, async (job) => {
        await worker.process(job.data);
    }, { concurrency: options.concurrency });
}
export function registerMetaRetryWorker(worker, options = {}) {
    return createWorker(queueNames.metaRetry, async (job) => {
        await worker.process(job.data);
    }, { concurrency: options.concurrency });
}
export function registerMetaWebhookWorker(worker, options = {}) {
    return createWorker(queueNames.metaWebhook, async (job) => {
        await worker.process(job.data);
    }, { concurrency: options.concurrency });
}
export function registerMetaHealthWorker(worker, options = {}) {
