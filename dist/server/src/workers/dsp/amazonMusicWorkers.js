...
export function registerAmazonMusicDeliveryWorker(worker, options = {}) {
    return createWorker(queueNames.amazonMusicDelivery, async (job) => {
        await worker.process(job.data);
    }, { concurrency: options.concurrency });
}
export function registerAmazonMusicPollingWorker(worker, options = {}) {
    return createWorker(queueNames.amazonMusicPolling, async (job) => {
        await worker.process(job.data);
    }, { concurrency: options.concurrency });
}
export function registerAmazonMusicRetryWorker(worker, options = {}) {
    return createWorker(queueNames.amazonMusicRetry, async (job) => {
        await worker.process(job.data);
    }, { concurrency: options.concurrency });
}
export function registerAmazonMusicWebhookWorker(worker, options = {}) {
    return createWorker(queueNames.amazonMusicWebhook, async (job) => {
        await worker.process(job.data);
    }, { concurrency: options.concurrency });
}
export function registerAmazonMusicHealthWorker(worker, options = {}) {
