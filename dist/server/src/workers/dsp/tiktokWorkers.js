...
export function registerTikTokDeliveryWorker(worker, options = {}) {
    return createWorker(queueNames.tiktokDelivery, async (job) => {
        await worker.process(job.data);
    }, { concurrency: options.concurrency });
}
export function registerTikTokPollingWorker(worker, options = {}) {
    return createWorker(queueNames.tiktokPolling, async (job) => {
        await worker.process(job.data);
    }, { concurrency: options.concurrency });
}
export function registerTikTokRetryWorker(worker, options = {}) {
    return createWorker(queueNames.tiktokRetry, async (job) => {
        await worker.process(job.data);
    }, { concurrency: options.concurrency });
}
export function registerTikTokWebhookWorker(worker, options = {}) {
    return createWorker(queueNames.tiktokWebhook, async (job) => {
        await worker.process(job.data);
    }, { concurrency: options.concurrency });
}
export function registerTikTokHealthWorker(worker, options = {}) {
