...
export function registerAnghamiDeliveryWorker(worker, options = {}) {
    return createWorker(queueNames.anghamiDelivery, async (job) => {
        await worker.process(job.data);
    }, { concurrency: options.concurrency });
}
export function registerAnghamiPollingWorker(worker, options = {}) {
    return createWorker(queueNames.anghamiPolling, async (job) => {
        await worker.process(job.data);
    }, { concurrency: options.concurrency });
}
export function registerAnghamiRetryWorker(worker, options = {}) {
    return createWorker(queueNames.anghamiRetry, async (job) => {
        await worker.process(job.data);
    }, { concurrency: options.concurrency });
}
export function registerAnghamiWebhookWorker(worker, options = {}) {
    return createWorker(queueNames.anghamiWebhook, async (job) => {
        await worker.process(job.data);
    }, { concurrency: options.concurrency });
}
export function registerAnghamiHealthWorker(worker, options = {}) {
