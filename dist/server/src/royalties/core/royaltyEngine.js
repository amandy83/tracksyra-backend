import { decimalUsdToMicros, microsToDecimalUsd } from "./money.js";
import { getRoyaltyRate } from "./rateCard.js";
export class RoyaltyEngine {
    deps;
    constructor(deps) {
        this.deps = deps;
    }
    async calculateTrackRevenue(input) {
        ...
        if (!context)
            throw new Error(`Track not found: ${normalized.trackId}`);
        ...
        for (const streamCount of streamCounts) {
            const platform = String(streamCount.platform);
