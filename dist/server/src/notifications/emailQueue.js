...
import { incrementMetric, setMetric } from "../queue/metrics.js";
...
export async function validateEmailQueueSchema() {
    const requiredTables = ["email_queue", "email_logs", "email_delivery_logs"];
...
export async function refreshEmailQueueDiagnostics() {
    const statuses = ["PENDING", "RETRYING", "PROCESSING", "FAILED", "SENT"];
    const results = await Promise.all(statuses.map(async (status) => {
        ...
        if (error)
            throw new Error(`Failed to read email queue ${status} metrics: ${error.message}`);
...
async function dequeuePendingEmailRows(limit) {
    ...
    const { data, error } = await getSupabaseClient()
        ...
        .limit(limit);
    if (error)
        throw new Error(`Failed to dispatch pending emails: ${error.message}`);
    return (data || []);
}
async function readPendingQueueDepth() {
    const { count, error } = await getSupabaseClient()
        .from("email_queue")
        .select("id", { count: "exact", head: true })
        .in("status", ["PENDING", "RETRYING"])
    ...
    if (error)
        throw new Error(`Failed to read email queue depth: ${error.message}`);
...
function createDeduplicationKey(to, templateType, payload) {
    const entityId = stringValue(payload.artist_request_id) ||
...
function normalizeTemplateType(value) {
    ...
    if (normalized === "artist_request_pending" || normalized === "artist_pending")
        return "ARTIST_PENDING";
    if (normalized === "artist_request_approved" || normalized === "artist_approved")
        return "ARTIST_APPROVED";
