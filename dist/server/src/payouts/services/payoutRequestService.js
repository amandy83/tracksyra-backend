...
export class PayoutRequestService {
    ...
    async findByEventId(event_id) {
        ...
        const rows = await this.deps.sequelize.query(`SELECT * FROM payout_requests WHERE event_id = :event_id LIMIT 1`, { replacements: { event_id }, type: QueryTypes.SELECT });
        const row = rows[0];
        if (!row)
            return null;
        return this.mapRowToRecord(row);
    }
    async getById(payout_id) {
        const rows = await this.deps.sequelize.query(`SELECT * FROM payout_requests WHERE id = :id LIMIT 1`, { replacements: { id: payout_id }, type: QueryTypes.SELECT });
        const row = rows[0];
        if (!row)
    ...
    async createRequested(params) {
        ...
        await this.deps.sequelize.query(`INSERT INTO payout_requests (
