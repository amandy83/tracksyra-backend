export class PayoutEngine {
    ...
    async processPayoutRequest(payoutRequestId) {
        ...
        if (request.state === "COMPLETED" || request.state === "REJECTED")
            return request;
    ...
    async getPayoutRequest(id) {
        const rows = await this.deps.db.query(`SELECT id, user_id, amount, currency, state, idempotency_key,
    ...
    async reject(id, riskDecisionId, governanceApprovalId, reason) {
        await this.deps.db.query(`UPDATE payout_requests
       SET state = 'REJECTED',
           risk_decision_id = :riskDecisionId,
           governance_approval_id = :governanceApprovalId,
           failure_reason = :reason,
           updated_at = now()
       WHERE id = :id AND state IN ('PENDING', 'PROCESSING')`, { id, riskDecisionId, governanceApprovalId, reason });
        return this.getPayoutRequest(id);
    }
    async markProcessingAndLockFunds(id, riskDecisionId, governanceApprovalId) {
        const request = await this.getPayoutRequest(id);
        const lockKey = `payout-lock:${id}`;
        const alreadyLocked = await this.deps.db.query(`SELECT id FROM payout_transactions WHERE idempotency_key = :lockKey LIMIT 1`, { lockKey });
        await this.deps.db.query(`UPDATE payout_requests
       SET state = 'PROCESSING',
           risk_decision_id = :riskDecisionId,
           governance_approval_id = :governanceApprovalId,
           updated_at = now()
       WHERE id = :id AND state = 'PENDING'`, { id, riskDecisionId, governanceApprovalId });
        if (alreadyLocked[0])
            return;
        const updatedWallet = await this.deps.db.query(`UPDATE payout_wallets
       ...
       FROM payout_requests
       WHERE id = :id
       ON CONFLICT (idempotency_key) DO NOTHING`, { id, lockKey });
    }
    async releaseLockedFunds(id, reason) {
        const request = await this.getPayoutRequest(id);
        const releaseKey = `payout-release:${id}`;
        const released = await this.deps.db.query(`SELECT id FROM payout_transactions WHERE idempotency_key = :releaseKey LIMIT 1`, { releaseKey });
        if (!released[0]) {
            await this.deps.db.query(`UPDATE payout_wallets
         SET available_balance = available_balance + CAST(:amount AS numeric),
             locked_balance = GREATEST(locked_balance - CAST(:amount AS numeric), 0),
             updated_at = now()
         WHERE id = (SELECT wallet_id FROM payout_requests WHERE id = :id)`, { id, amount: request.amount });
            await this.deps.db.query(`INSERT INTO payout_transactions (
           wallet_id, user_id, payout_request_id, transaction_type, amount,
           currency, idempotency_key, metadata
         )
         ...
         FROM payout_requests
         WHERE id = :id
         ON CONFLICT (idempotency_key) DO NOTHING`, { id, releaseKey, metadata: JSON.stringify({ reason }) });
        }
        await this.deps.db.query(`UPDATE payout_requests
       SET state = 'FAILED', failure_reason = :reason, updated_at = now()
       WHERE id = :id AND state = 'PROCESSING'`, { id, reason });
    }
    async completePayout(id, providerTransactionId) {
        const request = await this.getPayoutRequest(id);
        const debitKey = `payout-debit:${id}`;
        const debited = await this.deps.db.query(`SELECT id FROM payout_transactions WHERE idempotency_key = :debitKey LIMIT 1`, { debitKey });
        if (!debited[0]) {
            await this.deps.db.query(`UPDATE payout_wallets
         ...
         FROM payout_requests
         WHERE id = :id
         ON CONFLICT (idempotency_key) DO NOTHING`, { id, debitKey, metadata: JSON.stringify({ provider_transaction_id: providerTransactionId }) });
        }
        await this.deps.db.query(`UPDATE payout_requests
       SET state = 'COMPLETED', updated_at = now()
       WHERE id = :id AND state = 'PROCESSING'`, { id });
    }
}
