import { getRequiredServerEnvNames, loadRuntimeEnv } from "./envLoader.js";
export function validateProductionEnvironment() {
    ...
    for (const name of getRequiredServerEnvNames())
        requireAlways(name, errors);
    ...
    if (production && !hasResend && !hasSmtp)
        errors.push("RESEND_API_KEY or SMTP_* configuration is required in production.");
    if (!production && !hasResend && !hasSmtp)
        warnings.push("Email provider is not configured; local fallback may be used.");
    ...
    if (!env.TOO_LOST_CLIENT_ID)
        warnings.push("TOO_LOST_CLIENT_ID is empty pending Too Lost app approval.");
    if (!env.TOO_LOST_CLIENT_SECRET)
        warnings.push("TOO_LOST_CLIENT_SECRET is empty pending Too Lost app approval.");
    if (tooLostWebhooksEnabled && !env.TOO_LOST_WEBHOOK_SECRET)
        warnings.push("TOO_LOST_WEBHOOK_SECRET is empty pending Too Lost webhook approval.");
    if (!env.TOO_LOST_TOKEN_ENCRYPTION_KEY)
        warnings.push("TOO_LOST_TOKEN_ENCRYPTION_KEY is empty; token storage cannot be encrypted.");
...
function requireWhen(condition, name, errors) {
    if (condition && !getEnv()[name])
        errors.push(`${name} is required in production.`);
}
function requireAlways(name, errors) {
    if (!getEnv()[name])
...
function hasRedisConnectionConfig(env) {
    if (env.REDIS_URL)
        return true;
