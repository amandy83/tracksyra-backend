...
import { getRedisHealthSnapshot, refreshRedisHealthSnapshot } from "../queue/redis";
import { getClientIp, checkRateLimit, rateLimitRules, suspiciousActivityLog } from "../security/rateLimiter";
import { serializeError, type Logger } from "../observability/logger";
...
import type { OperationsServerDependencies } from "../distribution/composition/compositionRoot";
import type { SupabaseClient } from "@supabase/supabase-js";

const queueSet = new Set(Object.values(queueNames).filter((value) => typeof value === "string") as string[]);
type SpotifyTokenCacheRef = { current: { token: string; expiresAt: number } | null };
type HttpErrorLike = { status?: number; code?: string };
type DashboardName = "overview" | "rights-review" | "fraud-review" | "content-review" | "dsp-queue" | "catalog-health" | "delivery-health" | "metadata-errors" | "royalty-health";
type ReportName = "rejected-releases" | "duplicate-releases" | "rights-conflicts" | "delivery-failures" | "dsp-errors" | "royalty-exceptions" | "fraud-reports" | "audit-reports";
type ReviewDecision = "approve" | "reject" | "needs_changes" | "escalate";
type FraudDecision = "APPROVE" | "REJECT" | "ESCALATE";

export async function startOperationsServer(dependencies: OperationsServerDependencies, options: { port?: number } = {}) {
  ...
  app.post("/api/royalties/calculate", asyncHandler(async (req, res) => {
    const body = readJson<RoyaltyCalculationInput>(req);
  ...
  app.post("/api/royalties/statement", asyncHandler(async (req, res) => {
    const body = readJson<RoyaltyStatementRequest>(req);
  ...
  app.post("/api/royalties/statement/approve", asyncHandler(async (req, res) => {
    const body = readJson<{ statementId: string; approverId?: string; metadata?: Record<string, unknown> }>(req);
  ...
  app.post("/api/royalties/payment/release", asyncHandler(async (req, res) => {
    const body = readJson<{ statementId: string; approverId?: string; scheduledFor?: string | null; metadata?: Record<string, unknown> }>(req);
  ...
  app.post("/api/royalties/splits", asyncHandler(async (req, res) => {
    const body = readJson<{ amount: string; currency: string; splits: import("../royalties/accounting/royaltyAccountingTypes").RoyaltySplitInput[]; metadata?: Record<string, unknown> }>(req);
    const result = await royaltyAccountingService.calculateSplits(body);
    res.status(200).json({ ok: true, data: result });
  }));
  app.post("/api/royalties/taxes", asyncHandler(async (req, res) => {
    const body = readJson<RoyaltyTaxInput>(req);
  ...
  app.post("/api/royalties/currency", asyncHandler(async (req, res) => {
    const body = readJson<RoyaltyConversionInput>(req);
  ...
  app.post("/api/royalties/adjustments", asyncHandler(async (req, res) => {
    const body = readJson<RoyaltyAdjustmentInput>(req);
  ...
  app.post("/api/royalties/chargebacks", asyncHandler(async (req, res) => {
    const body = readJson<RoyaltyChargebackInput>(req);
  ...
  app.get("/api/royalties/reports/:reportName", asyncHandler(async (req, res) => {
    ...
    if (reportName === "revenue") return res.status(200).json({ ok: true, data: await royaltyAccountingService.generateRevenueReport(body) });
    if (reportName === "payment") return res.status(200).json({ ok: true, data: await royaltyAccountingService.generatePaymentReport(body) });
    if (reportName === "audit") return res.status(200).json({ ok: true, data: await royaltyAccountingService.generateAuditReport(body) });
  ...
  app.all(/^\/queues\/([^/]+)(?:\/(retry|dlq))?$/, asyncHandler(async (req, res) => {
    ...
    if (!isQueueName(queueName)) return res.status(404).json({ error: "UNKNOWN_QUEUE" });
    if (req.method === "GET" && !action) return res.status(200).json(await inspectQueue(queueName));
  ...
  app.post("/api/artists", asyncHandler(async (req, res) => {
    ...
    if (!name) return res.status(400).json({ error: "ARTIST_NAME_REQUIRED" });
    ...
    if (duplicate.data) return res.status(409).json({ error: "ARTIST_ALREADY_EXISTS" });
  ...
  app.get("/api/admin/dashboards/:dashboardName", asyncHandler(async (req, res) => {
    ...
    if (!dashboardName) return res.status(404).json({ error: "UNKNOWN_DASHBOARD" });
  ...
  app.get("/api/admin/reports/:reportName", asyncHandler(async (req, res) => {
    ...
    const limit = readLimit(req.query);
    if (!reportName) return res.status(404).json({ error: "UNKNOWN_REPORT" });
  ...
  app.get("/api/admin/releases/:releaseId/catalog-report", asyncHandler(async (req, res) => {
    ...
    if (!report) return res.status(404).json({ error: "RELEASE_NOT_FOUND" });
    res.status(200).json({ ok: true, data: report });
  }));
  ...
  app.post("/api/admin/review-queue/:queueId/assign", asyncHandler(async (req, res) => {
    ...
    if (!item) return res.status(404).json({ error: "REVIEW_QUEUE_ITEM_NOT_FOUND" });
  ...
  app.post("/api/admin/fraud/:reviewId/decision", asyncHandler(async (req, res) => {
    ...
    if (!item) return res.status(404).json({ error: "FRAUD_REVIEW_NOT_FOUND" });
    res.status(200).json({ ok: true, data: item });
  }));
  ...
  app.get("/api/distribution/too-lost/status", asyncHandler(async (_req, res) => {
    try {
      ...
      console.error(error);
      console.error((error as { stack?: unknown } | null | undefined)?.stack);
      return res.status(500).json({
        success: false,
        stage: typeof error === "object" && error && "stage" in error ? String((error as { stage?: unknown }).stage) : "operationsServer.getTooLostStatus",
  ...
  app.get("/api/distribution/too-lost/lookup/:resource", asyncHandler(async (req, res) => {
    const resource = normalizeTooLostCategory(requestParam(req.params.resource), ["genres", "languages", "countries", "platforms"] as const);
    if (!resource) return res.status(404).json({ error: "UNKNOWN_LOOKUP_RESOURCE" });
  ...
  app.get("/api/distribution/too-lost/preferences/:resource", asyncHandler(async (req, res) => {
    const resource = normalizeTooLostCategory(requestParam(req.params.resource), ["artist", "label", "spotify", "apple", "youtube", "search"] as const);
    if (!resource) return res.status(404).json({ error: "UNKNOWN_PREFERENCE_RESOURCE" });
  ...
  app.get("/api/distribution/too-lost/sales/:resource", asyncHandler(async (req, res) => {
    const resource = normalizeTooLostCategory(requestParam(req.params.resource), ["overview", "releases", "tracks", "artists", "channels", "territories", "stream-rates"] as const);
    if (!resource) return res.status(404).json({ error: "UNKNOWN_SALES_RESOURCE" });
  ...
  app.get("/api/distribution/too-lost/analytics/:resource", asyncHandler(async (req, res) => {
    const resource = normalizeTooLostCategory(requestParam(req.params.resource), ["overview", "tracks", "platforms", "charts", "audience", "release-links", "usage-discovery"] as const);
    if (!resource) return res.status(404).json({ error: "UNKNOWN_ANALYTICS_RESOURCE" });
...
function readJson<T extends Record<string, unknown> = Record<string, unknown>>(req: Request): T {
...
function normalizeDashboardName(value: string): DashboardName | null {
  const allowed: ReadonlySet<DashboardName> = new Set([
    "overview",
    "rights-review",
    "fraud-review",
    "content-review",
  ...
  return allowed.has(value as DashboardName) ? (value as DashboardName) : null;
}

function normalizeReportName(value: string): ReportName | null {
  const allowed: ReadonlySet<ReportName> = new Set([
    "rejected-releases",
    "duplicate-releases",
    "rights-conflicts",
    "delivery-failures",
  ...
  return allowed.has(value as ReportName) ? (value as ReportName) : null;
}

function normalizeReviewDecision(value: string): ReviewDecision | null {
  const allowed: ReadonlySet<ReviewDecision> = new Set(["approve", "reject", "needs_changes", "escalate"]);
  return allowed.has(value as ReviewDecision) ? (value as ReviewDecision) : null;
}

function normalizeFraudDecision(value: string): FraudDecision | null {
  const allowed: ReadonlySet<FraudDecision> = new Set(["APPROVE", "REJECT", "ESCALATE"]);
  return allowed.has(value as FraudDecision) ? (value as FraudDecision) : null;
}

function auditContextFromRequest(req: Request, body: Record<string, unknown>) {
  const trace = traceFromHeaders(req.headers);
  return {
...
function readLimit(query: Request["query"]): number {
...
function queryTextRecord(query: Request["query"]): Record<string, string> {
...
function isQueueName(value: string): value is Parameters<typeof inspectQueue>[0] {
...
function normalizeTooLostCategory<T extends string>(value: string, allowed: readonly T[]): T | null {
...
type ArtistDbRow = {
  ...
  nb_fan?: number | null;
  radio?: boolean | null;
  source: "local" | "distributed" | "too_lost" | "spotify" | "apple_music" | "deezer" | "musicbrainz";
  verified: boolean;
...
function mapArtistRow(row: Record<string, unknown>, fallbackProvider: string): ArtistDbRow {
  ...
  return {
    ...
    source: (typeof row.source === "string" ? row.source : fallbackProvider) as ArtistDbRow["source"],
...
function toArtistSelectionResponse(row: ArtistDbRow) {
  return {
    artist_id: row.id,
...
async function ensureDistributedArtistCache(db: { query<T>(sql: string, params: Record<string, unknown>): Promise<[T[], unknown]> }, supabase: SupabaseClient, rows: Array<{ name: string; provider: string; provider_artist_id: string | null; spotify_artist_id: string | null; apple_artist_id: string | null; image_url: string | null; verified: boolean | null }>, query: string) {
  const matched = rows.filter((row) => row.name && row.name.toLowerCase().includes(query.toLowerCase()));
  const cached: ArtistDbRow[] = [];
  for (const row of matched) {
    ...
    if (existing.data) {
      cached.push(mapArtistRow(existing.data as Record<string, unknown>, "distributed"));
      continue;
    }
    const created = await upsertArtistRecord({
      name,
...
type ArtistSearchContext = {
  ...
  db: { query<T>(sql: string, params: Record<string, unknown>): Promise<[T[], unknown]> };
  query: string;
  limit: number;
};

async function searchArtistsAcrossSources(input: ArtistSearchContext) {
  ...
  const [distributedRows] = await input.db.query<{
    name: string;
    provider: string;
    provider_artist_id: string | null;
    spotify_artist_id: string | null;
  ...
  const merged = mergeArtistSearchResults([
    ...localArtists,
    ...distributedArtists.map((artist) => ({ ...artist, source: "distributed" as const })),
  ], normalized);

  return merged.slice(0, input.limit);
}

async function searchLocalArtists(supabase: SupabaseClient, query: string, limit: number) {
  ...
  const { data, error } = await supabase
    ...
    .limit(limit * 2);
  if (error) throw error;
  return ((data || []) as Array<Record<string, unknown>>).map((row) => mapArtistRow({ ...row, source: typeof row.provider === "string" ? String(row.provider) : "local" }, "local"));
}
...
async function upsertArtistRecord(input: {
  name: string;
  displayName: string;
  slug: string;
  provider: string;
  ...
  biography?: string | null;
  verified: boolean;
}, supabase: SupabaseClient): Promise<ArtistDbRow> {
  ...
  if (existing.error) throw existing.error;
  if (existing.data) {
    return mapArtistRow(existing.data as Record<string, unknown>, input.provider);
  }
  ...
  if (error) throw error;
  return mapArtistRow(data as Record<string, unknown>, input.provider);
}

function mergeArtistSearchResults(artists: ArtistDbRow[], query: string) {
  const normalizedQuery = normalizeArtistQuery(query).toLowerCase();
  const deduped = new Map<string, ArtistDbRow & { sortScore: number }>();
...
function scoreArtistMatch(artist: ArtistDbRow, query: string) {
  const haystack = `${artist.display_name} ${artist.name} ${artist.slug}`.toLowerCase();
  if (!query) return 0;
  if (haystack === query) return 500;
  if (haystack.startsWith(query)) return 400;
  if (haystack.includes(query)) return 300;
  return 0;
}

function normalizeArtistKey(displayName: string, name: string, spotifyArtistId?: string | null, providerArtistId?: string | null) {
...
function sourceRank(source: ArtistDbRow["source"]) {
  switch (source) {
    case "local": return 0;
...
function sourceMetadataPriority(source: ArtistDbRow["source"]) {
  switch (source) {
    case "local": return 4;
...
async function searchSpotifyArtists(query: string, limit: number, cacheRef: SpotifyTokenCacheRef): Promise<Array<{
  id: string;
  name: string;
  popularity: number | null;
  followers: number | null;
  ...
  const payload = safeJsonParse(bodyText) as { artists?: { items?: SpotifySearchArtist[] } };
...
async function getSpotifyAccessToken(clientId: string, clientSecret: string, cacheRef: SpotifyTokenCacheRef) {
  const now = Date.now();
  if (cacheRef.current && cacheRef.current.expiresAt > now + 60_000) return cacheRef.current.token;
  ...
  const payload = safeJsonParse(bodyText) as { access_token?: string; expires_in?: number };
...
async function fetchTooLostArtistSearch(query: string): Promise<Array<Record<string, unknown>>> {
  void query;
  return [];
}

function extractArtistCandidates(value: unknown): Array<Record<string, unknown>> {
  if (!value) return [];
  ...
  if (typeof value === "object") {
    ...
    for (const key of ["items", "data", "artists", "results"]) {
      ...
      if (Array.isArray(nested)) return nested.filter((item) => item && typeof item === "object") as Array<Record<string, unknown>>;
    }
  }
  return [];
}
