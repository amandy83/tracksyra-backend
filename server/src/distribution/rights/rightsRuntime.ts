...
import type { RightsQueueJob } from "../../queue/jobTypes";
import type {
  RightsAuditEvent,
  RightsConflictRecord,
  RightsConflictSeverity,
  RightsConflictType,
  RightsDspName,
  RightsLicenseMatrixEntry,
  RightsLicenseRecord,
  RightsLicenseStatus,
  RightsLicenseWindow,
  RightsLogger,
  RightsOwnershipInput,
  RightsOwnershipRecord,
  RightsReport,
  RightsTerritoryLicenseInput,
  RightsTerritoryMode,
  RightsValidationIssue,
  RightsValidationResult,
  RightsWithdrawalKind,
  RightsWithdrawalRecord,
  RightsWorkerResult,
  RightsScope,
} from "./rightsTypes";

type RightsRuntimeDependencies = Readonly<{
  sql: SqlExecutor;
...
type RightsVersionRecord = Readonly<{
  key: string;
  id: string;
  createdAt: string;
}>;
...
function normalizeOptionalTerritories(values: readonly string[] | null | undefined): readonly string[] {
  const territories = new Set<string>();
  for (const value of values ?? []) {
    const territory = value.trim().toUpperCase();
    if (territory) territories.add(territory);
...
function normalizeWindow(input: RightsLicenseWindow | null | undefined): RightsLicenseWindow {
  return freeze({
...
function normalizeStatus(value: RightsLicenseStatus | null | undefined): RightsLicenseStatus {
  return value ?? "enabled";
}

function normalizeTerritoryMode(value: RightsTerritoryMode | null | undefined): RightsTerritoryMode {
  return value ?? "worldwide";
}

function normalizeDsp(value: RightsDspName | null | undefined): RightsDspName | null {
  return normalizeOptionalText(value ?? null) as RightsDspName | null;
}

function recordOwnershipKey(input: Readonly<{
  releaseId: string;
  trackId: string | null;
  ownerType: string;
  ownerName: string;
...
function recordLicenseKey(input: Readonly<{
  releaseId: string;
  trackId: string | null;
...
function recordConflictKey(input: Readonly<{
  releaseId: string;
  trackId: string | null;
...
function ownershipFromRow(row: PersistedRow): RightsOwnershipRecord {
  ...
  const rightsScope = String(row.rights_scope ?? metadata.rightsScope ?? "master") as RightsScope;
  ...
  return freeze({
    ...
    releaseId: String(row.release_id ?? ""),
    trackId: row.track_id ? String(row.track_id) : null,
    ownerType: String(row.owner_type ?? "rightsholder") as RightsOwnershipRecord["ownerType"],
    ownerName: String(row.owner_name ?? "Unknown"),
    rightsScope,
    territories,
    ...
    status: normalizeStatus(String(row.status ?? metadata.status ?? "enabled") as RightsLicenseStatus),
    source: String(row.source ?? metadata.source ?? "system"),
    licenseWindow: normalizeWindow(metadata.licenseWindow as RightsLicenseWindow | null | undefined),
...
function licenseFromRow(row: PersistedRow): RightsLicenseRecord {
  const metadata = readMetadata(row.metadata);
  ...
  const territoryMode = normalizeTerritoryMode(String(metadata.territoryMode ?? "worldwide") as RightsTerritoryMode);
  const dsp = normalizeDsp(metadata.dsp ? String(metadata.dsp) as RightsDspName : null);
...
function conflictFromRow(row: PersistedRow): RightsConflictRecord {
  ...
  const conflictType = String(row.conflict_type ?? details.conflictType ?? "duplicate_ownership") as RightsConflictType;
  ...
  return freeze({
    ...
    trackId: row.track_id ? String(row.track_id) : null,
    conflictType,
    severity: String(row.severity ?? details.severity ?? "warning") as RightsConflictSeverity,
...
function withdrawalFromRow(row: PersistedRow): RightsWithdrawalRecord {
  const metadata = readMetadata(row.metadata);
  const kind = String(metadata.kind ?? "catalog") as RightsWithdrawalKind;
  ...
  return freeze({
    ...
    trackId: row.track_id ? String(row.track_id) : null,
    kind,
    dsp: normalizeDsp(metadata.dsp ? String(metadata.dsp) as RightsDspName : null),
    ...
    status: normalizeStatus(String(row.status ?? metadata.status ?? "pending") as RightsLicenseStatus),
    metadata: freeze({ ...metadata }),
    createdAt: String(row.created_at ?? nowIso()),
    updatedAt: String(row.updated_at ?? row.created_at ?? nowIso()),
  });
...
function auditFromRow(row: PersistedRow): RightsAuditEvent {
  const metadata = readMetadata(row.metadata);
  return freeze({
...
function readMetadata(value: unknown): Readonly<Record<string, unknown>> {
  return value && typeof value === "object" ? freeze(value as Readonly<Record<string, unknown>>) : freeze({});
}
...
function territoriesOverlap(a: readonly string[], b: readonly string[]): boolean {
  if (a.includes("WORLD") || b.includes("WORLD")) return true;
  const shared = new Set(a);
  return b.some((territory) => shared.has(territory));
}

function licenseActive(record: RightsLicenseRecord, referenceTime: string): boolean {
  if (record.status !== "enabled" && record.status !== "pending") return false;
...
function conflictIdSuffix(type: RightsConflictType, references: readonly string[]): string {
...
class RightsRegistry {
  private readonly ownerships = new Map<string, RightsOwnershipRecord>();
  private readonly ownershipVersions = new Map<string, RightsVersionRecord>();
  private readonly licenses = new Map<string, RightsLicenseRecord>();
  private readonly licenseVersions = new Map<string, RightsVersionRecord>();
  private readonly conflicts = new Map<string, RightsConflictRecord>();
  private readonly withdrawals = new Map<string, RightsWithdrawalRecord>();
  private readonly audits = new Map<string, readonly RightsAuditEvent[]>();

  replaceOwnership(record: RightsOwnershipRecord): RightsOwnershipRecord {
  ...
  replaceLicense(record: RightsLicenseRecord): RightsLicenseRecord {
  ...
  replaceConflict(record: RightsConflictRecord): RightsConflictRecord {
    this.conflicts.set(record.conflictKey, record);
    return record;
  }

  resolveConflict(conflictKey: string, resolvedBy: string): RightsConflictRecord | null {
    const existing = this.conflicts.get(conflictKey);
    if (!existing || existing.resolved) return existing ?? null;
  ...
  replaceWithdrawal(record: RightsWithdrawalRecord): RightsWithdrawalRecord {
    this.withdrawals.set(record.withdrawalKey, record);
    return record;
  }

  recordAudit(event: RightsAuditEvent): RightsAuditEvent {
    const current = this.audits.get(event.aggregateId) ?? [];
    this.audits.set(event.aggregateId, freezeList([...current, event]));
    return event;
  }

  listOwnerships(releaseId?: string, trackId?: string | null): readonly RightsOwnershipRecord[] {
    return freezeList([...this.ownerships.values()].filter((record) =>
      (releaseId ? record.releaseId === releaseId : true) &&
      (trackId == null ? true : record.trackId === trackId),
    ));
  }

  listLicenses(releaseId?: string, trackId?: string | null): readonly RightsLicenseRecord[] {
    return freezeList([...this.licenses.values()].filter((record) =>
      (releaseId ? record.releaseId === releaseId : true) &&
      (trackId == null ? true : record.trackId === trackId),
    ));
  }

  listConflicts(releaseId?: string, trackId?: string | null): readonly RightsConflictRecord[] {
    return freezeList([...this.conflicts.values()].filter((record) =>
      (releaseId ? record.releaseId === releaseId : true) &&
      (trackId == null ? true : record.trackId === trackId),
    ));
  }

  listWithdrawals(releaseId?: string, trackId?: string | null): readonly RightsWithdrawalRecord[] {
    return freezeList([...this.withdrawals.values()].filter((record) =>
      (releaseId ? record.releaseId === releaseId : true) &&
      (trackId == null ? true : record.trackId === trackId),
    ));
  }

  listAuditEvents(releaseId?: string): readonly RightsAuditEvent[] {
    return freezeList([...this.audits.values()].flat().filter((record) => (releaseId ? record.aggregateId === releaseId : true)));
  }

  getOwnershipById(rightsId: string): RightsOwnershipRecord | null {
    const version = this.ownershipVersions.get(rightsId);
    return version ? this.ownerships.get(version.key) ?? null : null;
  }

  getLicenseById(licenseId: string): RightsLicenseRecord | null {
    const version = this.licenseVersions.get(licenseId);
    return version ? this.licenses.get(version.key) ?? null : null;
  }

  hydrateOwnerships(records: readonly RightsOwnershipRecord[]): void {
    for (const record of records) this.replaceOwnership(record);
  }

  hydrateLicenses(records: readonly RightsLicenseRecord[]): void {
    for (const record of records) this.replaceLicense(record);
  }

  hydrateConflicts(records: readonly RightsConflictRecord[]): void {
    for (const record of records) this.replaceConflict(record);
  }

  hydrateWithdrawals(records: readonly RightsWithdrawalRecord[]): void {
    for (const record of records) this.replaceWithdrawal(record);
  }

  hydrateAudits(records: readonly RightsAuditEvent[]): void {
    for (const event of records) this.recordAudit(event);
  }
}

export type EnterpriseRightsOwnershipInput = RightsOwnershipInput & Readonly<{
  dspLicenses?: readonly RightsTerritoryLicenseInput[];
  territoryLicenses?: readonly RightsTerritoryLicenseInput[];
  actor?: string | null;
  reason?: string | null;
  correlationId?: string | null;
  ipAddress?: string | null;
...
export type EnterpriseRightsUpdateInput = Readonly<{
  rightsId: string;
  ownerType?: RightsOwnershipInput["ownerType"];
  ownerName?: string;
...
export type EnterpriseRightsTransferInput = Readonly<{
  rightsId: string;
  newOwnerType: RightsOwnershipInput["ownerType"];
  newOwnerName: string;
  releaseId?: string;
  trackId?: string | null;
  actor?: string | null;
...
export type EnterpriseRightsTerritoryInput = Readonly<{
  releaseId: string;
  trackId?: string | null;
  territoryMode: RightsTerritoryMode;
  territories?: readonly string[];
...
export type EnterpriseRightsWithdrawInput = Readonly<{
  releaseId: string;
  trackId?: string | null;
...
export class EnterpriseRightsService {
  ...
  constructor(
    private readonly dependencies: RightsRuntimeDependencies,
    private readonly registry: RightsRegistry,
  ) {}

  async hydrate(): Promise<void> {
    if (this.hydrated) return;
    const [ownershipRows, licenseRows, conflictRows, withdrawalRows, auditRows] = await Promise.all([
      this.dependencies.sql.query<PersistedRow>("SELECT * FROM public.rights_ownership ORDER BY created_at ASC"),
      this.dependencies.sql.query<PersistedRow>("SELECT * FROM public.rights_ownership WHERE COALESCE(CAST(metadata ->> 'recordKind' AS text), 'ownership') = 'license' ORDER BY created_at ASC"),
      this.dependencies.sql.query<PersistedRow>("SELECT * FROM public.ownership_conflicts ORDER BY created_at ASC"),
      this.dependencies.sql.query<PersistedRow>("SELECT * FROM public.withdrawal_queue ORDER BY created_at ASC"),
      this.dependencies.sql.query<PersistedRow>("SELECT * FROM public.audit_events WHERE aggregate_type IN ('rights_ownership', 'rights_license', 'rights_conflict', 'rights_withdrawal', 'rights_audit') ORDER BY created_at ASC"),
    ]);
  ...
  async registerRights(input: EnterpriseRightsOwnershipInput): Promise<readonly RightsOwnershipRecord[]> {
    await this.hydrate();
    const ownerships: RightsOwnershipRecord[] = [];
    ...
    for (const rightsScope of (input.rightsScopes.length ? input.rightsScopes : ["master"] as RightsScope[])) {
  ...
  async updateRights(input: EnterpriseRightsUpdateInput): Promise<RightsOwnershipRecord | null> {
    await this.hydrate();
  ...
  async transferRights(input: EnterpriseRightsTransferInput): Promise<RightsOwnershipRecord | null> {
    await this.hydrate();
    const current = this.registry.getOwnershipById(input.rightsId);
    if (!current) return null;
    const actor = normalizeOptionalText(input.actor ?? null) ?? "system";
    ...
    const withdrawn = freeze({
      ...current,
      status: "withdrawn" as RightsLicenseStatus,
      updatedAt: this.now(),
    ...
    const transferred = freeze({
      ...
      ownerType: input.newOwnerType,
      ownerName: normalizeText(input.newOwnerName),
      status: "enabled" as RightsLicenseStatus,
  ...
  async assignTerritories(input: EnterpriseRightsTerritoryInput): Promise<RightsLicenseRecord> {
    await this.hydrate();
    const actor = normalizeOptionalText(input.actor ?? null) ?? "system";
    const reason = normalizeOptionalText(input.reason ?? null);
    const correlationId = normalizeOptionalText(input.correlationId ?? null);
  ...
  async withdrawRights(input: EnterpriseRightsWithdrawInput): Promise<RightsWithdrawalRecord> {
    await this.hydrate();
    const actor = normalizeOptionalText(input.actor ?? null) ?? "system";
    const correlationId = normalizeOptionalText(input.correlationId ?? null);
    const ipAddress = normalizeOptionalText(input.ipAddress ?? null);
    ...
    for (const ownership of this.registry.listOwnerships(input.releaseId, input.trackId)) {
      if (input.dsp && ownership.metadata.dsp !== input.dsp) continue;
      if (input.territory && !ownership.territories.includes(input.territory.toUpperCase())) continue;
      const withdrawnOwnership = freeze({ ...ownership, status: "withdrawn" as RightsLicenseStatus, updatedAt: this.now(), metadata: freeze({ ...ownership.metadata, withdrawnAt: this.now(), withdrawalKey }) });
    ...
    for (const license of this.registry.listLicenses(input.releaseId, input.trackId)) {
      if (input.dsp && license.dsp !== input.dsp) continue;
      if (input.territory && !license.territories.includes(input.territory.toUpperCase())) continue;
  ...
  async restoreRights(input: Readonly<{
    withdrawalId: string;
    actor?: string | null;
    reason?: string | null;
    correlationId?: string | null;
    ipAddress?: string | null;
  }>): Promise<RightsWithdrawalRecord | null> {
    await this.hydrate();
    ...
    for (const ownership of this.registry.listOwnerships(restored.releaseId, restored.trackId)) {
      if (restored.dsp && ownership.metadata.dsp !== restored.dsp) continue;
      if (restored.territory && !ownership.territories.includes(restored.territory.toUpperCase())) continue;
      if (ownership.status === "withdrawn") {
        const revived = freeze({ ...ownership, status: "enabled" as RightsLicenseStatus, updatedAt: this.now(), metadata: freeze({ ...ownership.metadata, restoredAt: this.now(), restoreReason: reason }) });
    ...
    for (const license of this.registry.listLicenses(restored.releaseId, restored.trackId)) {
      if (restored.dsp && license.dsp !== restored.dsp) continue;
      if (restored.territory && !license.territories.includes(restored.territory.toUpperCase())) continue;
  ...
  async verifyRights(releaseId: string): Promise<RightsValidationResult> {
    await this.hydrate();
    ...
    const validation: RightsValidationResult = freeze({
  ...
  async generateRightsReport(releaseId?: string): Promise<RightsReport> {
    await this.hydrate();
    const ownerships = this.registry.listOwnerships(releaseId);
    const licenses = this.registry.listLicenses(releaseId);
  ...
  async generateTerritoryReport(releaseId?: string): Promise<Readonly<{
    generatedAt: string;
    territories: readonly RightsLicenseRecord[];
  ...
  async generateConflictReport(releaseId?: string): Promise<Readonly<{
    generatedAt: string;
  ...
  async generateLicenseMatrix(releaseId?: string): Promise<readonly RightsLicenseMatrixEntry[]> {
    await this.hydrate();
    return freezeList(this.buildLicenseMatrix(this.registry.listLicenses(releaseId)));
  }

  async listAuditEvents(releaseId?: string): Promise<readonly RightsAuditEvent[]> {
    await this.hydrate();
    return this.registry.listAuditEvents(releaseId);
  }

  async listConflicts(releaseId?: string): Promise<readonly RightsConflictRecord[]> {
    await this.hydrate();
    return this.registry.listConflicts(releaseId);
  }

  async listOwnerships(releaseId?: string): Promise<readonly RightsOwnershipRecord[]> {
    await this.hydrate();
    return this.registry.listOwnerships(releaseId);
  }

  private async persistOwnership(
    record: RightsOwnershipRecord,
    actor: string,
    reason: string | null,
    correlationId: string | null,
    ipAddress: string | null,
    previous: RightsOwnershipRecord | null = null,
  ): Promise<void> {
    const payload = {
      releaseId: record.releaseId,
      trackId: record.trackId,
    ...
    await this.dependencies.sql.query(
  ...
  private async persistLicense(
    input: RightsTerritoryLicenseInput,
    actor: string,
    reason: string | null,
    correlationId: string | null,
    ipAddress: string | null,
    ...
    const record: RightsLicenseRecord = freeze({
  ...
  private async persistAuditEvent(input: {
    aggregateType: string;
    aggregateId: string;
    action: string;
    actor: string;
    ...
    const event: RightsAuditEvent = buildAudit(
      input.aggregateType,
      input.aggregateId,
      input.action,
      input.oldValue,
  ...
  private async persistValidationResult(releaseId: string, validation: RightsValidationResult): Promise<void> {
    await this.dependencies.sql.query(
      `INSERT INTO public.catalog_validation_results (
         release_id,
         validation_type,
         severity,
         status,
  ...
  private async sql(
    table: "withdrawal_queue" | "takedown_queue",
    record: RightsWithdrawalRecord,
    actor: string,
    correlationId: string | null,
    ipAddress: string | null,
  ): Promise<void> {
  ...
  private async refreshExpirations(releaseId?: string): Promise<void> {
    ...
    for (const license of this.registry.listLicenses(releaseId)) {
      ...
      if (endTime != null && endTime < Date.parse(referenceTime) && license.status !== "expired") {
        const expired = freeze({ ...license, status: "expired" as RightsLicenseStatus, updatedAt: referenceTime, metadata: freeze({ ...license.metadata, expiredAt: referenceTime }) });
  ...
  private detectConflicts(
  ...
  ): readonly RightsConflictRecord[] {
    const conflicts: RightsConflictRecord[] = [];
    const ownershipGroups = new Map<string, RightsOwnershipRecord[]>();
    ...
    for (let left = 0; left < ownerships.length; left += 1) {
      for (let right = left + 1; right < ownerships.length; right += 1) {
        ...
        if (a.releaseId !== b.releaseId || a.trackId !== b.trackId || a.rightsScope !== b.rightsScope) continue;
        if (a.ownerName === b.ownerName) continue;
        if (!territoriesOverlap(a.territories, b.territories)) continue;
    ...
    if ((releaseMetadata as Record<string, unknown>).fraudIndicator ?? false) {
  ...
  private async syncConflicts(conflicts: readonly RightsConflictRecord[]): Promise<void> {
  ...
  private async persistConflict(conflict: RightsConflictRecord): Promise<void> {
    this.registry.replaceConflict(conflict);
    const existingRows = await this.dependencies.sql.query<{ id: string }>(
      `SELECT id
    ...
    if (existingRows.length > 0) {
      await this.dependencies.sql.query(
        `UPDATE public.ownership_conflicts
    ...
    await this.dependencies.sql.query(
      `INSERT INTO public.ownership_conflicts (
         release_id,
         track_id,
         conflict_type,
         severity,
  ...
  private async resolveConflict(conflictKey: string, resolvedBy: string): Promise<void> {
    ...
    await this.dependencies.sql.query(
      `UPDATE public.ownership_conflicts
       ...
       WHERE CAST(details ->> 'conflictKey' AS text) = :conflictKey`,
      { conflictKey, resolvedBy },
    );
  }

  private dedupeConflicts(conflicts: readonly RightsConflictRecord[]): readonly RightsConflictRecord[] {
    const seen = new Set<string>();
    const output: RightsConflictRecord[] = [];
    for (const conflict of conflicts) {
      if (seen.has(conflict.conflictKey)) continue;
  ...
  private buildLicenseMatrix(licenses: readonly RightsLicenseRecord[]): readonly RightsLicenseMatrixEntry[] {
    const matrix = new Map<string, RightsLicenseMatrixEntry>();
    for (const license of licenses) {
      ...
      if (license.trackId) trackIds.add(license.trackId);
  ...
  private countByScope(ownerships: readonly RightsOwnershipRecord[]): Readonly<Record<RightsScope, number>> {
    const result: Record<string, number> = {};
    for (const ownership of ownerships) result[ownership.rightsScope] = (result[ownership.rightsScope] ?? 0) + 1;
    return result as Readonly<Record<RightsScope, number>>;
  }

  private countByTerritory(ownerships: readonly RightsOwnershipRecord[]): Readonly<Record<string, number>> {
    const result: Record<string, number> = {};
    for (const ownership of ownerships) {
  ...
  private toIssue(conflict: RightsConflictRecord, severity: "error" | "warning"): RightsValidationIssue {
    return freeze({
...
export class RightsValidationWorker {
  readonly name = "rights-validation";
  constructor(private readonly dependencies: Readonly<{ service: EnterpriseRightsService; logger: RightsLogger }>) {}

  async process(job: Job<RightsQueueJob>): Promise<RightsWorkerResult> {
...
export class TerritorySyncWorker {
  readonly name = "rights-territory-sync";
  constructor(private readonly dependencies: Readonly<{ service: EnterpriseRightsService; logger: RightsLogger }>) {}
...
export class ConflictDetectionWorker {
  readonly name = "rights-conflict-detection";
  constructor(private readonly dependencies: Readonly<{ service: EnterpriseRightsService; logger: RightsLogger }>) {}
...
export class WithdrawalWorker {
  readonly name = "rights-withdrawal";
  constructor(private readonly dependencies: Readonly<{ service: EnterpriseRightsService; logger: RightsLogger }>) {}
...
export class LicenseExpirationWorker {
  readonly name = "rights-license-expiration";
  constructor(private readonly dependencies: Readonly<{ service: EnterpriseRightsService; logger: RightsLogger }>) {}
...
export type EnterpriseRightsRuntimeBundle = Readonly<{
  registry: RightsRegistry;
  service: EnterpriseRightsService;
  validationWorker: RightsValidationWorker;
  territorySyncWorker: TerritorySyncWorker;
  conflictDetectionWorker: ConflictDetectionWorker;
  withdrawalWorker: WithdrawalWorker;
  licenseExpirationWorker: LicenseExpirationWorker;
  rightsAuditWorker: RightsAuditWorker;
}>;

export function createEnterpriseRightsRuntime(dependencies: RightsRuntimeDependencies): EnterpriseRightsRuntimeBundle {
...
export function registerRightsValidationQueueWorker(service: EnterpriseRightsService, logger: RightsLogger, options: { concurrency?: number } = {}): WorkerLike {
  return createWorker(queueNames.rightsValidation, async (job: Job<RightsQueueJob>) => new RightsValidationWorker({ service, logger }).process(job), { concurrency: options.concurrency });
}

export function registerRightsTerritorySyncQueueWorker(service: EnterpriseRightsService, logger: RightsLogger, options: { concurrency?: number } = {}): WorkerLike {
  return createWorker(queueNames.rightsTerritorySync, async (job: Job<RightsQueueJob>) => new TerritorySyncWorker({ service, logger }).process(job), { concurrency: options.concurrency });
}

export function registerRightsConflictQueueWorker(service: EnterpriseRightsService, logger: RightsLogger, options: { concurrency?: number } = {}): WorkerLike {
  return createWorker(queueNames.rightsConflictDetection, async (job: Job<RightsQueueJob>) => new ConflictDetectionWorker({ service, logger }).process(job), { concurrency: options.concurrency });
}

export function registerRightsWithdrawalQueueWorker(service: EnterpriseRightsService, logger: RightsLogger, options: { concurrency?: number } = {}): WorkerLike {
  return createWorker(queueNames.rightsWithdrawal, async (job: Job<RightsQueueJob>) => new WithdrawalWorker({ service, logger }).process(job), { concurrency: options.concurrency });
}

export function registerRightsLicenseExpirationQueueWorker(service: EnterpriseRightsService, logger: RightsLogger, options: { concurrency?: number } = {}): WorkerLike {
  return createWorker(queueNames.rightsLicenseExpiration, async (job: Job<RightsQueueJob>) => new LicenseExpirationWorker({ service, logger }).process(job), { concurrency: options.concurrency });
}

export function registerRightsAuditQueueWorker(service: EnterpriseRightsService, logger: RightsLogger, options: { concurrency?: number } = {}): WorkerLike {
  return createWorker(queueNames.rightsAudit, async (job: Job<RightsQueueJob>) => new RightsAuditWorker({ service, logger }).process(job), { concurrency: options.concurrency });
}

export function generateRightsFingerprint(input: Readonly<Record<string, unknown>>): string {
