...
import { Contributor, DistributionVersion, Release, ReleaseId, ReleaseVersion, Track, TerritorySet } from "../../domain";
import type { DistributionRelease, DistributionTrack } from "../../models/distributionTypes";
import type {
  AudioDuplicateReport,
  AudioFraudReport,
  AudioFraudSignal,
  AudioFingerprintComparison,
  AudioFingerprintDuplicateMatch,
  AudioFingerprintGenerationResult,
  AudioFingerprintJobInput,
  AudioFingerprintRecord,
  AudioFingerprintReport,
  AudioFingerprintSimilarityScore,
  AudioRightsMatchReport,
  AudioSimilarityReport,
  CatalogDuplicateReport,
  DuplicateMatchKind,
  FingerprintCapabilityMatrix,
  AudioFingerprintFeatureVector,
  AudioFingerprintMetrics,
} from "./audioFingerprintingTypes";

export type AudioFingerprintingEngineDependencies = Readonly<{
  sql: SqlExecutor;
...
type FingerprintSource = Readonly<{
...
type StoredFingerprintRow = Readonly<{
  id: string;
  release_id: string;
  track_id: string | null;
  asset_id: string | null;
  acoustic_fingerprint_hash: string;
  chromaprint_compatible_hash: string;
  waveform_hash: string;
  spectral_hash: string;
  tempo_hash: string;
  rhythm_hash: string;
  frequency_hash: string;
  overall_hash: string;
...
type FingerprintAnalysis = Readonly<{
...
type FingerprintCandidate = Readonly<{
  row: StoredFingerprintRow;
  comparison: AudioFingerprintComparison;
  rowToFingerprint: AudioFingerprintRecord;
}>;

type StoredDuplicateRow = Readonly<{
  match_id: string;
  fingerprint_id: string;
  release_id: string;
  track_id: string | null;
  matched_release_id: string;
...
type StoredSimilarityRow = Readonly<{
  similarity_id: string;
  fingerprint_id: string;
  release_id: string;
  track_id: string | null;
  compared_release_id: string;
  compared_track_id: string | null;
  waveform_similarity: number;
  spectral_similarity: number;
  tempo_similarity: number;
  pitch_similarity: number;
  silence_similarity: number;
  dynamic_range_similarity: number;
...
function normalizedDistance(left: number, right: number, scale = 1): number {
  const distance = Math.abs(left - right) / Math.max(scale, Math.abs(left), Math.abs(right), 1);
  return clamp(1 - distance, 0, 1);
}

function compareValues(left: readonly number[], right: readonly number[]): number {
...
function analyzeBuffer(buffer: Buffer, sampleRateHz: number): FingerprintAnalysis {
  ...
  const metrics: AudioFingerprintMetrics = freeze({
  ...
  const features: AudioFingerprintFeatureVector = freeze({
...
function bucketize(values: readonly number[], buckets: number): number[] {
  ...
  while (result.length < buckets) result.push(0);
  return result.slice(0, buckets).map((value) => round(value));
}

function frameEnergy(values: readonly number[], frameSize: number, hopSize: number): number[] {
  const result: number[] = [];
...
function estimateTempo(frames: readonly number[], sampleRateHz: number, hopSize: number): number {
...
function tempoStrength(frames: readonly number[], bpm: number): number {
  if (!frames.length || bpm <= 0) return 0;
...
function tempoVariance(frames: readonly number[], bpm: number): number {
  if (!frames.length || bpm <= 0) return 0;
...
function peakIntervals(frames: readonly number[]): number[] {
  const peaks: number[] = [];
...
function rhythmIntervals(frames: readonly number[], bpm: number): number[] {
...
function estimateZeroCrossingRate(values: readonly number[]): number {
  if (values.length < 2) return 0;
  let count = 0;
  for (let index = 1; index < values.length; index += 1) {
    if ((values[index - 1] ?? 0) <= 0 && (values[index] ?? 0) > 0) count += 1;
    else if ((values[index - 1] ?? 0) >= 0 && (values[index] ?? 0) < 0) count += 1;
  }
  return count / values.length;
}

function spectralCentroid(bands: readonly number[]): number {
...
function spectralSpread(bands: readonly number[]): number {
  if (!bands.length) return 0;
...
function spectralRolloff(bands: readonly number[]): number {
  if (!bands.length) return 0;
  const total = bands.reduce((sum, value) => sum + value, 0);
  let cumulative = 0;
...
function goertzelBands(values: readonly number[], sampleRateHz: number, bandCount: number): number[] {
  const window = values.slice(0, Math.min(values.length, 4096));
  if (!window.length) return Array.from({ length: bandCount }, () => 0);
  const startFreq = 40;
...
function goertzelMagnitude(values: readonly number[], sampleRateHz: number, targetFreq: number): number {
  const n = values.length;
  if (!n || targetFreq <= 0) return 0;
...
function dctSignature(values: readonly number[], coefficientCount: number): number[] {
...
function confidenceFromFrames(frames: readonly number[], durationSeconds: number, silenceRatio: number): number {
...
function compareFingerprintRecords(left: AudioFingerprintRecord, right: AudioFingerprintRecord): AudioFingerprintComparison {
...
function classifyDuplicate(left: AudioFingerprintRecord, right: AudioFingerprintRecord, similarityScore: number, confidenceScore: number): DuplicateMatchKind {
  if (left.acousticFingerprintHash === right.acousticFingerprintHash || left.overallHash === right.overallHash) return "exact_duplicate";
  ...
  if (similarityScore >= 0.985 && confidenceScore >= 0.9) return "near_duplicate";
  if (sameArtist && sameLabel && similarityScore >= 0.96 && durationRatio >= 0.85 && durationRatio <= 1.15 && (left.metrics.dynamicRange > right.metrics.dynamicRange + 0.08 || right.metrics.dynamicRange > left.metrics.dynamicRange + 0.08)) return "remaster";
  if (similarityScore >= 0.9 && durationRatio >= 0.55 && durationRatio <= 0.85) return "radio_edit";
  if (similarityScore >= 0.9 && durationRatio > 1.15) return "extended_mix";
  if (similarityScore >= 0.88 && (left.metrics.zeroCrossingRate > right.metrics.zeroCrossingRate * 1.2 || right.metrics.zeroCrossingRate > left.metrics.zeroCrossingRate * 1.2)) return "pitch_shifted";
  if (similarityScore >= 0.88 && Math.abs(left.metrics.bpm - right.metrics.bpm) / Math.max(1, left.metrics.bpm, right.metrics.bpm) > 0.15) return "time_stretched";
  if (similarityScore >= 0.88 && Math.max(left.metrics.silenceRatio, right.metrics.silenceRatio) > 0.45) return "noise_altered";
  if (similarityScore >= 0.9 && durationRatio < 0.75) return "partial_upload";
  if (similarityScore >= 0.9 && sameArtist && titleSimilar && durationRatio >= 0.95 && durationRatio <= 1.05) return "album_duplicate";
  if (similarityScore >= 0.9 && !sameArtist && titleSimilar) return "cover_version";
  if (similarityScore >= 0.9 && typeof right.metadata.remixer === "string" && right.metadata.remixer) return "remix";
  if (similarityScore >= 0.9 && !sameArtist && (left.metadata.lyrics || right.metadata.lyrics) && (!left.metadata.lyrics || !right.metadata.lyrics)) return "instrumental";
  if (similarityScore >= 0.9 && titleLeft.toLowerCase().includes("live")) return "live_version";
  if (similarityScore >= 0.9 && /acapella/i.test(titleLeft) && /acapella/i.test(titleRight)) return "acapella";
  if (similarityScore >= 0.95 && sameLabel && !sameArtist) return "cross_label_duplicate";
  if (similarityScore >= 0.95 && !sameArtist && !sameLabel) return "cross_artist_duplicate";
  if (similarityScore >= 0.95) return "cross_release_duplicate";
  if (confidenceScore >= 0.85 && similarityScore >= 0.92) return "lossy_reencode";
  if (confidenceScore >= 0.8 && similarityScore >= 0.9) return "near_duplicate";
  return "none";
}

function duplicateReasons(left: AudioFingerprintRecord, right: AudioFingerprintRecord, similarityScore: number, duplicateType: DuplicateMatchKind): string[] {
  const reasons = [
...
function parseNumericArray(value: unknown, fallbackLength: number): readonly number[] {
...
function duplicateMatchKindFromComparison(comparison: AudioFingerprintComparison): DuplicateMatchKind {
  return comparison.duplicateType ?? "none";
}

function makeDuplicateMatch(input: {
  fingerprint: AudioFingerprintRecord;
  compared: AudioFingerprintRecord;
  comparison: AudioFingerprintComparison;
}): AudioFingerprintDuplicateMatch {
  return freeze({
...
function buildFraudSignals(
  ...
  rightsSummary: Readonly<Record<string, unknown>> | null,
): AudioFraudSignal[] {
  const signals: AudioFraudSignal[] = [];
...
function createCapabilityMatrix(): FingerprintCapabilityMatrix {
  return freeze({
...
function createEmptyRecord(input: { releaseId: string; trackId: string | null; assetId: string | null; metadata?: Readonly<Record<string, unknown>> }): AudioFingerprintRecord {
  const zero = freeze({
  ...
  return buildFingerprintRecord(randomUUID(), {
    release: { id: input.releaseId, userId: "", title: "", primaryArtist: "", metadata: input.metadata ?? {}, tracks: [] } as DistributionRelease,
    track: null,
...
export class AudioFingerprintingEngine {
  ...
  private readonly capabilityMatrix = createCapabilityMatrix();

  constructor(private readonly deps: AudioFingerprintingEngineDependencies) {
    this.log = deps.logger ?? defaultLogger.child({ component: "audio-fingerprint-engine" });
  }

  getCapabilities(): FingerprintCapabilityMatrix {
    return this.capabilityMatrix;
  }

  async generateFingerprint(input: AudioFingerprintJobInput): Promise<AudioFingerprintGenerationResult> {
    ...
    if (duplicates.length) incrementMetric("tracksyra_audio_duplicate_total", { releaseId: input.releaseId });
    if (fraudSignals.length) incrementMetric("tracksyra_audio_fraud_total", { releaseId: input.releaseId });
  ...
  async compareFingerprint(input: {
    left: AudioFingerprintRecord | null;
    right: AudioFingerprintRecord | null;
    leftFingerprintId?: string | null;
    rightFingerprintId?: string | null;
  ...
  async findDuplicates(input: AudioFingerprintJobInput): Promise<readonly AudioFingerprintDuplicateMatch[]> {
    const result = await this.generateFingerprint(input);
    return result.duplicates;
  }

  async findSimilarTracks(input: AudioFingerprintJobInput): Promise<readonly AudioFingerprintSimilarityScore[]> {
    const result = await this.generateFingerprint(input);
    return result.similarityScores.filter((score) => score.overallSimilarity >= 0.7);
  }

  async detectFraud(input: AudioFingerprintJobInput): Promise<readonly AudioFraudSignal[]> {
    const result = await this.generateFingerprint(input);
    return result.fraudSignals;
  }

  async generateFingerprintReport(releaseId?: string | null): Promise<AudioFingerprintReport> {
  ...
  async generateDuplicateReport(releaseId?: string | null): Promise<AudioDuplicateReport> {
  ...
  async generateSimilarityReport(releaseId?: string | null): Promise<AudioSimilarityReport> {
  ...
  async generateFraudReport(releaseId?: string | null): Promise<AudioFraudReport> {
  ...
  async generateRightsMatchReport(releaseId?: string | null): Promise<AudioRightsMatchReport> {
  ...
  async generateCatalogDuplicateReport(releaseId?: string | null): Promise<CatalogDuplicateReport> {
  ...
  async healthCheck(): Promise<Readonly<Record<string, unknown>>> {
    ...
    const totalFingerprints = Number((report.summary as { totalFingerprints?: number }).totalFingerprints ?? 0);
  ...
  async retry(input: AudioFingerprintJobInput & { error?: unknown; attempt?: number | null; }): Promise<AudioFingerprintGenerationResult> {
  ...
  async withdraw(input: { releaseId: string; trackId?: string | null; reason: string; actor?: string | null; correlationId?: string | null; }): Promise<void> {
  ...
  async restore(input: { releaseId: string; trackId?: string | null; reason: string; actor?: string | null; correlationId?: string | null; }): Promise<void> {
  ...
  async audit(input?: { releaseId?: string | null; }): Promise<AudioFingerprintReport> {
    return this.generateFingerprintReport(input?.releaseId ?? null);
  }

  async generateDashboard(input: { releaseId?: string | null; kind: "fingerprint" | "duplicate" | "similarity" | "fraud" | "catalog-duplicate"; }) {
    switch (input.kind) {
      case "fingerprint":
        return this.generateFingerprintReport(input.releaseId ?? null);
      case "duplicate":
  ...
  private async resolveSource(input: AudioFingerprintJobInput): Promise<FingerprintSource> {
  ...
  private resolveAudioSource(release: DistributionRelease | null, track: DistributionTrack | null): string | null {
  ...
  private async tryLegacyFingerprint(sourceUrl: string, assetId: string, trackId: string | null, waveformHash: string): Promise<{ acousticFingerprintHash: string; similarityScore: number } | null> {
    try {
  ...
  private async resolveAssetId(releaseId: string, trackId: string | null): Promise<string | null> {
    if (!trackId) return null;
    const rows = await this.deps.sql.query<{ id: string }>(
      `SELECT id FROM public.media_assets WHERE release_id = :releaseId::uuid AND track_id = :trackId::uuid ORDER BY created_at DESC LIMIT 1`,
      { releaseId, trackId },
    );
    return rows[0]?.id ?? null;
  }

  private toDomainRelease(release: DistributionRelease | null, track: DistributionTrack | null): import("../../domain").Release {
  ...
  private async readAudioSource(sourceUrl: string): Promise<Buffer> {
  ...
  private async persistFingerprint(fingerprint: AudioFingerprintRecord, source: FingerprintSource): Promise<AudioFingerprintRecord> {
    const fingerprintId = fingerprint.fingerprintId;
    await this.deps.sql.query(
      `INSERT INTO public.fingerprint_jobs (id, release_id, track_id, asset_id, job_type, status, attempts, max_attempts, metadata)
    ...
    await this.deps.sql.query(
      `INSERT INTO public.fingerprint_hashes (
         fingerprint_id,
         hash_type,
         hash_value,
         hash_algorithm,
    ...
    await this.deps.sql.query(
      `INSERT INTO public.audio_signatures (
         fingerprint_id,
         release_id,
         track_id,
         asset_id,
  ...
  private async persistAnalysis(input: {
    ...
    fraudSignals: readonly AudioFraudSignal[];
    rightsReport: Awaited<ReturnType<EnterpriseRightsService["generateRightsReport"]>> | null;
    ...
    for (const similarityScore of input.similarityScores) {
      await this.deps.sql.query(
        `INSERT INTO public.similarity_scores (
           fingerprint_id,
           release_id,
           track_id,
           compared_release_id,
    ...
    await this.deps.sql.query(
      `INSERT INTO public.fingerprint_history (
         fingerprint_id,
         release_id,
         track_id,
         action,
    ...
    await this.deps.sql.query(
      `INSERT INTO public.fingerprint_audit (
         fingerprint_id,
         aggregate_type,
         aggregate_id,
         action,
  ...
  private async persistRetry(input: AudioFingerprintJobInput & { error?: unknown; attempt?: number | null }, error: unknown): Promise<void> {
    await this.deps.sql.query(
      `INSERT INTO public.fingerprint_jobs (
         id,
         release_id,
         track_id,
         asset_id,
  ...
  private async findCandidateFingerprints(fingerprint: AudioFingerprintRecord, source: FingerprintSource): Promise<readonly FingerprintCandidate[]> {
    const rows = await this.loadFingerprintRows(null);
    const candidates: FingerprintCandidate[] = [];
    for (const row of rows) {
      if (row.id === fingerprint.fingerprintId) continue;
      ...
      if (!sameRelease && !sameArtist && comparison.similarityScore < 0.7 && comparison.duplicateType === "none") continue;
      candidates.push({
        row,
        comparison,
  ...
  private rowToRecord(row: StoredFingerprintRow): AudioFingerprintRecord {
  ...
  private async loadFingerprintRows(releaseId: string | null): Promise<readonly StoredFingerprintRow[]> {
    const whereClause = releaseId ? "WHERE s.release_id = :releaseId::uuid" : "";
    return this.deps.sql.query<StoredFingerprintRow>(
      `SELECT
  ...
  private async loadDuplicateRows(releaseId: string | null): Promise<readonly AudioFingerprintDuplicateMatch[]> {
    const whereClause = releaseId ? "WHERE dm.release_id = :releaseId::uuid" : "";
    const rows = await this.deps.sql.query<StoredDuplicateRow>(
      `SELECT
         dm.id AS match_id,
         dm.fingerprint_id,
         dm.release_id,
    ...
    return rows.map((row) => freeze({
      ...
      duplicateType: row.duplicate_type as DuplicateMatchKind,
  ...
  private async loadSimilarityRows(releaseId: string | null): Promise<readonly AudioFingerprintSimilarityScore[]> {
    const whereClause = releaseId ? "WHERE ss.release_id = :releaseId::uuid" : "";
    const rows = await this.deps.sql.query<StoredSimilarityRow>(
      `SELECT
  ...
  private async loadFraudRows(releaseId: string | null): Promise<readonly AudioFraudSignal[]> {
    const whereClause = releaseId ? "WHERE aa.release_id = :releaseId::uuid" : "";
    const rows = await this.deps.sql.query<StoredFraudRow>(
      `SELECT
  ...
  private async loadFingerprintRecord(fingerprintId: string): Promise<AudioFingerprintRecord | null> {
    const rows = await this.loadFingerprintRows(null);
