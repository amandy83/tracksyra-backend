...
import { UniversalSerializer } from "../../metadata/metadataSerializer";
import { MetadataAudit } from "../../metadata/metadataAudit";
import { createMetadataSnapshot } from "../../metadata/metadataSnapshot";
import type { UniversalMetadataDiff, UniversalRelease, UniversalValidationResult } from "../../metadata";
...
import type { EnterpriseRightsService } from "../../rights";
import type { DdexFoundationService } from "../../ddex";
import type { ReleaseDeliveryEngine } from "../../core/releaseDeliveryEngine";
import type { AudioFingerprintingEngine } from "../fingerprinting";
import { listMetadataDspProfiles, METADATA_DSP_PROFILES } from "./metadataProfiles";
import type {
  MetadataAssistantSuggestion,
  MetadataAuditEntry,
  MetadataCompatibilityReport,
  MetadataCompatibilityScore,
  MetadataConflict,
  MetadataConflictKind,
  MetadataDspName,
  MetadataDspProfile,
  MetadataIntelligenceEngineInput,
  MetadataIntelligenceNormalizationInput,
  MetadataIntelligenceRepairInput,
  MetadataIntelligenceReport,
  MetadataNormalizationResult,
  MetadataPredictiveIssue,
  MetadataProfileRuleSet,
  MetadataQualityReport,
  MetadataQualityScore,
  MetadataRecommendation,
  MetadataRecommendationKind,
  MetadataRepairAction,
  MetadataRepairResult,
  MetadataRightsReport,
  MetadataValidationReport,
  MetadataPublishingReport,
  ReleaseReadinessReport,
  MetadataVersionRecord,
} from "./metadataIntelligenceTypes";
import { QueueDispatcher } from "../../../queue/queueDispatcher";
import { incrementMetric, recordRetry, setMetric, setWorkerHealth } from "../../../queue/metrics";
import { queueNames } from "../../../queue/queueNames";
import { logger as defaultLogger, serializeError, type Logger } from "../../../observability/logger";

type MetadataIntelligenceDependencies = Readonly<{
  sql: SqlExecutor;
  ...
  enterpriseRightsService: EnterpriseRightsService;
  metadataTransformer: MetadataTransformer;
  metadataValidator: MetadataValidator;
  metadataComparator: MetadataComparator;
  metadataHasher: MetadataHasher;
  ddexFoundationService: DdexFoundationService;
  audioFingerprintingEngine?: AudioFingerprintingEngine | null;
...
type PreparedMetadata = Readonly<{
  release: DistributionRelease;
  track: DistributionTrack | null;
...
type RepairContext = Readonly<{
  field: string;
  profile: MetadataDspProfile | null;
}>;

type StoredVersionRow = Readonly<{
  version_id: string;
  release_id: string;
...
type StoredRecommendationRow = Readonly<{
...
type StoredQualityRow = Readonly<{
  quality_id: string;
  release_id: string;
  track_id: string | null;
  metadata_quality_score: number;
  metadata_confidence_score: number;
  metadata_completeness_score: number;
  dsp_compatibility_score: number;
  publishing_score: number;
  rights_score: number;
  delivery_score: number;
  artwork_score: number;
  overall_release_score: number;
  metadata: unknown;
  created_at: string;
}>;
...
function normalizeRecord(value: unknown): Readonly<Record<string, unknown>> {
  return value && typeof value === "object" ? freeze(value as Record<string, unknown>) : freeze({});
}
...
function dedupeText(values: readonly unknown[]): readonly string[] {
...
function cleanText(value: unknown, options: { titleCase?: boolean; allowEmoji?: boolean; preserveCase?: boolean } = {}): string | null {
  const text = asString(value);
  if (!text) return null;
  let normalized = normalizeUnicode(text).replace(/\s+/g, " ").trim();
  normalized = normalized.replace(/[<>]/g, "");
  if (!options.allowEmoji) normalized = normalized.replace(/\p{Extended_Pictographic}/gu, "");
  if (!options.preserveCase) normalized = Array.from(normalized).filter((character) => {
    const code = character.charCodeAt(0);
    return code >= 32 && code !== 127;
  }).join("");
  if (options.titleCase) normalized = toTitleCase(normalized);
...
function isValidIsrc(value: string | null): boolean {
  return Boolean(value && /^[A-Z]{2}[A-Z0-9]{3}\d{2}\d{5}$/.test(value));
}

function repairIsrc(value: unknown): string | null {
  const normalized = normalizeIdentifier(value);
  if (!normalized) return null;
  const compact = normalized.replace(/[^A-Z0-9]/g, "");
  if (/^[A-Z]{2}[A-Z0-9]{3}\d{2}\d{5}$/.test(compact)) return compact;
  if (compact.length >= 12) return compact.slice(0, 12);
  return null;
}

function isValidUpc(value: string | null): boolean {
  if (!value || !/^\d{12}$/.test(value)) return false;
  return computeUpcCheckDigit(value.slice(0, 11)) === value[11];
}

function repairUpc(value: unknown): string | null {
...
function isValidIswc(value: string | null): boolean {
  return Boolean(value && /^T-\d{3}\.\d{3}\.\d{3}-\d$/.test(value));
}

function repairIswc(value: unknown): string | null {
  const normalized = normalizeIdentifier(value);
  if (!normalized) return null;
...
function isValidIpi(value: string | null): boolean {
  return Boolean(value && /^\d{9,11}$/.test(value));
}

function repairIpi(value: unknown): string | null {
  const normalized = normalizeIdentifier(value)?.replace(/\D/g, "");
  if (!normalized) return null;
  return normalized.slice(0, 11);
}

function isValidIsni(value: string | null): boolean {
  return Boolean(value && /^\d{15}[\dX]$/.test(value));
}

function repairIsni(value: unknown): string | null {
  const normalized = normalizeIdentifier(value)?.replace(/[^0-9X]/g, "");
  if (!normalized) return null;
  return normalized.slice(0, 16);
}

function detectDuplicateName(values: readonly string[]): readonly string[] {
  const seen = new Set<string>();
...
function deriveMood(genre: string | null, explicit: boolean): readonly string[] {
  const normalized = (genre ?? "").toLowerCase();
  if (explicit) return freeze(["Intense", "Aggressive"]);
  if (/chill|ambient|lofi/.test(normalized)) return freeze(["Chill", "Relaxed"]);
  if (/dance|club|electronic|house|techno/.test(normalized)) return freeze(["Energetic", "Upbeat"]);
  if (/rock|metal|punk/.test(normalized)) return freeze(["Intense", "Aggressive"]);
  if (/pop|rnb|soul/.test(normalized)) return freeze(["Romantic", "Upbeat"]);
  return freeze(["Balanced"]);
}

function cleanMetadataRecord(record: Readonly<Record<string, unknown>>, profile: MetadataDspProfile | null): Readonly<Record<string, unknown>> {
  ...
  for (const [key, value] of Object.entries(record)) {
    ...
    if (Array.isArray(value)) {
      output[key] = freeze(
        dedupeText(value).map((entry) => cleanText(entry, { titleCase: false, allowEmoji: profile?.allowEmoji ?? false }) ?? entry),
      );
      continue;
    }
    output[key] = value;
...
function mergeRecord<T extends Record<string, unknown>>(base: T, metadata: Readonly<Record<string, unknown>>): T {
  return Object.freeze({ ...(base as Record<string, unknown>), ...metadata }) as T;
}

function buildRepairAction(field: string, before: unknown, after: unknown, reason: string, confidence: number): MetadataRepairAction {
...
export class MetadataIntelligenceEngine {
  ...
  private readonly serializer = new UniversalSerializer();
  private readonly metadataAudit: MetadataAudit;

  constructor(private readonly deps: MetadataIntelligenceDependencies) {
  ...
  getProfiles(): readonly MetadataDspProfile[] {
    return listMetadataDspProfiles();
  }

  getProfile(platform: MetadataDspName): MetadataDspProfile {
    return METADATA_DSP_PROFILES[platform];
  }

  async suggestMetadata(input: MetadataIntelligenceEngineInput): Promise<MetadataAssistantSuggestion> {
    const source = await this.resolveSource(input as MetadataIntelligenceRepairInput);
  ...
  async validateMetadata(input: MetadataIntelligenceNormalizationInput): Promise<MetadataValidationReport> {
  ...
  async normalizeMetadata(input: MetadataIntelligenceNormalizationInput): Promise<MetadataNormalizationResult> {
    return this.normalizeInternal(input, { persistAction: "normalize" });
  }

  async repairMetadata(input: MetadataIntelligenceNormalizationInput): Promise<MetadataRepairResult> {
  ...
  async compareMetadata(before: UniversalRelease, after: UniversalRelease): Promise<UniversalMetadataDiff> {
    return this.deps.metadataComparator.compare(before, after);
  }

  async recommendMetadata(input: MetadataIntelligenceNormalizationInput): Promise<readonly MetadataRecommendation[]> {
  ...
  async predictDSPCompatibility(input: MetadataIntelligenceNormalizationInput): Promise<MetadataCompatibilityReport> {
    const prepared = await this.prepare(input);
  ...
  async generateMetadataReport(input: MetadataIntelligenceNormalizationInput): Promise<MetadataIntelligenceReport> {
    const prepared = await this.prepare(input);
  ...
  async generateCompatibilityReport(input: MetadataIntelligenceNormalizationInput): Promise<MetadataCompatibilityReport> {
    return this.predictDSPCompatibility(input);
  }

  async generateQualityReport(input: MetadataIntelligenceNormalizationInput): Promise<MetadataQualityReport> {
    const prepared = await this.prepare(input);
    const compatibility = this.computeCompatibility(prepared.release, prepared.track, prepared.universalRelease);
    const quality = this.computeQuality(prepared.release, prepared.track, prepared.universalRelease, prepared.validation, prepared.recommendations, prepared.conflicts, compatibility);
  ...
  async generateReleaseReadiness(input: MetadataIntelligenceNormalizationInput): Promise<ReleaseReadinessReport> {
  ...
  async generateIdentifierReport(input: MetadataIntelligenceNormalizationInput): Promise<Readonly<Record<string, unknown>>> {
    const prepared = await this.prepare(input);
  ...
  async generatePublishingReport(input: MetadataIntelligenceNormalizationInput): Promise<MetadataPublishingReport> {
    const prepared = await this.prepare(input);
  ...
  async generateRightsReport(input: MetadataIntelligenceNormalizationInput): Promise<MetadataRightsReport> {
  ...
  async generateDashboard(input: MetadataIntelligenceEngineInput & { kind?: "metadata" | "compatibility" | "quality" | "identifier" | "release-readiness" | "recommendation"; }) {
    switch (input.kind ?? "metadata") {
      case "compatibility":
        return this.generateCompatibilityReport(input as MetadataIntelligenceNormalizationInput);
      case "quality":
        return this.generateQualityReport(input as MetadataIntelligenceNormalizationInput);
      case "identifier":
        return this.generateIdentifierReport(input as MetadataIntelligenceNormalizationInput);
      case "release-readiness":
        return this.generateReleaseReadiness(input as MetadataIntelligenceNormalizationInput);
      case "recommendation":
        return this.recommendMetadata(input as MetadataIntelligenceNormalizationInput);
      default:
        return this.generateMetadataReport(input as MetadataIntelligenceNormalizationInput);
    }
  }

  async predictDSPIssues(input: MetadataIntelligenceNormalizationInput): Promise<readonly MetadataPredictiveIssue[]> {
    const compatibility = await this.predictDSPCompatibility(input);
    return freeze(
      compatibility.scores.flatMap((score) =>
  ...
  async retry(input: MetadataIntelligenceEngineInput & { error?: unknown; attempt?: number | null; }): Promise<MetadataIntelligenceReport> {
    recordRetry(queueNames.metadataRetry);
    const result = await this.generateMetadataReport(input as MetadataIntelligenceNormalizationInput);
    if (input.error) {
  ...
  async healthCheck(): Promise<Readonly<Record<string, unknown>>> {
    const rows = await this.deps.sql.query<{ count: number }>("SELECT COUNT(*)::int AS count FROM public.metadata_versions");
  ...
  async audit(input: MetadataIntelligenceEngineInput): Promise<readonly MetadataAuditEntry[]> {
    const rows = await this.loadAuditRows(input.releaseId, input.trackId ?? null);
    return rows;
  }

  private async prepare(input: MetadataIntelligenceNormalizationInput): Promise<PreparedMetadata> {
    const repaired = await this.repairInternal(input);
  ...
  private async normalizeInternal(input: MetadataIntelligenceNormalizationInput, options: { persistAction: "validate" | "normalize"; }): Promise<MetadataNormalizationResult> {
    const repaired = await this.repairInternal(input);
  ...
  private async repairInternal(input: MetadataIntelligenceNormalizationInput): Promise<MetadataRepairResult & Readonly<{ recommendations: readonly MetadataRecommendation[]; conflicts: readonly MetadataConflict[]; generatedAt: string; }>> {
    ...
    const repairs: MetadataRepairAction[] = [];
    ...
    if (repairedRelease.title !== source.release.title) repairs.push(buildRepairAction("release.title", source.release.title, repairedRelease.title, "Normalized capitalization and spacing", 0.95));
    if (repairedRelease.primaryArtist !== source.release.primaryArtist) repairs.push(buildRepairAction("release.primaryArtist", source.release.primaryArtist, repairedRelease.primaryArtist, "Normalized artist capitalization and spacing", 0.95));
    if (repairedRelease.upc !== source.release.upc) repairs.push(buildRepairAction("release.upc", source.release.upc, repairedRelease.upc, "Repaired UPC", 0.99));
    if (repairedTrack && source.track) {
      if (repairedTrack.title !== source.track.title) repairs.push(buildRepairAction("track.title", source.track.title, repairedTrack.title, "Normalized track title", 0.95));
      if (repairedTrack.isrc !== source.track.isrc) repairs.push(buildRepairAction("track.isrc", source.track.isrc, repairedTrack.isrc, "Repaired ISRC", 0.99));
    }
  ...
  private repairIdentifiers(metadata: Readonly<Record<string, unknown>>, repairs: MetadataRepairAction[]): Readonly<Record<string, unknown>> {
    const output: Record<string, unknown> = { ...metadata };
    const isrc = repairIsrc(metadata.isrc);
    if (isrc && isrc !== metadata.isrc) repairs.push(buildRepairAction("metadata.isrc", metadata.isrc, isrc, "Repaired ISRC", 0.99));
    if (isrc) output.isrc = isrc;
    const upc = repairUpc(metadata.upc);
    if (upc && upc !== metadata.upc) repairs.push(buildRepairAction("metadata.upc", metadata.upc, upc, "Repaired UPC", 0.99));
    if (upc) output.upc = upc;
    const iswc = repairIswc(metadata.iswc);
    if (iswc && iswc !== metadata.iswc) repairs.push(buildRepairAction("metadata.iswc", metadata.iswc, iswc, "Repaired ISWC", 0.9));
    if (iswc) output.iswc = iswc;
    const ipi = repairIpi(metadata.ipi);
    if (ipi && ipi !== metadata.ipi) repairs.push(buildRepairAction("metadata.ipi", metadata.ipi, ipi, "Repaired IPI", 0.9));
    if (ipi) output.ipi = ipi;
    const isni = repairIsni(metadata.isni);
    if (isni && isni !== metadata.isni) repairs.push(buildRepairAction("metadata.isni", metadata.isni, isni, "Repaired ISNI", 0.9));
    if (isni) output.isni = isni;
    return freeze(output);
  }

  private async detectConflicts(release: DistributionRelease, track: DistributionTrack | null, universalRelease: UniversalRelease): Promise<readonly MetadataConflict[]> {
    ...
    const releaseRows = await this.deps.sql.query<Readonly<Record<string, unknown>>>(
      `SELECT id, title, primary_artist, upc
       ...
       WHERE id <> :releaseId::uuid
         ...
         OR (NULLIF(upc, '') = NULLIF(:upc, ''))`,
      { releaseId: release.id, title, artist, upc: release.upc ?? null },
    ).catch(() => []);
    ...
    if (track?.isrc) {
      const trackRows = await this.deps.sql.query<Readonly<Record<string, unknown>>>(
        `SELECT id, release_id, title, primary_artist, isrc
         FROM public.tracks
         WHERE release_id <> :releaseId::uuid AND NULLIF(UPPER(TRIM(isrc)), '') = NULLIF(UPPER(TRIM(:isrc)), '')`,
        { releaseId: release.id, isrc: track.isrc },
      ).catch(() => []);
  ...
  private buildSuggestions(release: DistributionRelease, track: DistributionTrack | null, universalRelease: UniversalRelease): { genres: readonly string[]; mood: readonly string[]; keywords: readonly string[]; issues: readonly string[] } {
    const genres = new Set<string>();
    const keywords = new Set<string>();
    const issues = new Set<string>();
    const genre = asString(release.genre ?? track?.genre ?? universalRelease.genre.primary);
    if (genre) genres.add(this.mapGenre(genre, this.profileForRelease(release)) ?? genre);
    if (!release.genre && !track?.genre) issues.add("Missing genre metadata.");
    if (!release.labelName && !track?.publisher) issues.add("Missing label or publisher metadata.");
    if (!release.coverArtUrl) issues.add("Missing artwork.");
    if (!track?.lyrics) issues.add("Missing lyrics.");
    if (!release.upc) issues.add("Missing UPC.");
    if (!track?.isrc) issues.add("Missing ISRC.");
  ...
  private buildRecommendations(release: DistributionRelease, track: DistributionTrack | null, universalRelease: UniversalRelease, validation: UniversalValidationResult, conflicts: readonly MetadataConflict[]): readonly MetadataRecommendation[] {
  ...
  private recommendation(kind: MetadataRecommendationKind, platform: MetadataDspName | null, message: string, severity: "info" | "warning" | "error", confidence: number, metadata: Readonly<Record<string, unknown>>): MetadataRecommendation {
    return freeze({
      kind,
      platform,
      field: kind,
  ...
  private computeCompatibility(release: DistributionRelease, track: DistributionTrack | null, universalRelease: UniversalRelease): readonly MetadataCompatibilityScore[] {
    ...
    return freeze(listMetadataDspProfiles().map((entry) => {
      ...
      if (!title) issues.push("Missing release title");
      if (title && title.length > entry.maxTitleLength) issues.push("Title exceeds platform length");
      if (track?.title && track.title.length > entry.maxTitleLength) issues.push("Track title exceeds platform length");
      if (!release.primaryArtist) issues.push("Missing primary artist");
      if (release.genre && !entry.genreMap[(release.genre ?? "").toLowerCase()]) warnings.push("Genre may need canonical mapping");
      if (release.language && !entry.supportedLanguages.includes(release.language.toLowerCase())) warnings.push("Language not listed in profile support");
      if (release.upc && !isValidUpc(repairUpc(release.upc))) issues.push("UPC invalid");
      if (track?.isrc && !isValidIsrc(repairIsrc(track.isrc))) issues.push("ISRC invalid");
      if (!release.coverArtUrl) warnings.push("Artwork missing");
      if (!track?.lyrics && universalRelease.kind !== "instrumental" && entry.platform !== "pandora") warnings.push("Lyrics missing");
      const baseScore = 100
        - issues.length * 18
        - warnings.length * 6
  ...
  private computeQuality(release: DistributionRelease, track: DistributionTrack | null, universalRelease: UniversalRelease, validation: UniversalValidationResult, recommendations: readonly MetadataRecommendation[], conflicts: readonly MetadataConflict[], compatibility: readonly MetadataCompatibilityScore[]): MetadataQualityScore {
  ...
  private async persistNormalization(input: MetadataIntelligenceNormalizationInput, repaired: MetadataRepairResult & Readonly<{ recommendations: readonly MetadataRecommendation[]; conflicts: readonly MetadataConflict[]; generatedAt: string; }>, quality: MetadataQualityScore, compatibility: readonly MetadataCompatibilityScore[], metadata: Readonly<Record<string, unknown>>, action: string): Promise<void> {
    await this.deps.sql.query(
      `INSERT INTO public.metadata_normalization (
  ...
  private async persistQuality(input: MetadataIntelligenceNormalizationInput, quality: MetadataQualityScore, compatibility: readonly MetadataCompatibilityScore[], repaired: MetadataRepairResult & Readonly<{ recommendations: readonly MetadataRecommendation[]; conflicts: readonly MetadataConflict[]; generatedAt: string; }>): Promise<void> {
    const payload = {
      releaseId: input.releaseId,
      trackId: input.trackId ?? repaired.track?.id ?? null,
    ...
    await this.deps.sql.query(
      `INSERT INTO public.metadata_quality (
  ...
  private async persistVersion(action: string, input: MetadataIntelligenceNormalizationInput, repaired: MetadataRepairResult & Readonly<{ recommendations: readonly MetadataRecommendation[]; conflicts: readonly MetadataConflict[]; generatedAt: string; }>, quality: MetadataQualityScore, compatibility: readonly MetadataCompatibilityScore[]): Promise<MetadataVersionRecord> {
    ...
    await this.deps.sql.query(
      `INSERT INTO public.metadata_history (
  ...
  private async persistRecommendations(input: MetadataIntelligenceNormalizationInput, recommendations: readonly MetadataRecommendation[], version: MetadataVersionRecord): Promise<void> {
    for (const recommendation of recommendations) {
      await this.deps.sql.query(
        `INSERT INTO public.metadata_recommendations (
  ...
  private async persistConflicts(input: MetadataIntelligenceNormalizationInput, conflicts: readonly MetadataConflict[], version: MetadataVersionRecord): Promise<void> {
    for (const conflict of conflicts) {
      await this.deps.sql.query(
        `INSERT INTO public.metadata_conflicts (
  ...
  private async persistAudit(input: MetadataIntelligenceNormalizationInput, version: MetadataVersionRecord, action: string, metadata: Readonly<Record<string, unknown>>, validation: UniversalValidationResult): Promise<void> {
    await this.deps.sql.query(
      `INSERT INTO public.metadata_audit (
  ...
  private async persistRetry(input: MetadataIntelligenceEngineInput, error: unknown): Promise<void> {
    await this.deps.sql.query(
      `INSERT INTO public.metadata_repairs (
  ...
  private async loadHistory(releaseId: string, trackId: string | null): Promise<readonly MetadataVersionRecord[]> {
    const rows = await this.deps.sql.query<StoredVersionRow>(
      `SELECT version_id, release_id, track_id, fingerprint, created_at::text AS created_at, metadata, diff, action, actor, correlation_id
    ...
    return freeze(rows.map((row) => freeze({
      ...
      metadata: typeof row.metadata === "string" ? JSON.parse(row.metadata) as UniversalRelease : row.metadata as UniversalRelease,
      diff: typeof row.diff === "string" ? JSON.parse(row.diff) as UniversalMetadataDiff : row.diff as UniversalMetadataDiff | null,
    })));
  }

  private async loadLatestVersion(releaseId: string, trackId: string | null): Promise<MetadataVersionRecord | null> {
    const rows = await this.deps.sql.query<StoredVersionRow>(
      `SELECT version_id, release_id, track_id, fingerprint, created_at::text AS created_at, metadata, diff, action, actor, correlation_id
       FROM public.metadata_versions
       WHERE release_id = :releaseId::uuid
    ...
    const row = rows[0];
    if (!row) return null;
    const parsed = typeof row.metadata === "string" ? JSON.parse(row.metadata) as UniversalRelease : row.metadata as UniversalRelease;
    return freeze({
      versionId: row.version_id,
      releaseId: row.release_id,
      trackId: row.track_id,
      ...
      createdAt: row.created_at,
      metadata: parsed,
      diff: row.diff ? (row.diff as UniversalMetadataDiff) : null,
    });
  }

  private async loadAuditRows(releaseId: string, trackId: string | null): Promise<readonly MetadataAuditEntry[]> {
    const rows = await this.deps.sql.query<Readonly<Record<string, unknown>>>(
      `SELECT audit_id, release_id, track_id, version_id, action, actor, correlation_id, created_at::text AS created_at, metadata
  ...
  private async resolveSource(input: MetadataIntelligenceRepairInput): Promise<MetadataSource> {
    if (input.release) {
      if (input.track) {
  ...
  private profileForRelease(release: DistributionRelease): MetadataDspProfile {
    const platform = asString(release.metadata?.targetPlatform) as MetadataDspName | null;
    return platform && platform in METADATA_DSP_PROFILES ? METADATA_DSP_PROFILES[platform] : METADATA_DSP_PROFILES.spotify;
  }

  private mapGenre(value: string | null | undefined, profile: MetadataDspProfile | null): string | null {
    const text = asString(value);
    if (!text) return null;
  ...
  private mapLanguage(value: string | null | undefined, profile: MetadataDspProfile | null): string | null {
    const text = asString(value);
    if (!text) return null;
