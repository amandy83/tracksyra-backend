...
export type TidalConnectorConfiguration = Readonly<{
  apiVersion: string;
...
export type TidalConnectorDependencies = Readonly<{
  ...
  configuration: TidalConnectorConfiguration;
...
type MutableTidalConnector = TidalConnector & {
...
type TidalEnterpriseDependencies = Readonly<{
  connector: TidalConnector;
  logger?: Logger | null;
  clock?: () => string;
}>;

type TidalAuditRecord = Readonly<{
  auditId: string;
  reportType: string;
  recordedAt: string;
...
function buildContext(job: DSPDeliveryJob, packageModel: DSPDeliveryPackage, connector: MutableTidalConnector): ConnectorContext {
...
function buildTidalMetadata(job: DSPDeliveryJob) {
  const release = job.release;
  const metadata = release ? releaseMetadata(release) : {};
...
export class TidalAuthentication {
  constructor(private readonly dependencies: TidalEnterpriseDependencies) {}

  async authenticate(job: DSPDeliveryJob) {
    const packageModel = await this.dependencies.connector.buildPackage(job);
    const context = buildContext(job, packageModel, this.dependencies.connector as MutableTidalConnector);
...
export class TidalWebhookService {
  ...
  async handle(event: DSPWebhookEvent): Promise<{ valid: boolean; event: DSPWebhookEvent; audit: TidalAuditRecord | null }> {
...
export class TidalCapabilityResolver {
  ...
  resolve(connectorId = "Tidal"): DSPCapabilities {
    void connectorId;
    return (this.dependencies.connector as MutableTidalConnector).capabilities;
  }
}
...
export class TidalErrorTranslator {
  ...
  translate(error: unknown, job: Pick<DSPDeliveryJob, "jobId" | "releaseId" | "target">): ConnectorError {
    ...
    const code =
      /auth|unauthoriz|forbidden|token/i.test(message) ? "TIDAL_MUSIC_AUTH_FAILED"
...
export class TidalDeliveryAudit {
  ...
  record(reportType: string, payload: Readonly<Record<string, unknown>>): TidalAuditRecord {
  ...
  recordWebhook(event: DSPWebhookEvent, valid: boolean): TidalAuditRecord {
  ...
  buildCapabilityReport(connectorId: string): DSPConnectorCapabilityReport {
    const capabilities = (this.dependencies.connector as MutableTidalConnector).capabilities;
    return Object.freeze({
      connectorId,
      generatedAt: nowIso(this.dependencies.clock),
      capabilities,
...
export class TidalEnterpriseService {
  readonly authentication: TidalAuthentication;
  readonly packageBuilder: TidalPackageBuilder;
  readonly metadataNormalizer: TidalMetadataNormalizer;
  readonly artworkNormalizer: TidalArtworkNormalizer;
  readonly audioNormalizer: TidalAudioNormalizer;
  readonly deliveryService: TidalDeliveryService;
  readonly pollingService: TidalPollingService;
  readonly webhookService: TidalWebhookService;
  readonly withdrawalService: TidalWithdrawalService;
  readonly restoreService: TidalRestoreService;
  readonly retryPolicy: TidalRetryPolicy;
  readonly healthChecker: TidalHealthCheck;
  readonly capabilityResolver: TidalCapabilityResolver;
  readonly errorTranslator: TidalErrorTranslator;
  readonly deliveryAudit: TidalDeliveryAudit;
