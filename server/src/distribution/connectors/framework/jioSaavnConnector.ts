...
export type JioSaavnConnectorConfiguration = Readonly<{
  apiVersion: string;
...
export type JioSaavnConnectorDependencies = Readonly<{
  ...
  configuration: JioSaavnConnectorConfiguration;
...
type MutableJioSaavnConnector = JioSaavnConnector & {
...
type JioSaavnEnterpriseDependencies = Readonly<{
  connector: JioSaavnConnector;
  logger?: Logger | null;
  clock?: () => string;
}>;

type JioSaavnAuditRecord = Readonly<{
  auditId: string;
  reportType: string;
  recordedAt: string;
...
function buildContext(job: DSPDeliveryJob, packageModel: DSPDeliveryPackage, connector: MutableJioSaavnConnector): ConnectorContext {
...
function buildJioSaavnMetadata(job: DSPDeliveryJob) {
  const release = job.release;
  const metadata = release ? releaseMetadata(release) : {};
...
export class JioSaavnAuthentication {
  constructor(private readonly dependencies: JioSaavnEnterpriseDependencies) {}

  async authenticate(job: DSPDeliveryJob) {
    const packageModel = await this.dependencies.connector.buildPackage(job);
    const context = buildContext(job, packageModel, this.dependencies.connector as MutableJioSaavnConnector);
...
export class JioSaavnWebhookService {
  ...
  async handle(event: DSPWebhookEvent): Promise<{ valid: boolean; event: DSPWebhookEvent; audit: JioSaavnAuditRecord | null }> {
...
export class JioSaavnCapabilityResolver {
  ...
  resolve(connectorId = "JioSaavn"): DSPCapabilities {
    void connectorId;
    return (this.dependencies.connector as MutableJioSaavnConnector).capabilities;
  }
}
...
export class JioSaavnErrorTranslator {
  ...
  translate(error: unknown, job: Pick<DSPDeliveryJob, "jobId" | "releaseId" | "target">): ConnectorError {
    ...
    const code =
      /auth|unauthoriz|forbidden|token/i.test(message) ? "JIOSAAVN_MUSIC_AUTH_FAILED"
...
export class JioSaavnDeliveryAudit {
  ...
  record(reportType: string, payload: Readonly<Record<string, unknown>>): JioSaavnAuditRecord {
  ...
  recordWebhook(event: DSPWebhookEvent, valid: boolean): JioSaavnAuditRecord {
  ...
  buildCapabilityReport(connectorId: string): DSPConnectorCapabilityReport {
    const capabilities = (this.dependencies.connector as MutableJioSaavnConnector).capabilities;
    return Object.freeze({
      connectorId,
      generatedAt: nowIso(this.dependencies.clock),
      capabilities,
...
export class JioSaavnEnterpriseService {
  readonly authentication: JioSaavnAuthentication;
  readonly packageBuilder: JioSaavnPackageBuilder;
  readonly metadataNormalizer: JioSaavnMetadataNormalizer;
  readonly artworkNormalizer: JioSaavnArtworkNormalizer;
  readonly audioNormalizer: JioSaavnAudioNormalizer;
  readonly deliveryService: JioSaavnDeliveryService;
  readonly pollingService: JioSaavnPollingService;
  readonly webhookService: JioSaavnWebhookService;
  readonly withdrawalService: JioSaavnWithdrawalService;
  readonly restoreService: JioSaavnRestoreService;
  readonly retryPolicy: JioSaavnRetryPolicy;
  readonly healthChecker: JioSaavnHealthCheck;
  readonly capabilityResolver: JioSaavnCapabilityResolver;
  readonly errorTranslator: JioSaavnErrorTranslator;
  readonly deliveryAudit: JioSaavnDeliveryAudit;
