...
export type { TooLostConfig, TooLostOAuthToken, TooLostProviderHealth } from "./tooLostTypes";
...
export function readTooLostConfig(overrides: Partial<TooLostConfig> = {}): TooLostConfig {
  ...
  const apiUrl = trimTrailingSlash(
    overrides.apiUrl ?? (readEnvOneOf(["TOO_LOST_API_URL", "TOO_LOST_API_BASE_URL"]) || DEFAULT_TOO_LOST_API_URL),
  );
  return {
...
export async function exchangeTooLostOAuthCode(input: {
  ...
  const body = await response.json() as TooLostTokenResponse;
...
type TooLostTokenResponse = Readonly<{
  access_token?: string;
