...
import { DistributionIntelligenceStore, mapProviderStatus } from "../../intelligence";
...
import type { DistributionAudioUrlResolver } from "../../services/distributionMediaResolver";
...
type TooLostAccountProfile = Readonly<{
  id?: string | null;
  name?: string | null;
...
type TooLostJsonRecord = Readonly<Record<string, unknown>> & {
  data?: TooLostJsonRecord | null;
  release?: TooLostJsonRecord | null;
  item?: TooLostJsonRecord | null;
  tracks?: ReadonlyArray<unknown> | null;
  items?: ReadonlyArray<unknown> | null;
  deliveries?: ReadonlyArray<unknown> | null;
  platformDeliveries?: ReadonlyArray<unknown> | null;
...
export class TooLostIntegrationService {
  ...
  private readonly intelligence: DistributionIntelligenceStore;
  private readonly ffmpeg: FfmpegRunner;
  private readonly resolveAudioUrl: DistributionAudioUrlResolver;
  private readonly log = logger.child({ component: "too-lost-integration" });
  ...
  async handleOAuthCallback(input: { code: string; state: string }): Promise<{ connection: TooLostConnectionStatus; redirectTo: string | null }> {
    ...
    let profile: TooLostAccountProfile | null;
    try {
  ...
  async updateRelease(releaseId: string): Promise<{
    ...
    const payload = buildReleasePayload({
      ...
      contributors: await this.db.query<{
        track_id: string | null;
        name: string;
        role: string;
      }>(
  ...
  async syncReleaseByLocalReleaseId(releaseId: string): Promise<{
    release: TooLostReleaseStatusSnapshot;
    updated: boolean;
    reason: string;
  }> {
  ...
  async syncReleaseByProviderReleaseId(
    providerReleaseId: string,
    input: { localReleaseId?: string | null; source: "MANUAL_REFRESH" | "WEBHOOK" },
  ): Promise<{
    release: TooLostReleaseStatusSnapshot;
    updated: boolean;
    reason: string;
  ...
  async listLookup(resource: "genres" | "languages" | "countries" | "platforms"): Promise<unknown> {
    return this.api.requestJson(`/lookup/${resource}`, { method: "GET" });
  }

  async getPreferences(resource: "artist" | "label" | "spotify" | "apple" | "youtube" | "search"): Promise<unknown> {
    return this.api.requestJson(`/preferences/${resource}`, { method: "GET" });
  }

  async getSales(resource: "overview" | "releases" | "tracks" | "artists" | "channels" | "territories" | "stream-rates", query: Record<string, string | number | boolean | null | undefined> = {}): Promise<unknown> {
    return this.api.requestJson(`/sales/${resource}${buildQuery(query)}`, { method: "GET" });
  }

  async getAnalytics(resource: "overview" | "tracks" | "platforms" | "charts" | "audience" | "release-links" | "usage-discovery", query: Record<string, string | number | boolean | null | undefined> = {}): Promise<unknown> {
  ...
  async createReleaseProxy(body: unknown): Promise<unknown> {
  ...
  async patchReleaseMetadataProxy(releaseId: string, body: unknown): Promise<unknown> {
  ...
  async uploadTrackUploadUrl(releaseId: string, body: unknown): Promise<unknown> {
    return this.api.requestJson(
      `/releases/${encodeURIComponent(releaseId)}/tracks/upload-url`,
      { method: "POST", body: JSON.stringify(normalizeTrackUploadRequest(body)) },
    );
  }

  async putReleaseTrackList(releaseId: string, body: unknown): Promise<unknown> {
  ...
  async patchTrackMetadataProxy(releaseId: string, trackId: string, body: unknown): Promise<unknown> {
  ...
  async patchTrackFileProxy(releaseId: string, trackId: string, body: unknown): Promise<unknown> {
  ...
  async patchReleaseDelivery(releaseId: string, body: unknown): Promise<unknown> {
    return this.api.requestJson(`/releases/${encodeURIComponent(releaseId)}/delivery`, { method: "PATCH", body: JSON.stringify(body) });
  }

  async validateReleaseProxy(releaseId: string, body: unknown = {}): Promise<unknown> {
    void body;
    const release = await this.store.getReleaseWithTracks(releaseId);
    if (!release) {
      throw httpError(404, "RELEASE_NOT_FOUND", `Release ${releaseId} was not found.`);
  ...
  async uploadArtworkProxy(releaseId: string, body: unknown): Promise<unknown> {
    return this.patchReleaseMetadataProxy(releaseId, normalizeArtworkMetadata(body));
  }

  private async updateReleaseMetadata(releaseId: string, payload: TooLostReleasePayload): Promise<unknown> {
  ...
  private async uploadReleaseArtwork(releaseId: string, payload: TooLostReleasePayload): Promise<unknown> {
    if (!payload.release.coverUrl) return null;
    return this.patchReleaseMetadataProxy(releaseId, { coverUrl: payload.release.coverUrl });
  }

  private async uploadReleaseTracks(releaseId: string, payload: TooLostReleasePayload): Promise<{ tracks: Array<Record<string, unknown>> }> {
  ...
  private async persistReleaseSubmissionState(input: {
    releaseId: string;
    providerReleaseId: string | null;
    status: string;
    distributionStatus: string;
  ...
  private async readLocalReleaseSyncContext(input: { releaseId?: string | null; providerReleaseId?: string | null }): Promise<{
    id: string;
    provider_release_id: string | null;
    provider_status: string | null;
    distribution_status: string | null;
    ...
    const rows = await this.db.query<{
      ...
      `SELECT id, provider_release_id, provider_status, distribution_status, status, last_synced_at, provider_updated_at
       FROM releases
       ...
       LIMIT 1`,
      { releaseId: input.releaseId ?? null, providerReleaseId: input.providerReleaseId ?? null },
    );
    return rows[0] ?? null;
  }

  private async readLocalTrackSyncTargets(releaseId: string): Promise<Array<{ id: string; provider_track_id: string | null }>> {
    const rows = await this.db.query<{
      id: string;
      provider_track_id: string | null;
    }>(
  ...
  private async syncLocalReleaseAudit(
    releaseId: string,
    parsed: TooLostNormalizedReleaseSyncPayload,
    providerReleaseId: string,
    ...
    if (releaseNotes.length) changedFields.push("releases.admin_notes", "releases.rejection_reason");
  ...
  private async syncLocalJobs(
    releaseId: string,
    localTracks: Array<{ id: string; provider_track_id: string | null }>,
    parsed: TooLostNormalizedReleaseSyncPayload,
    providerReleaseId: string,
    syncTimestamp: string,
    changedFields: string[],
  ...
  private async syncLocalDeliveries(
    releaseId: string,
    localTracks: Array<{ id: string; provider_track_id: string | null }>,
    parsed: TooLostNormalizedReleaseSyncPayload,
    syncTimestamp: string,
  ...
  async refreshStatusCache(reason: "oauth_callback" | "token_refresh" | "webhook" | "reconnect" | "disconnect" | "sync_now" | "background_refresh" = "background_refresh"): Promise<void> {
    try {
  ...
  private async validateReleaseIdentifiers(payload: TooLostReleasePayload): Promise<void> {
  ...
  private async fetchAccountProfile(token: TooLostOAuthToken): Promise<TooLostAccountProfile | null> {
    ...
    if (response.status === 401 || response.status === 403) {
      throw httpError(
        response.status,
        "TOO_LOST_PROFILE_VERIFICATION_FAILED",
        `Too Lost OAuth token verification failed: GET /me returned ${response.status}.`,
      );
    }
  ...
  private async ensureFlacBlob(sourceBlob: Blob, trackId: string, releaseId: string, trackTitle: string | null): Promise<Blob> {
...
export function buildReleasePayload(input: {
  ...
  const trackContributorRows = new Map<string, Array<{ track_id: string | null; name: string; role: string }>>();
...
function buildTrackPayloadEntry(
  track: TooLostReleasePayload["tracks"][number],
...
function buildImplicitValidationResult(releaseId: string, trackCount: number): Record<string, unknown> {
  return {
    releaseId,
    valid: true,
...
type TooLostNormalizedTrackSyncPayload = {
  trackId: string;
  providerTrackId: string | null;
  status: string | null;
  isrc: string | null;
...
type TooLostNormalizedReleaseSyncPayload = {
  status: string | null;
  updatedAt: string | null;
  reviewNotes: string[];
  warnings: string[];
  ...
  deliveryMessages: string[];
  rejectionReason: string | null;
  tracks: TooLostNormalizedTrackSyncPayload[];
};

function normalizeTooLostProviderStatus(value: string | null | undefined): string {
  const normalized = String(value ?? "unknown").trim().toLowerCase();
  if (!normalized) return "unknown";
  if (normalized === "under_review") return "in_review";
  if (normalized === "sent_to_stores") return "delivered";
...
function mapTooLostProviderReleaseStatus(status: string): string {
...
function mapTooLostProviderToDistributionStatus(status: string): string {
  switch (normalizeTooLostProviderStatus(status)) {
    case "pending":
    case "draft":
    case "unknown":
...
function mapTooLostProviderToJobStatus(status: string | null | undefined): string {
  switch (normalizeTooLostProviderStatus(status)) {
    case "pending":
    case "draft":
    case "unknown":
...
function mapTooLostProviderToDeliveryStatus(status: string | null | undefined): string {
  switch (normalizeTooLostProviderStatus(status)) {
    case "pending":
    case "draft":
    case "unknown":
...
function joinNotes(values: string[]): string | null {
  const notes = uniqueStrings(values);
  return notes.length ? notes.join("\n") : null;
}

function normalizeTooLostReleaseSyncPayload(value: unknown): TooLostNormalizedReleaseSyncPayload {
...
function normalizeTooLostReleaseTracks(root: TooLostJsonRecord): TooLostNormalizedTrackSyncPayload[] {
  const data = asRecord(root.data);
  const candidates = [
    root.tracks,
    data.tracks,
  ...
  return source
    ...
    .filter((entry): entry is TooLostNormalizedTrackSyncPayload => Boolean(entry));
}

function normalizeTooLostTrackSyncPayload(value: unknown, index: number): TooLostNormalizedTrackSyncPayload | null {
...
function resolveLocalTrackTarget(
  localTracks: Array<{ id: string; provider_track_id: string | null }>,
  track: TooLostNormalizedTrackSyncPayload,
  index: number,
): { id: string; provider_track_id: string | null } | null {
  ...
  for (const entry of value) {
    if (Array.isArray(entry)) {
      for (const nested of entry) {
        if (typeof nested === "string" && nested.trim()) notes.push(nested.trim());
...
function parseExternalReleaseId(value: unknown): string | null {
  ...
  const body = value as Record<string, unknown> & { data?: unknown };
...
function parseProviderTrackIds(value: unknown): string[] {
  ...
  return tracks
    .map((entry: unknown) => {
      ...
      return firstNonEmptyString(
        ...
        track.id,
        asRecord(track.data).trackId,
        asRecord(track.data).track_id,
        asRecord(track.data).providerTrackId,
        asRecord(track.data).provider_track_id,
        asRecord(track.data).id,
      );
    })
    .filter((entry: unknown): entry is string => Boolean(entry));
}

function mapReleaseType(value: string | null | undefined): string {
...
function buildWriterParticipants(
  track: { composer?: string | null; writers?: Array<{ name: string; role: string[] }> },
  fallbackName: string,
...
function buildContributorParticipants(
  rows: Array<{ track_id: string | null; name: string; role: string }>,
  fallbackName: string,
): Array<{ name: string; role: string[] }> {
  const rowsByName = new Map<string, { name: string; roles: Set<string> }>();
  for (const row of rows) {
    const name = row.name?.trim();
    const role = String(row.role || "").trim();
    if (!name || !role) continue;
    const normalizedRole = role.toLowerCase();
    if (normalizedRole === "composer" || normalizedRole === "lyricist" || normalizedRole === "songwriter") continue;
    const key = name.toLowerCase();
    const current = rowsByName.get(key) ?? { name, roles: new Set<string>() };
...
function buildContributorWriters(
  rows: Array<{ track_id: string | null; name: string; role: string }>,
  fallbackName: string,
): Array<{ name: string; role: string[] }> {
  const rowsByName = new Map<string, { name: string; roles: Set<string> }>();
...
function isTooLostValidationFailure(value: unknown): boolean {
  const body = asRecord(value);
  const data = asRecord(body.data);
  ...
  if (record.valid === false || record.isValid === false || record.success === false || record.ok === false) return true;
  if (["invalid", "failed", "error", "rejected"].includes(status)) return true;
...
function normalizeAudioFilename(value: string, audioFormat: string | null | undefined = null): string {
...
function normalizeTrackUploadRequest(body: unknown): Record<string, unknown> {
...
function normalizeTrackFileRequest(body: unknown): Record<string, unknown> {
  const source = asRecord(body);
  return {
    kind: String(source.kind || "audio"),
...
function normalizeArtworkMetadata(body: unknown): Record<string, unknown> {
  const source = asRecord(body);
  return {
...
function normalizeReleaseCreateRequest(body: unknown): Record<string, unknown> {
...
function normalizeReleaseMetadataRequest(body: unknown): Record<string, unknown> {
  const source = asRecord(body);
  const metadata: Record<string, unknown> = {};
  const label = firstNonEmptyString(source.label, source.primaryArtist, source.artist);

  if (hasOwn(source, "title")) metadata.title = nullableString(source.title);
  if (hasOwn(source, "version")) metadata.version = nullableString(source.version);
  if (hasOwn(source, "primaryArtist") || hasOwn(source, "artist")) metadata.primaryArtist = nullableString(source.primaryArtist, source.artist);
  if (hasOwn(source, "artistMode") || hasOwn(source, "artist_mode")) metadata.artistMode = nullableString(source.artistMode, source.artist_mode);
  if (hasOwn(source, "spotifyArtistId") || hasOwn(source, "spotify_artist_id")) metadata.spotifyArtistId = nullableString(source.spotifyArtistId, source.spotify_artist_id);
  if (hasOwn(source, "appleArtistId") || hasOwn(source, "apple_artist_id")) metadata.appleArtistId = nullableString(source.appleArtistId, source.apple_artist_id);
  if (hasOwn(source, "featuringArtists")) metadata.featuringArtists = Array.isArray(source.featuringArtists) ? source.featuringArtists : undefined;
  if (hasOwn(source, "variousArtists")) metadata.variousArtists = Boolean(source.variousArtists);
  if (hasOwn(source, "label") || hasOwn(source, "primaryArtist") || hasOwn(source, "artist")) metadata.label = label;
  if (hasOwn(source, "primaryGenre") || hasOwn(source, "genre")) metadata.primaryGenre = nullableString(source.primaryGenre, source.genre);
  if (hasOwn(source, "subGenre") || hasOwn(source, "subgenre")) metadata.subGenre = nullableString(source.subGenre, source.subgenre);
  if (hasOwn(source, "language")) metadata.language = nullableString(source.language);
  if (hasOwn(source, "upc")) metadata.upc = nullableString(source.upc);
  if (hasOwn(source, "releaseDate")) metadata.releaseDate = nullableString(source.releaseDate);
  if (hasOwn(source, "originalReleaseDate")) metadata.originalReleaseDate = nullableString(source.originalReleaseDate);
  ...
  if (hasOwn(source, "format")) metadata.format = nullableString(source.format);
  if (hasOwn(source, "producerCatalogueNumber")) metadata.producerCatalogueNumber = nullableString(source.producerCatalogueNumber);
  if (hasOwn(source, "cYear")) metadata.cYear = nullableString(source.cYear);
  ...
  if (hasOwn(source, "pYear")) metadata.pYear = nullableString(source.pYear);
...
function normalizeTrackListRequest(body: unknown): Record<string, unknown> {
  const source = asRecord(body);
    ...
    tracks: tracks.map((entry, index) => {
      ...
      const writers = hasParticipants(track.writers)
        ...
        : normalizeParticipants([
            ...
            firstNonEmptyString(track.writer, track.songwriter) ? { name: firstNonEmptyString(track.writer, track.songwriter) as string, role: ["songwriter"] } : null,
          ].filter(Boolean));
...
function asRecord(value: unknown): TooLostJsonRecord {
  return value && typeof value === "object" ? value as TooLostJsonRecord : {};
}
...
function numericOrNull(...values: unknown[]): number | null {
  for (const value of values) {
...
function hasParticipants(value: unknown): value is Array<{ name: string; role: string[] }> {
  return Array.isArray(value) && value.some((entry) => typeof asRecord(entry).name === "string");
}

function normalizeParticipants(value: unknown): Array<{ name: string; role: string[] }> {
  if (!Array.isArray(value)) return [];
  return value
    .map((entry: unknown) => {
...
function deriveProviderStatus(
  rows: Array<{ job_status: string | null; delivery_status: string | null }>,
  releaseStatus: string | null,
): string {
  if (rows.some((row) => row.job_status === "FAILED" || row.job_status === "REJECTED" || row.delivery_status === "FAILED")) return "failed";
  if (rows.some((row) => row.job_status === "PUBLISHED" || row.delivery_status === "PUBLISHED") || releaseStatus === "live") return "live";
  if (rows.some((row) => row.job_status === "DELIVERED") || releaseStatus === "delivered") return "delivered";
  if (rows.some((row) => row.job_status === "APPROVED") || releaseStatus === "approved") return "approved";
  if (rows.some((row) => row.job_status === "IN_REVIEW") || releaseStatus === "in_review" || releaseStatus === "under_review") return "in_review";
  if (
    rows.some((row) => row.job_status === "SUBMITTED" || row.job_status === "PROCESSING" || row.delivery_status === "PROCESSING")
    || releaseStatus === "processing"
  ) return "processing";
  return "pending";
}

function deriveDistributionStatus(
  rows: Array<{ job_status: string | null; delivery_status: string | null }>,
  releaseStatus: string | null,
): string {
  if (rows.some((row) => row.job_status === "FAILED" || row.job_status === "REJECTED" || row.delivery_status === "FAILED")) return "failed";
  if (rows.some((row) => row.delivery_status === "PUBLISHED" || row.job_status === "PUBLISHED") || releaseStatus === "live") return "live";
  if (rows.some((row) => row.job_status === "DELIVERED") || releaseStatus === "delivered") return "delivered";
  if (rows.some((row) => row.job_status === "APPROVED") || releaseStatus === "approved") return "approved";
  if (
    rows.some((row) =>
...
function normalizeReleaseStatus(value: string | null): string | null {
  if (!value) return null;
  if (value === "under_review") return "in_review";
  if (value === "sent_to_stores") return "delivered";
  return value;
}
...
function safeJsonParse(value: string): TooLostJsonRecord {
  try {
    return JSON.parse(value) as TooLostJsonRecord;
  } catch {
    return { raw: value };
  }
}
...
function readPrimaryAudioProbe(probe: Awaited<ReturnType<FfmpegRunner["probe"]>>) {
...
function verifyFlacBinary(
  bytes: Buffer,
  probe: ReturnType<typeof readPrimaryAudioProbe>,
  context: { releaseId: string; trackId: string; trackTitle: string | null },
) {
