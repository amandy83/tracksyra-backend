...
import type { ExecutionPipeline, ExecutionStageName } from "../types";
import type { ExecutionEventPublisher } from "../types/events";
import { createExecutionEvent } from "../types/events";
import { CheckpointManager } from "../checkpoint/checkpoint";
import { RecoveryCoordinator } from "../recovery/recovery";
import { ExecutionStageRegistry } from "../stages/stages";
import { ExecutionScheduler } from "../scheduler/scheduler";
import { LeaseManager } from "../lease/lease";
...
export class ExecutionCoordinator {
  ...
  async execute(job: DistributionJobAggregate, context: DistributionExecutionContext, pipeline: ExecutionPipeline): Promise<DistributionExecutionResult> {
    ...
    await this.eventPublisher.publish(
      createExecutionEvent("ExecutionStarted", {
    ...
    let completedStage: ExecutionStageName | null = null;
    ...
    try {
      while (true) {
        ...
        if (!nextStage) {
          ...
          await this.eventPublisher.publish(
            createExecutionEvent("ExecutionCompleted", {
        ...
        await this.eventPublisher.publish(
          createExecutionEvent("StageStarted", {
            executionId: current.executionId,
            pipeline: pipeline.name,
            stage: nextStage,
            occurredAt: new Date().toISOString(),
        ...
        if (checkpoint) {
          await this.eventPublisher.publish(
            createExecutionEvent("CheckpointCreated", {
              executionId: current.executionId,
              pipeline: pipeline.name,
              stage: nextStage,
        ...
        await this.eventPublisher.publish(
          createExecutionEvent("StageCompleted", {
            executionId: current.executionId,
            pipeline: pipeline.name,
            stage: nextStage,
            occurredAt: new Date().toISOString(),
      ...
      if (decision.pause) {
        await this.eventPublisher.publish(
          createExecutionEvent("ExecutionPaused", {
            executionId: current.executionId,
            pipeline: pipeline.name,
            stage: current.stage,
            occurredAt: new Date().toISOString(),
      ...
      if (decision.resume) {
        await this.eventPublisher.publish(
          createExecutionEvent("ExecutionRecovered", {
            executionId: current.executionId,
            pipeline: pipeline.name,
            stage: current.stage,
            occurredAt: new Date().toISOString(),
      ...
      const executionTime = Date.now() - startedAt;
      await this.eventPublisher.publish(
        createExecutionEvent("ExecutionFailed", {
          executionId: current.executionId,
          pipeline: pipeline.name,
          stage: current.stage,
          occurredAt: new Date().toISOString(),
