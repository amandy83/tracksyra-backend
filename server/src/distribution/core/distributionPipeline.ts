...
export class DistributionPipeline {
  ...
  constructor(private readonly deps: DistributionPipelineDependencies) {
    this.logger = deps.logger;
    this.retryPolicy = deps.retryPolicy;
    this.now = deps.now;
  }

  async execute(job: DistributionJob, context: DistributionContext): Promise<DistributionResult> {
    ...
    try {
      ...
      if (submission.status === DistributionStatus.FAILED) {
        ...
        if (nextRetryAt) this.deps.events?.emit("retry:scheduled", { job, context, error, retryAt: nextRetryAt, attempt: job.attempt + 1 });
      } else {
      ...
      if (nextRetryAt) this.deps.events?.emit("retry:scheduled", { job, context, error: normalized, retryAt: nextRetryAt, attempt: job.attempt + 1 });
      return result;
    }
  }

  private resolveProvider(context: DistributionContext): DistributionProvider {
  ...
  private computeRetryAt(
    ...
    if (status !== DistributionStatus.FAILED) return null;
