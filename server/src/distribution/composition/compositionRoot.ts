...
import { DEFAULT_COMPOSITION_MODULE_ORDER } from "./types/compositionTypes";
import { BullMQQueueFactory, BullMQQueueRegistry } from "../queue/integration/bullmq";
import { DistributionQueueDispatcher } from "../queue/integration/queueBridge";
import { registerDistributionExecutionStages } from "../execution/wiring/executionWiring";
import { ExecutionStageRegistry } from "../execution/stages/stages";
import {
  DistributionWorkerConfigurationProvider,
  DistributionWorkerExecutor,
  DistributionWorkerFactory,
  DistributionWorkerHealthChecker,
  DistributionWorkerLogger,
  DistributionWorkerMetricsCollector,
  DistributionWorkerRegistry,
  DistributionWorkerRuntime,
} from "../runtime/integration";
import type { WorkerRepositoryBundle } from "../runtime/integration/workerRuntime";
import {
  TrackSyraDspRuntimeEngine,
  TrackSyraDspRuntimeStore,
  TrackSyraDspRegistry,
  TrackSyraDspIntegrationRegistryFacade,
  TrackSyraDspFactory,
  TrackSyraDspResolver,
  TrackSyraDspRouter,
  TrackSyraDspLifecycleManager,
  TrackSyraDspAuthenticationManager,
  TrackSyraDspSessionManager,
  TrackSyraDspCredentialManager,
  TrackSyraDspCapabilityResolver,
  TrackSyraDspSelector,
  TrackSyraDspHealthManager,
  TrackSyraDspUploadManager,
  TrackSyraDspAssetManager,
  TrackSyraDspMetadataManager,
  TrackSyraDspCatalogManager,
  TrackSyraDspStatusManager,
  TrackSyraDspWebhookManager,
  TrackSyraDspPollingManager,
  TrackSyraDspRoyaltyManager,
  TrackSyraDspReportManager,
  TrackSyraDspTakedownManager,
  TrackSyraDspRateLimitManager,
  TrackSyraDspRetryManager,
  TrackSyraDspMetricsCollector,
  TrackSyraDspLogger,
  TrackSyraDspConfigurationProvider,
  TrackSyraDspEventPublisher,
} from "../provider-integration/runtime";
import type { ProviderIntegrationRuntimeDependencies, ProviderRepositoryBundle } from "../provider-integration/runtime/nativeDspRuntime";
...
import {
  DefaultConnectorResolver,
  InMemoryConnectorRegistry,
  OFFICIAL_DSP_CONNECTORS,
  OfficialDspConnectorFactory,
} from "../connectors";
...
import {
  ...
  PartnerOnboardingRuntimeEngine,
  PartnerOnboardingRegistry,
  PartnerOnboardingCredentialsStore,
  PartnerOnboardingDocumentationRegistry,
  PartnerOnboardingActivationResolver,
  TrackSyraPartnerOnboardingRuntime,
} from "../partner-onboarding";
import type { PartnerRepositoryBundle } from "../partner-onboarding/runtime/partnerOnboardingRuntime";
import {
  ...
  CredentialFactory,
  CredentialHealthPublisher,
  CredentialHealthChecker,
  CredentialInjectionPipeline,
  CredentialLogger,
  CredentialMetrics,
  CredentialMetricsPublisher,
  CredentialRecoveryManager,
  CredentialRefreshCoordinator,
  CredentialRegistry,
  CredentialResolverMiddleware,
  CredentialResolver,
  CredentialRotator,
  CredentialVersionBinding,
  CredentialVersionManager,
  CredentialVersionPinning,
  CredentialRotationPolicy,
  CredentialValidator,
  CredentialVault,
  CredentialExpiryGuard,
  CredentialRevocationGuard,
  CredentialConsistencyValidator,
  createTrackSyraCredentialService,
  CredentialSerializer,
} from "../partner-credentials";
import type { CredentialRepositoryBundle } from "../partner-credentials/credentialRuntime";
import {
  TrackSyraConflictResolver,
  TrackSyraPollingDispatcher,
  TrackSyraPollingExecutor,
  TrackSyraPollingHealthMonitor,
  TrackSyraPollingProcessor,
  TrackSyraPollingRegistry,
  TrackSyraPollingStrategyResolver,
  TrackSyraProjectionUpdater,
  TrackSyraReconciliationEngine,
  TrackSyraStatusEventPublisher,
  TrackSyraStatusMapper,
  TrackSyraStatusLogger,
  TrackSyraStatusMetrics,
  TrackSyraStatusScheduler,
  TrackSyraStatusNormalizer,
  TrackSyraTimelineUpdater,
  TrackSyraTransitionValidator,
  TrackSyraWebhookAuthenticationVerifier,
  TrackSyraWebhookAuditLogger,
  TrackSyraWebhookDeadLetterRouter,
  TrackSyraWebhookDispatcher,
  TrackSyraWebhookEventParser,
  TrackSyraWebhookDuplicateDetector,
  TrackSyraWebhookFailureRecovery,
  TrackSyraWebhookEventOrdering,
  TrackSyraWebhookPayloadValidator,
  TrackSyraWebhookProcessor,
  TrackSyraWebhookReceiver,
  TrackSyraWebhookRegistry,
  TrackSyraWebhookRetryQueue,
  TrackSyraWebhookReplayProtection,
  TrackSyraWebhookRouter,
  TrackSyraWebhookSignatureVerifier,
  TrackSyraWebhookValidator,
  TrackSyraStatusSyncRuntimeEngine,
  StatusSyncRuntimeDependencies,
} from "../status-sync/runtime";
import { DistributionStatusSyncEngineBridge } from "../status-sync/statusSyncBridge";
import {
  AnalyticsProjection,
  AuditTimeline,
  BackgroundSync,
  ...
  StateSyncHealthChecker,
  StateSyncMetrics,
  StateSyncRuntimeEngine,
  StateSyncLogger,
  StateVersionManager,
  TimelineBuilder,
  TimelineProjector,
  ...
  TransitionPlanner,
  RetrySync,
  StatusEvidenceCollector,
  StateReconciliationEngine,
} from "../status-sync/stateSyncRuntime";
import type { StatusSyncRepositoryBundle } from "../status-sync/stateSyncRuntime";
import {
  AdjustmentManager as RoyaltyAdjustmentManager,
  ArtistStatements as RoyaltyArtistStatements,
  AnalyticsPublisher as RoyaltyAnalyticsPublisher,
  AuditTrail as RoyaltyAuditTrail,
  BatchImportEngine as RoyaltyBatchImportEngine,
  ConflictResolver as RoyaltyConflictResolver,
  CurrencyConversionEngine as RoyaltyCurrencyConversionEngine,
  CurrencyNormalizer as RoyaltyCurrencyNormalizer,
  DefaultExchangeRateProvider as RoyaltyExchangeRateProvider,
  DefaultTaxCalculator as RoyaltyTaxCalculator,
  DistributorReports as RoyaltyDistributorReports,
  DuplicateDetection as RoyaltyDuplicateDetection,
  FinancialHistory as RoyaltyFinancialHistory,
  HoldManager as RoyaltyHoldManager,
  IncrementalImport as RoyaltyIncrementalImport,
  ImportRecovery as RoyaltyImportRecovery,
  LabelStatements as RoyaltyLabelStatements,
  LedgerBuilder as RoyaltyLedgerBuilder,
  ManualImport as RoyaltyManualImport,
  ProductNormalizer as RoyaltyProductNormalizer,
  ReportImportManager as RoyaltyReportImportManager,
  ReportNormalizer as RoyaltyReportNormalizer,
  ReportParser as RoyaltyReportParser,
  RevenueAllocator as RoyaltyRevenueAllocator,
  RevenueCalculator as RoyaltyRevenueCalculator,
  RevenueDashboard as RoyaltyRevenueDashboard,
  RoyaltyHealthChecker as RoyaltyHealthChecker,
  RoyaltyLifecycleManager as RoyaltyLifecycleManager,
  RoyaltyLogger as RoyaltyLogger,
  RoyaltyMetrics as RoyaltyMetrics,
  RoyaltyReconciliationEngine as RoyaltyReconciliationEngine,
  RoyaltyRegistry as RoyaltyRegistry,
  RoyaltyResolver as RoyaltyResolver,
  RoyaltyCoordinator as RoyaltyCoordinator,
  RoyaltyRuntimeEngine as RoyaltyRuntimeEngine,
  RoyaltyRuntimeDependencies,
  ScheduledImport as RoyaltyScheduledImport,
  SettlementManager as RoyaltySettlementManager,
  StatementGenerator as RoyaltyStatementGenerator,
  TerritoryNormalizer as RoyaltyTerritoryNormalizer,
  VersionManager as RoyaltyVersionManager,
} from "../royalty/runtime";
import type { RoyaltyRepositoryBundle } from "../royalty/runtime/royaltyRuntime";
import {
  AlertDispatcher,
  AlertEngine,
  AlertRules,
  ...
  Histograms,
  MetricsRuntime,
  ObservabilityEventPublisherFacade,
  ...
  ProviderDashboard,
  QueueDashboard,
  QueueMetrics as ObservabilityQueueMetrics,
  ...
  RoyaltyDashboard,
  RoyaltyMetrics as ObservabilityRoyaltyMetrics,
  SecurityLogManager,
  StateSyncDashboard,
  StateSyncMetrics as ObservabilityStateSyncMetrics,
  StructuredLogger,
  SystemDashboard,
  StartupValidator,
  Timers,
...
import {
  ...
  SpecificationSchemaValidator,
  SpecificationSerializer,
  SpecificationSignatureValidator,
  SpecificationRetryPolicy,
  SpecificationValidator,
  SpecificationVersionManager,
  SpecificationActivationManager,
} from "../dsp-specification";
import { createTrackSyraDeliveryRuntime } from "../runtime/delivery";
import { createPublishingStandardsRuntime } from "../publishing";
import { BackupDisasterRecoveryService } from "../backup";
import {
  createEnterpriseRightsRuntime,
  registerRightsAuditQueueWorker,
  registerRightsConflictQueueWorker,
  registerRightsLicenseExpirationQueueWorker,
  registerRightsTerritorySyncQueueWorker,
  registerRightsValidationQueueWorker,
  registerRightsWithdrawalQueueWorker,
} from "../rights";
import { HealthStatus } from "../observability/health/healthStatus";
import { ValidationRuntimeEngine } from "../validation/runtime";
import * as ValidationValidators from "../validation/runtime/validationValidators";
import {
  ValidationCoordinatorImpl,
  ValidationPipelineImpl,
  ValidationRegistryImpl,
  ValidationSchedulerImpl,
  ValidationSerializer,
  RuntimeValidationLogger,
...
import {
  DefaultCheckpointOwnership,
  DefaultDistributedLeaderElection,
  DefaultDuplicateExecutionPrevention,
  DefaultExecutionFencing,
  DefaultExecutionTokenService,
  DefaultHeartbeatOwnership,
  DefaultLeaseOwnership,
  DefaultRecoveryOwnership,
  DefaultStaleLeaseDetection,
  DefaultWorkerOwnership,
  DistributedCoordinationBundle,
  DefaultStorageEngine,
  DefaultStorageFactory,
  DefaultStorageHealthChecker,
  DefaultStorageLogger,
  DefaultStorageMetrics,
  DefaultStorageRegistry,
  AdapterStorageRepository,
  FileStorageAdapter,
  JsonStorageSerializer,
} from "../infrastructure/storage";
import { type RuntimeCollectionState, RuntimeRepository } from "../infrastructure/repositories/runtime";
...
import { WorkerRuntime as QueueWorkerRuntimeClass, createQueueSchedulers, type WorkerRuntimeRegistration, type WorkerRuntime as QueueWorkerRuntime } from "../../workers/runtime/workerRuntime";
import { WorkerSupervisor } from "../../workers/runtime/workerSupervisor";
import { registerEmailWorker } from "../../workers/email/emailWorker";
import { registerRoyaltyWorker } from "../../workers/royalties/royaltyWorker";
import {
  registerAdjustmentWorker as registerRoyaltyAdjustmentWorker,
  registerCurrencyWorker as registerRoyaltyCurrencyWorker,
  registerForecastWorker as registerRoyaltyForecastWorker,
  registerPaymentWorker as registerRoyaltyPaymentWorker,
  registerRoyaltyAuditWorker,
  registerRoyaltyCalculationWorker,
  registerRoyaltyRetryWorker,
  registerReserveWorker as registerRoyaltyReserveWorker,
  registerStatementWorker,
  registerTaxWorker as registerRoyaltyTaxWorker,
} from "../../workers/royalties/royaltyAccountingWorkers";
import { registerFraudWorker } from "../../workers/fraud/fraudWorker";
import { registerAnalyticsWorker } from "../../workers/analytics/analyticsWorker";
import { registerRealtimeWorker } from "../../workers/realtime/realtimeWorker";
import { registerMediaProcessingWorker, registerArtworkProcessingWorker, registerWaveformGenerationWorker, registerFingerprintAnalysisWorker } from "../../media/workers/mediaWorkers";
import { PromoAssetWorker, registerPromoAssetWorker } from "../../workers/promoAssetWorker";
import { registerAuditQueueWorker, registerDeliveryQueueWorker, registerFraudReviewQueueWorker, registerReviewQueueWorker, registerRetryQueueWorker, registerTakedownQueueWorker, registerValidationQueueWorker, registerWithdrawalQueueWorker } from "../../workers/enterprise/enterpriseWorkers";
import { registerBackupWorker, registerBackupVerificationWorker, registerIncrementalBackupWorker, registerRecoveryAuditWorker, registerRestoreWorker } from "../../workers/enterprise/backupWorkers";
import { registerDuplicateDetectionWorker, registerFingerprintAuditWorker, registerFingerprintRetryWorker, registerFingerprintWorker, registerFraudAudioWorker, registerSimilarityWorker } from "../../workers/fingerprinting/fingerprintWorkers";
import { registerMetadataAuditWorker, registerMetadataNormalizationWorker, registerMetadataRecommendationWorker, registerMetadataRepairWorker, registerMetadataRetryWorker, registerMetadataValidationWorker } from "../../workers/metadata/metadataWorkers";
import { ReleaseAutomationEngine } from "../workflow/releaseAutomationEngine";
import {
  PackageAssets,
  PackageAudit,
  PackageBuilder,
...
import { ReleaseDeliveryEngine } from "../core/releaseDeliveryEngine";
import { ChecksumGenerator } from "../core/checksumGenerator";
...
import {
  registerDeliveryAuditWorker,
  registerDeliveryHealthWorker,
  registerDeliveryOrchestratorWorker,
  registerDeliveryRollbackWorker,
  registerDeliveryRetryWorker,
  registerReleaseApprovalWorker,
  registerReleaseAutomationWorker,
  registerReleaseSchedulerWorker,
  registerSLAWorker,
  registerWebhookProcessingWorker,
} from "../../workers/release-automation/releaseAutomationWorkers";
import {
  registerAmazonMusicDeliveryWorker,
  registerAmazonMusicHealthWorker,
  registerAmazonMusicPollingWorker,
  registerAmazonMusicRetryWorker,
  registerAmazonMusicWebhookWorker,
  AmazonMusicDeliveryWorker,
  AmazonMusicHealthWorker,
  AmazonMusicPollingWorker,
  AmazonMusicRetryWorker,
  AmazonMusicWebhookWorker,
} from "../../workers/dsp/amazonMusicWorkers";
import {
  registerDeezerDeliveryWorker,
  registerDeezerHealthWorker,
  registerDeezerPollingWorker,
  registerDeezerRetryWorker,
  registerDeezerWebhookWorker,
  DeezerDeliveryWorker,
  DeezerHealthWorker,
  DeezerPollingWorker,
  DeezerRetryWorker,
  DeezerWebhookWorker,
} from "../../workers/dsp/deezerMusicWorkers";
import {
  registerJioSaavnDeliveryWorker,
  registerJioSaavnHealthWorker,
  registerJioSaavnPollingWorker,
  registerJioSaavnRetryWorker,
  registerJioSaavnWebhookWorker,
  JioSaavnDeliveryWorker,
  JioSaavnHealthWorker,
  JioSaavnPollingWorker,
  JioSaavnRetryWorker,
  JioSaavnWebhookWorker,
} from "../../workers/dsp/jioSaavnWorkers";
import {
  registerAnghamiDeliveryWorker,
  registerAnghamiHealthWorker,
  registerAnghamiPollingWorker,
  registerAnghamiRetryWorker,
  registerAnghamiWebhookWorker,
  AnghamiDeliveryWorker,
  AnghamiHealthWorker,
  AnghamiPollingWorker,
  AnghamiRetryWorker,
  AnghamiWebhookWorker,
} from "../../workers/dsp/anghamiWorkers";
import {
  registerBoomplayDeliveryWorker,
  registerBoomplayHealthWorker,
  registerBoomplayPollingWorker,
  registerBoomplayRetryWorker,
  registerBoomplayWebhookWorker,
  BoomplayDeliveryWorker,
  BoomplayHealthWorker,
  BoomplayPollingWorker,
  BoomplayRetryWorker,
  BoomplayWebhookWorker,
} from "../../workers/dsp/boomplayWorkers";
import {
  registerTikTokDeliveryWorker,
  registerTikTokHealthWorker,
  registerTikTokPollingWorker,
  registerTikTokRetryWorker,
  registerTikTokWebhookWorker,
  TikTokDeliveryWorker,
  TikTokHealthWorker,
  TikTokPollingWorker,
  TikTokRetryWorker,
  TikTokWebhookWorker,
} from "../../workers/dsp/tiktokWorkers";
import {
  registerMetaDeliveryWorker,
  registerMetaHealthWorker,
  registerMetaPollingWorker,
  registerMetaRetryWorker,
  registerMetaWebhookWorker,
  MetaDeliveryWorker,
  MetaHealthWorker,
  MetaPollingWorker,
  MetaRetryWorker,
  MetaWebhookWorker,
} from "../../workers/dsp/metaRightsManagerWorkers";
import {
  registerTidalDeliveryWorker,
  registerTidalHealthWorker,
  registerTidalPollingWorker,
  registerTidalRetryWorker,
  registerTidalWebhookWorker,
  TidalDeliveryWorker,
  TidalHealthWorker,
  TidalPollingWorker,
  TidalRetryWorker,
  TidalWebhookWorker,
} from "../../workers/dsp/tidalMusicWorkers";
import {
  registerYouTubeContentIdWorker,
  registerYouTubeDeliveryWorker,
  registerYouTubeHealthWorker,
  registerYouTubePollingWorker,
  registerYouTubeRetryWorker,
  registerYouTubeWebhookWorker,
  YouTubeContentIdWorker,
  YouTubeDeliveryWorker,
  YouTubeHealthWorker,
  YouTubePollingWorker,
  YouTubeRetryWorker,
  YouTubeWebhookWorker,
} from "../../workers/dsp/youtubeMusicWorkers";
import { registerPayoutJobProcessor } from "../../payouts/queue/payoutJobProcessor";
import { DistributionWorker, registerDistributionQueueWorker } from "../queue/distributionWorker";
import { createSequelize, SequelizeSqlExecutor } from "../services/sequelizeSqlExecutor";
import { SqlDistributionStore } from "../services/distributionStore";
import { DistributionIdGenerator } from "../idGenerator";
import { createDistributionAudioUrlResolver } from "../services/distributionMediaResolver";
import { EnterpriseDistributionService } from "../admin/enterpriseDistributionService";
import { EnterpriseOperationsService } from "../admin/enterpriseOperationsService";
import { RoyaltyStore } from "../../royalties/services/royaltyStore";
import { RoyaltyEngine } from "../../royalties/core/royaltyEngine";
import { RoyaltyAccountingService } from "../../royalties/accounting/royaltyAccountingService";
import { RoyaltyStatementGenerator as RoyaltyDocumentGenerator } from "../../royalties/statements/statementGenerator";
import { FraudStore } from "../../fraud/services/fraudStore";
import { FraudDetectionEngine } from "../../fraud/detectors/fraudDetectionEngine";
import { FraudFeatureExtractor } from "../../fraud/detectors/featureExtractor";
import { FraudRuleEngine } from "../../fraud/rules";
import { StreamAnalyticsService } from "../../analytics/streams/streamAnalyticsService";
import { RevenueAnalyticsService } from "../../analytics/revenue/revenueAnalyticsService";
import { FraudAnalyticsService } from "../../fraud/analytics/fraudAnalyticsService";
import { DistributionAnalyticsService } from "../../distribution/analytics/distributionAnalyticsService";
import { DistributionIntelligenceStore, RetryEngine } from "../intelligence";
import { RealtimeEventStore } from "../../realtime/events/realtimeEventStore";
import { EventBus } from "../../realtime/events/eventBus";
import { LiveDashboardService } from "../../realtime/liveDashboardService";
import { MediaProcessingEngine } from "../../media/services/MediaProcessingEngine";
import { AudioFingerprintService } from "../../media/services/AudioFingerprintService";
import { AudioTranscodingService } from "../../media/services/AudioTranscodingService";
import { ArtworkOptimizationService } from "../../media/services/ArtworkOptimizationService";
import { MediaValidationService } from "../../media/services/MediaValidationService";
import { PreviewClipGenerator } from "../../media/services/PreviewClipGenerator";
import { WaveformGenerator } from "../../media/services/WaveformGenerator";
import { MetadataTransformer } from "../metadata/metadataTransformer";
import { MetadataComparator } from "../metadata/metadataComparator";
import { MetadataHasher } from "../metadata/metadataHasher";
import { MetadataValidator } from "../metadata/metadataValidator";
import { DdexCompressionService, DdexErnMapper, DdexFoundationService, DdexMeadMapper, DdexRinMapper, DdexValidator, DdexXmlSerializer } from "../ddex";
import { AudioFingerprintingEngine } from "../intelligence/fingerprinting";
import { MetadataIntelligenceEngine } from "../intelligence/metadata";
import { S3CompatibleMediaStorageAdapter, SupabaseMediaStorageAdapter } from "../../media/storage/mediaStorage";
import { FfmpegService as PromoAssetFfmpegService } from "../../media/promo-assets/processing/ffmpegService";
import { PromoAssetVideoProcessor } from "../../media/promo-assets/processing/videoProcessor";
import { PromoAssetPlatformValidationEngine } from "../../media/promo-assets/platformValidation/platformValidator";
import { readQueueEnvironment } from "../../queue/redis";
import { PayoutEngine } from "../../payouts/core/payoutEngine";
import { PayoutRequestService } from "../../payouts/services/payoutRequestService";
import { PayoutService } from "../../payouts/services/payoutService";
import { createWalletService } from "../../wallet/services/walletService";
import { PayoutWalletService } from "../../payments/payouts/services/payoutWalletService";
...
import { TooLostCredentialStore, TooLostIntegrationService } from "../providers/too-lost";
import { TooLostApiClient } from "../providers/too-lost/tooLostApiClient";
import { TooLostWebhookController } from "../webhooks";
import { FfmpegRunner } from "../../media/services/ffmpeg";
import { DistributedLock, LockLease, LockToken } from "../infrastructure/locking/locking";
import {
  DefaultAggregateConflictResolver,
  DefaultAggregateLockManager,
  DefaultAggregateVersionTracker,
  DefaultRepositoryLifetimeManager,
  DefaultUnitOfWork,
  DefaultUnitOfWorkFactory,
  DefaultUnitOfWorkManager,
  DefaultRepositoryScope,
} from "../infrastructure/unit-of-work";
import {
  DefaultAggregateFactoryProvider,
  DefaultAggregateProvider,
  DefaultDependencyProvider,
  DefaultLifetimeProvider,
  DefaultRepositoryProvider,
  DefaultRuntimeFactoryProvider,
...
export type DistributionCompositionRootState = Readonly<{
  modules: readonly ModuleDescriptor[];
  services: readonly ServiceDescriptor[];
  container: DistributionDependencyContainer;
  moduleRegistry: DistributionModuleRegistry;
  serviceRegistry: DistributionServiceRegistry;
}>;

export class DistributionDependencyContainer implements DependencyContainer {
  private readonly values = new Map<string, unknown>();
  private currentConfiguration: CompositionConfiguration | null = null;
  ...
  resolve<T>(token: string): T | null {
    return (this.values.get(token) ?? null) as T | null;
  }
  ...
  setConfiguration(configuration: CompositionConfiguration | null): void {
...
export class DistributionModuleRegistry implements ModuleRegistry {
  ...
  registerAll(modules: readonly ModuleDescriptor[]): void {
...
export class DistributionServiceRegistry implements ServiceRegistry {
  ...
  registerAll(services: readonly ServiceDescriptor[]): void {
...
export function createDistributionDependencyGraph(configuration: CompositionConfiguration): DependencyGraph {
  const modules = DEFAULT_COMPOSITION_MODULE_ORDER.map((moduleName, index) => new ModuleDescriptor({
    ...
    metadata: { environment: configuration.environment } as never,
  }));
  return new DependencyGraph({
    ...
    metadata: { compositionId: configuration.compositionId } as never,
  });
}

function createDefaultServices(modules: readonly ModuleDescriptor[]): readonly ServiceDescriptor[] {
...
function seedRegistries(
  state: DistributionCompositionRootState,
  ...
  const runtimeStorageRepository = storageEngine.repository<RuntimeCollectionState<unknown>>("default");
  ...
  const runtimeRepositoryFactory = <TKey, TValue>(
    repository: StorageRepository<RuntimeCollectionState<TValue>>,
    namespace: string,
    name: string,
    serializeKey?: (key: TKey) => string,
    deserializeKey?: (key: string) => TKey,
  ): RuntimeRepository<TKey, TValue> => new RuntimeRepository(repository, namespace, name, serializeKey, deserializeKey);
  const workerRepositories = {
    configurations: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "worker-configurations"),
    workers: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "worker-workers"),
    leases: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "worker-leases"),
    heartbeats: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "worker-heartbeats"),
    checkpoints: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "worker-checkpoints"),
    recoveries: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "worker-recoveries"),
    cancellations: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "worker-cancellations"),
    activeExecutions: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "worker-active-executions"),
    health: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "worker-health"),
    statistics: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "worker-statistics"),
    metrics: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "worker-metrics"),
  } as WorkerRepositoryBundle;
  ...
  const runtimeFactory = (): DistributionWorkerRuntime => {
  ...
  const storageCoordination: DistributedCoordinationBundle = {
    lock: new (class implements DistributedLock {
      constructor(private readonly leases: DefaultLeaseOwnership) {}
  ...
  const createDistributionRuntimeRepository = (name: string) =>
    runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", name);

  const partnerRepositories = {
    states: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "partner-states"),
    credentials: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "partner-credentials"),
    documentation: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "partner-documentation"),
    contacts: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "partner-contacts"),
    agreements: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "partner-agreements"),
    metadata: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "partner-metadata"),
    requirements: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "partner-requirements"),
    checklists: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "partner-checklists"),
    validationRules: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "partner-validation-rules"),
  } as PartnerRepositoryBundle;
  ...
  const partnerOnboardingRuntime = new TrackSyraPartnerOnboardingRuntime(
    partnerOnboardingRuntimeEngine,
    partnerOnboardingRegistry,
    partnerOnboardingCredentialsStore,
    partnerOnboardingDocumentationRegistry,
  ...
  const observabilityLogger = new StructuredLogger(
    ...
    observabilityRequestIds,
    observabilityCorrelationIds,
    observabilityLogContexts,
  );
  ...
  const observabilityTraceRuntime = new DistributedTraceRuntime(
    observabilitySpanManager,
    observabilityParentChildSpanResolver,
    observabilityPipelineTraceBuilder,
    observabilityRuntimeTracePublisher,
  );
  ...
  const observabilityHealthRuntime = new HealthRuntime(
    observabilityComponentHealthRegistry,
    observabilityReadinessEngine,
    observabilityLivenessEngine,
    observabilityStartupValidator,
    observabilityShutdownValidator,
    observabilityDependencyHealthAnalyzer,
  );
  ...
  const observabilityDiagnosticRuntime = new DiagnosticRuntime(
    observabilityDependencyDiagnostics,
    observabilityRuntimeDiagnostics,
    observabilityFailureAnalyzer,
    observabilityRecoveryAnalyzer,
    observabilityDeadlockDetection,
    observabilityConfigurationValidator,
    observabilityLogger,
    observabilityMetricsRuntime,
    observabilityHealthRuntime,
  );
  ...
  const observabilityRuntimeDependencies: ObservabilityRuntimeDependencies = {
  ...
  const credentialRepositories = {
    bundles: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "credential-bundles"),
    versions: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "credential-versions"),
    metrics: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "credential-metrics"),
  } as CredentialRepositoryBundle;
  ...
  const credentialFactory = new CredentialFactory(
    credentialEncryptor,
    credentialVersionManager,
    (credentials, environment) => new CredentialAccessPolicy({
  ...
  const credentialRecoveryManager = new CredentialRecoveryManager(
    credentialRegistry,
    credentialSerializer,
    (value) => new CredentialAccessPolicy(value),
    (value) => new CredentialRotationPolicy(value),
  );
  ...
  const providerRepositories = {
    configurations: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "provider-configurations"),
    records: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "provider-records"),
    entries: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "provider-entries"),
    metrics: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "provider-metrics"),
  } as ProviderRepositoryBundle;
  const providerRuntimeDependencies: ProviderIntegrationRuntimeDependencies = {
  ...
  const providerRuntime: TrackSyraDspRuntimeEngine = new TrackSyraDspRuntimeEngine(providerRuntimeDependencies);
  ...
  const specificationRegistryRef: { current: SpecificationRegistry | null } = { current: null };
  ...
  const dspProtocolRuntimeFactory = () => createTrackSyraDspProtocolRuntime({
    ...
    eventPublisher: observabilityRuntime.observabilityEventPublisher as never,
    healthChecker: partnerOnboardingRuntime.activationResolver as never,
  ...
  state.container.register("partner.onboarding.runtimeFactory", () =>
    new TrackSyraPartnerOnboardingRuntime(
      new PartnerOnboardingRuntimeEngine(partnerRepositories, {}),
      new PartnerOnboardingRegistry(partnerOnboardingRuntimeEngine),
      new PartnerOnboardingCredentialsStore(partnerOnboardingRuntimeEngine),
      new PartnerOnboardingDocumentationRegistry(partnerOnboardingRuntimeEngine),
      new PartnerOnboardingActivationResolver(partnerOnboardingRuntimeEngine),
    ));
  ...
  state.container.register("credential.execution.scope", (input: ConstructorParameters<typeof CredentialExecutionScope>[0]) => new CredentialExecutionScope(input));
  state.container.register("credential.version.binding", (input: ConstructorParameters<typeof CredentialVersionBinding>[0]) => new CredentialVersionBinding(input));
  state.container.register("credential.version.pinning", (input: ConstructorParameters<typeof CredentialVersionPinning>[0]) => new CredentialVersionPinning(input));
  ...
  state.container.register("credential.binding.factory", (input: ConstructorParameters<typeof CredentialBinding>[0]) => new CredentialBinding(input));
  state.container.register("credential.authentication.context.factory", (input: ConstructorParameters<typeof AuthenticationContext>[0]) => new AuthenticationContext(input));
  ...
  state.container.register("delivery.runtimeFactory", (dependencies: Parameters<typeof createTrackSyraDeliveryRuntime>[0]) =>
    createTrackSyraDeliveryRuntime({
      ...dependencies,
      credentialResolver: credentialService.resolver,
    }));
  ...
  const statusSyncWebhookReceiver = new TrackSyraWebhookReceiver(
    statusSyncWebhookEventParser,
    statusSyncWebhookSignatureVerifier,
    statusSyncWebhookAuthenticationVerifier,
    statusSyncWebhookPayloadValidator,
    statusSyncLogger,
  );
  const statusSyncWebhookValidator = new TrackSyraWebhookValidator(
    statusSyncWebhookSignatureVerifier,
    statusSyncWebhookAuthenticationVerifier,
    statusSyncWebhookPayloadValidator,
    new TrackSyraWebhookDuplicateDetector(),
    new TrackSyraWebhookReplayProtection(),
    new TrackSyraWebhookEventOrdering(),
  );
  const statusSyncWebhookRouter = new TrackSyraWebhookRouter(statusSyncWebhookRegistry);
  const statusSyncWebhookDispatcher = new TrackSyraWebhookDispatcher(
    statusSyncWebhookRouter,
    statusSyncWebhookValidator,
    statusSyncWebhookReceiver,
    statusSyncLogger,
    statusSyncMetrics,
  );
  const statusSyncPollingStrategyResolver = new TrackSyraPollingStrategyResolver();
  const statusSyncPollingExecutor = new TrackSyraPollingExecutor(
    statusSyncMapper,
    statusSyncScheduler,
    statusSyncMetrics,
    statusSyncLogger,
    statusSyncPollingStrategyResolver,
  );
  const statusSyncPollingDispatcher = new TrackSyraPollingDispatcher(
    statusSyncPollingRegistry,
    statusSyncPollingExecutor,
    statusSyncLogger,
    statusSyncMetrics,
  );
  const statusSyncWebhookProcessor = new TrackSyraWebhookProcessor(
    statusSyncWebhookDispatcher,
    statusSyncNormalizer,
    statusSyncTransitionValidator,
    statusSyncReconciliationEngine,
    statusSyncProjectionUpdater,
    statusSyncTimelineUpdater,
    statusSyncEventPublisher,
    statusSyncMetrics,
    statusSyncLogger,
    statusSyncConflictResolver,
  );
  const statusSyncPollingProcessor = new TrackSyraPollingProcessor(statusSyncPollingDispatcher);
  const statusSyncRuntimeDependenciesBase: Omit<StatusSyncRuntimeDependencies, "bridge"> = {
  ...
  const stateSyncRepositories = {
    values: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "state-sync-values"),
    snapshots: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "state-sync-snapshots"),
    history: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "state-sync-history"),
    versions: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "state-sync-versions"),
    published: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "state-sync-published"),
    distribution: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "state-sync-distribution"),
    release: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "state-sync-release"),
    dashboard: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "state-sync-dashboard"),
    analytics: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "state-sync-analytics"),
    checkpoints: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "state-sync-checkpoints"),
  } as StatusSyncRepositoryBundle;
  ...
  const stateSyncPublisher = new ProjectionPublisher(
    stateSyncRepositories.distribution,
    stateSyncRepositories.release,
    stateSyncRepositories.dashboard,
    stateSyncRepositories.analytics,
    stateSyncRepositories.checkpoints,
  );
  ...
  state.container.register("state.sync.runtimeFactory", (orchestrator: DistributionOrchestrator) =>
    new StateSyncRuntimeEngine({
      ...statusSyncRuntimeDependenciesBase,
      bridge: new DistributionStatusSyncEngineBridge(orchestrator),
    }));
  state.container.register("status.sync.runtimeFactory", (orchestrator: DistributionOrchestrator) =>
    new TrackSyraStatusSyncRuntimeEngine({
      ...statusSyncRuntimeDependenciesBase,
      bridge: new DistributionStatusSyncEngineBridge(orchestrator),
    }));
  const royaltyRepositories = {
    records: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "royalty-records"),
    stages: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "royalty-stages"),
    counters: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "royalty-counters"),
    observations: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "royalty-observations"),
    rates: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "royalty-rates"),
    holds: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "royalty-holds"),
    adjustments: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "royalty-adjustments"),
    snapshots: runtimeRepositoryFactory(runtimeStorageRepository as never, "distribution", "royalty-snapshots"),
  } as RoyaltyRepositoryBundle;
  ...
  const royaltyReportImportManager = new RoyaltyReportImportManager(
    royaltyRegistry,
    royaltyLifecycleManager,
    royaltyReportParser,
    royaltyReportNormalizer,
    royaltyLedgerBuilder,
    royaltyRevenueCalculator,
    royaltySettlementManager,
    royaltyStatementGenerator,
    royaltyReconciliationEngine,
    royaltyAuditTrail,
    royaltyFinancialHistory,
    royaltyLogger,
    royaltyMetrics,
    royaltyAnalyticsPublisher,
  );
  ...
  const royaltyRuntimeDependencies: RoyaltyRuntimeDependencies = {
  ...
  state.container.register("royalty.coordinator.factory", (runtime: RoyaltyRuntimeEngine) => new RoyaltyCoordinator(runtime));
  ...
  const validationRuntime: ValidationRuntimeEngine = new ValidationRuntimeEngine(validationRuntimeDependencies);
  ...
  state.container.register(
    "queue.dispatcher.factory",
    (runtime: WorkerRuntime) => new DistributionQueueDispatcher(runtime),
  );
  state.container.register(
    "execution.stageRegistry.factory",
...
export class DistributionCompositionRoot implements CompositionRoot {
  constructor(
    private readonly state: DistributionCompositionRootState,
  ) {}
...
export type OperationsServerDependencies = Readonly<{
  runtime: QueueWorkerRuntime;
  recovery: AdminRecoveryService;
  passwordResetService: PasswordResetService;
  resendWebhookService: ResendWebhookService;
  tooLostCredentialStore: TooLostCredentialStore;
  tooLostIntegrationService: TooLostIntegrationService;
  tooLostWebhookController: TooLostWebhookController;
  enterpriseDistributionService: EnterpriseDistributionService;
  royaltyAccountingService: RoyaltyAccountingService;
  adminSupabaseClient: SupabaseClient;
  logger: Logger;
}>;

export type DistributionBootstrapDependencies = Readonly<{
  runtime: QueueWorkerRuntime;
  queueSchedulers: readonly unknown[];
  workerRegistrations: readonly WorkerRuntimeRegistration[];
  emailWorkerRegistration: Awaited<ReturnType<typeof registerEmailWorker>>;
  operationsServerDependencies: OperationsServerDependencies;
  logger: Logger;
}>;

export async function createDistributionBootstrapDependencies(options: { emailWorkerIntervalMs?: number } = {}): Promise<DistributionBootstrapDependencies> {
  loadRuntimeEnv();
  ...
  const state: DistributionCompositionRootState = {
    modules: [],
    services: [],
  ...
  const ddexFoundationService = new DdexFoundationService(
    new DdexErnMapper(),
    new DdexMeadMapper(),
    new DdexRinMapper(),
    new DdexValidator(),
    new DdexXmlSerializer(),
    new DdexCompressionService(false),
    null,
  );
  ...
  const promoAssetProcessor = new PromoAssetVideoProcessor(
    adminSupabaseClient,
    promoAssetStorage,
    promoAssetFfmpeg,
    promoAssetPlatformValidation,
  );
  ...
  const workerRegistrations: WorkerRuntimeRegistration[] = [
