...
export function validateMusicRelease(release: MusicRelease): MusicReleaseValidationResult {
  ...
  if (!release.primaryArtistName?.trim()) errors.push("MusicRelease.primaryArtistName is required");
  ...
  release.audioFiles.forEach((audio, index) => {
    ...
    if (!audio.primaryArtistName?.trim()) errors.push(`audioFiles[${index}].primaryArtistName is required`);
...
function hasLinkedArtistIdentity(spotifyArtistId?: string | null, appleArtistId?: string | null) {
