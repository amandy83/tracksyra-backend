...
export type QueueEnvironment = {
  ...
  redisConnectionSource: RedisConnectionSource;
...
export type RedisConnectionSource = "redis-url" | "redis-host-port" | "unconfigured";
export type RedisHealthState = "unknown" | "connecting" | "healthy" | "degraded" | "unhealthy";

export type RedisHealthSnapshot = Readonly<{
  provider: "redis";
  ...
  infoLatencyMs: number | null;
  redisUrl: string | null;
  redisConnectionSource: RedisConnectionSource;
  redisTls: boolean;
}>;

type RedisHealthProbeResult = {
  pingLatencyMs: number | null;
  infoLatencyMs: number | null;
};
...
let redisHealthSnapshot: RedisHealthSnapshot = createRedisHealthSnapshot({
  provider: "redis",
  state: "unknown",
  healthy: false,
...
let redisHealthRefreshPromise: Promise<RedisHealthSnapshot> | null = null;
...
export async function refreshRedisHealthSnapshot(options: { force?: boolean; background?: boolean } = {}): Promise<RedisHealthSnapshot> {
  if (!options.force && redisHealthRefreshPromise) return redisHealthRefreshPromise;
  if (redisHealthRefreshPromise) return redisHealthRefreshPromise;
  ...
  try {
    ...
    if (!options.background) redisHealthRefreshPromise = null;
    else queueMicrotask(() => {
...
function isClosedConnection(connection: IORedis) {
...
function readRedisConfiguration() {
  ...
  if (fromUrl) {
    ...
    const source: RedisConnectionSource = "redis-url";
    return {
  ...
  if (host && port) {
    const source: RedisConnectionSource = "redis-host-port";
...
function normalizeDatabaseIndex(value: string | undefined): number {
...
function createRedisHealthSnapshot(snapshot: Omit<RedisHealthSnapshot, "cacheAgeMs"> & { cacheAgeMs?: number }): RedisHealthSnapshot {
  return {
    ...snapshot,
...
function updateRedisHealthSnapshot(partial: Partial<RedisHealthSnapshot>) {
...
function updateRedisMetrics(snapshot: RedisHealthSnapshot) {
  ...
  if (typeof snapshot.pingLatencyMs === "number") setMetric("tracksyra_redis_ping_latency_ms", { source: snapshot.redisConnectionSource }, snapshot.pingLatencyMs);
  if (typeof snapshot.infoLatencyMs === "number") setMetric("tracksyra_redis_info_latency_ms", { source: snapshot.redisConnectionSource }, snapshot.infoLatencyMs);
...
function redactRedisUrl(redisUrl: string | null | undefined) {
  if (!redisUrl) return null;
  try {
    const parsed = new URL(redisUrl);
    if (parsed.password) parsed.password = "[REDACTED]";
    if (parsed.username) parsed.username = parsed.username ? "[REDACTED]" : "";
