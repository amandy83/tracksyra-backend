...
import type { BoomplayDeliveryJob, BoomplayHealthJob, BoomplayPollingJob, BoomplayRetryJob, BoomplayWebhookJob } from "../../queue/jobTypes";
...
import { BoomplayEnterpriseService } from "../../distribution/connectors/framework/boomplayConnector";

export type BoomplayWorkerDependencies = Readonly<{
...
function resolveReleaseId(job: BoomplayDeliveryJob | BoomplayPollingJob | BoomplayRetryJob | BoomplayWebhookJob | BoomplayHealthJob): string {
...
function buildDeliveryJob(job: BoomplayDeliveryJob | BoomplayPollingJob | BoomplayRetryJob | BoomplayWebhookJob | BoomplayHealthJob): DSPDeliveryJob {
  if (!job.release) {
...
async function recordAudit(
  dependencies: BoomplayWorkerDependencies,
  input: Readonly<{
    aggregateId: string;
    action: string;
...
export class BoomplayDeliveryWorker {
  constructor(private readonly dependencies: BoomplayWorkerDependencies) {}

  async process(job: BoomplayDeliveryJob): Promise<void> {
...
export class BoomplayPollingWorker {
  ...
  async process(job: BoomplayPollingJob): Promise<void> {
    const deliveryJob = buildDeliveryJob(job);
    const packageModel = await this.dependencies.enterpriseService.buildPackage(deliveryJob);
...
export class BoomplayRetryWorker {
  ...
  async process(job: BoomplayRetryJob): Promise<void> {
    const deliveryJob = buildDeliveryJob(job);
    const packageModel = await this.dependencies.enterpriseService.buildPackage(deliveryJob);
...
export class BoomplayWebhookWorker {
  ...
  async process(job: BoomplayWebhookJob): Promise<void> {
    const deliveryJob = buildDeliveryJob(job);
    const packageModel = await this.dependencies.enterpriseService.buildPackage(deliveryJob);
...
export class BoomplayHealthWorker {
  ...
  async process(job: BoomplayHealthJob): Promise<void> {
    const deliveryJob = buildDeliveryJob(job);
    const packageModel = await this.dependencies.enterpriseService.buildPackage(deliveryJob);
...
export function registerBoomplayDeliveryWorker(worker: BoomplayDeliveryWorker, options: { concurrency?: number } = {}): WorkerLike {
  return createWorker(queueNames.boomplayDelivery, async (job) => {
    await worker.process(job.data as BoomplayDeliveryJob);
  }, { concurrency: options.concurrency });
}

export function registerBoomplayPollingWorker(worker: BoomplayPollingWorker, options: { concurrency?: number } = {}): WorkerLike {
  return createWorker(queueNames.boomplayPolling, async (job) => {
    await worker.process(job.data as BoomplayPollingJob);
  }, { concurrency: options.concurrency });
}

export function registerBoomplayRetryWorker(worker: BoomplayRetryWorker, options: { concurrency?: number } = {}): WorkerLike {
  return createWorker(queueNames.boomplayRetry, async (job) => {
    await worker.process(job.data as BoomplayRetryJob);
  }, { concurrency: options.concurrency });
}

export function registerBoomplayWebhookWorker(worker: BoomplayWebhookWorker, options: { concurrency?: number } = {}): WorkerLike {
  return createWorker(queueNames.boomplayWebhook, async (job) => {
    await worker.process(job.data as BoomplayWebhookJob);
  }, { concurrency: options.concurrency });
}

export function registerBoomplayHealthWorker(worker: BoomplayHealthWorker, options: { concurrency?: number } = {}): WorkerLike {
  return createWorker(queueNames.boomplayHealth, async (job) => {
    await worker.process(job.data as BoomplayHealthJob);
  }, { concurrency: options.concurrency });
}
