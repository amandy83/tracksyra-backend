import { closeQueues, getQueueMetrics, pauseQueue, resumeQueue, type WorkerLike } from "../../queue/queueFactory";
import { closeRedisConnection, checkRedisHealth, getRedisHealthSnapshot, type QueueEnvironment } from "../../queue/redis";
...
import { createQueueEvents, createScheduler } from "../../queue/queueFactory";
...
export type WorkerRuntimeDependencies = Readonly<{
  logger: Logger;
...
export class WorkerRuntime {
  ...
  constructor(
    ...
    private readonly queueEnvironment: () => QueueEnvironment,
  ) {}

  async startupHealthCheck() {
    logRuntimeEnv("worker-runtime");
    const validation = validateProductionEnvironment();
    for (const warning of validation.warnings) this.logger.warn("environment validation warning", { component: "worker-runtime", warning });
...
export function createQueueSchedulers() {
  return productionQueueNames
    ...
    .filter((scheduler): scheduler is JobScheduler => Boolean(scheduler));
}
