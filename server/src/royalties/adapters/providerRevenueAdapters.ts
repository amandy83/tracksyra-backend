...
function parseProviderRows(source: RevenueImportSource, input: unknown, fallbackDsp?: string): RevenueImportResult {
  const rawRows = Array.isArray(input) ? input as ProviderRow[] : input && typeof input === "object" && Array.isArray((input as { rows?: unknown }).rows) ? (input as { rows?: ProviderRow[] }).rows as ProviderRow[] : [];
